# Evidence pack: BOSTON GEOSPATIAL, INC.
slug: boston-geospatial-inc | domain: bostongeospatial.com | HQ: Boston, Massachusetts US | employees(LI): 2 | founded:  | stage_band: revenue_stage
sources: sbir
industry(LI): Information Services | specialties: 
channel notes: SBIR PhII x1 [2023] Air Force latest: Protecting Critical Infrastructure with Near Real-Time Geospatial Intelligence hq: Boston,MA emp: 3
own-language word count: ~983

## Their own words (website via Exa, livecrawl-preferred)

URL: https://bostongeospatial.com
Home | Boston Geospatial

top of page

# ACTIONABLE, READY-TO-USE GEOSPATIAL INTELLIGENCE

Identify, measure, and manage exposure to natural threats with Boston Geospatial

## MISSION

We're Changing the Way Organizations Manage Natural Threat Risk

Boston Geospatial measures where and how the planet is changing every day, empowering organizations to understand and adapt to natural threats. By providing screening tools as well as monitoring and alert solutions, we are helping decision makers improve their outcomes.

## OFFERINGS

Boston Geospatial offers a comprehensive set of solutions for its clients along their entire risk management journey.

### RISK SCREENING

Evaluate where your assets are exposed to potential threats using curated datasets for risks such as geohazards, flooding, fire, and more. These datasets build off of insights from prior incidents, terrain digital twins, and other advanced risk modeling.

### NEAR REAL-TIME ALERTS

Receive alerts of and contextualized insights about emerging threats from a mix of terrestrial and space-based sensing networks. Manage and prioritize response efforts, inspections, and other related activities to compliance and continued operations.

### MONITORING

Detect and measure your asset’s exposure to geohazards and other natural and anthropogenic threats using our automated and scalable geospatial processing platform. We do this by leveraging optical imaging change detection and classification as well as satellite radar interferometry and polarimetry. Through our satellite operator data partner network, we can tailor historical surveys and monitoring campaigns towards virtually any threat type.

### INTEGRATIONS & COMPATIBILITY

Our insights are fully integrated or compatible with several commercial and government platforms. With a growing customer base and channel partner network, Boston Geospatial is constantly expanding into other interfaces.

## PARTNERS

HIGH IMPACT, TURNKEY INTELLIGENCE DELIVERED

As a dual-use company, Boston Geospatial is helping critical infrastructure owners and operators be safer, more prepared, and more successful in a world with a changing threat landscape and evolving regulatory environment.

## GOVERNMENT

## COMMERCIAL

bottom of page

URL: https://www.bostongeospatial.com/
Home | Boston Geospatial

top of page

# ACTIONABLE, READY-TO-USE GEOSPATIAL INTELLIGENCE

Identify, measure, and manage exposure to natural threats with Boston Geospatial

## MISSION

We're Changing the Way Organizations Manage Natural Threat Risk

Boston Geospatial measures where and how the planet is changing every day, empowering organizations to understand and adapt to natural threats. By providing screening tools as well as monitoring and alert solutions, we are helping decision makers improve their outcomes.

## OFFERINGS

Boston Geospatial offers a comprehensive set of solutions for its clients along their entire risk management journey.

### RISK SCREENING

Evaluate where your assets are exposed to potential threats using curated datasets for risks such as geohazards, flooding, fire, and more. These datasets build off of insights from prior incidents, terrain digital twins, and other advanced risk modeling.

### NEAR REAL-TIME ALERTS

Receive alerts of and contextualized insights about emerging threats from a mix of terrestrial and space-based sensing networks. Manage and prioritize response efforts, inspections, and other related activities to compliance and continued operations.

### MONITORING

Detect and measure your asset’s exposure to geohazards and other natural and anthropogenic threats using our automated and scalable geospatial processing platform. We do this by leveraging optical imaging change detection and classification as well as satellite radar interferometry and polarimetry. Through our satellite operator data partner network, we can tailor historical surveys and monitoring campaigns towards virtually any threat type.

### INTEGRATIONS & COMPATIBILITY

Our insights are fully integrated or compatible with several commercial and government platforms. With a growing customer base and channel partner network, Boston Geospatial is constantly expanding into other interfaces.

## PARTNERS

HIGH IMPACT, TURNKEY INTELLIGENCE DELIVERED

As a dual-use company, Boston Geospatial is helping critical infrastructure owners and operators be safer, more prepared, and more successful in a world with a changing threat landscape and evolving regulatory environment.

## GOVERNMENT

## COMMERCIAL

bottom of page

URL: mailto:[email]
Home | Boston Geospatial

top of page

# ACTIONABLE, READY-TO-USE GEOSPATIAL INTELLIGENCE

Identify, measure, and manage exposure to natural threats with Boston Geospatial

## MISSION

We're Changing the Way Organizations Manage Natural Threat Risk

Boston Geospatial measures where and how the planet is changing every day, empowering organizations to understand and adapt to natural threats. By providing screening tools as well as monitoring and alert solutions, we are helping decision makers improve their outcomes.

## OFFERINGS

Boston Geospatial offers a comprehensive set of solutions for its clients along their entire risk management journey.

### RISK SCREENING

Evaluate where your assets are exposed to potential threats using curated datasets for risks such as geohazards, flooding, fire, and more. These datasets build off of insights from prior incidents, terrain digital twins, and other advanced risk modeling.

### NEAR REAL-TIME ALERTS

Receive alerts of and contextualized insights about emerging threats from a mix of terrestrial and space-based sensing networks. Manage and prioritize response efforts, inspections, and other related activities to compliance and continued operations.

### MONITORING

Detect and measure your asset’s exposure to geohazards and other natural and anthropogenic threats using our automated and scalable geospatial processing platform. We do this by leveraging optical imaging change detection and classification as well as satellite radar interferometry and polarimetry. Through our satellite operator data partner network, we can tailor historical surveys and monitoring campaigns towards virtually any threat type.

### INTEGRATIONS & COMPATIBILITY

Our insights are fully integrated or compatible with several commercial and government platforms. With a growing customer base and channel partner network, Boston Geospatial is constantly expanding into other interfaces.

## PARTNERS

HIGH IMPACT, TURNKEY INTELLIGENCE DELIVERED

As a dual-use company, Boston Geospatial is helping critical infrastructure owners and operators be safer, more prepared, and more successful in a world with a changing threat landscape and evolving regulatory environment.

## GOVERNMENT

## COMMERCIAL

bottom of page

## LinkedIn about



## SBIR/STTR record (their own abstracts)

### Protecting Critical Infrastructure with Near Real-Time Geospatial Intelligence — Air Force 2023 ($1,249,955)
Founded in 2020, Boston Geospatial (BG) brings near real-time natural threat intelligence to operators and decision makers using its sensor agnostic radar processing platform. The Air Force Civil Engineering Center (AFCEC) and civil engineering squadrons

## News (Exa, top 3)
- Boston Geospatial, Inc. () https://govtribe.com/vendors/boston-geospatial-inc-dot-8fnd4
  Boston Geospatial, Inc.

Boston Geospatial, Inc.
- Boston Geospatial Profile () https://www.highergov.com/awardee/boston-geospatial-inc-12696186/
  Boston Geospatial Profile

HigherGov

Notify

- New Contract Awards
- New Grant Awards

Favorite

Export

- CSV
- Excel

Share

- Copy URL
- Email
- Tweet

# Boston Geospatial

UEI: QBJ5B38RCRH4 &#x2022 CAGE: 8FND4

- List
- Text

## Overview

Awardee Type

Parent

Name

Boston Geospatial

Website

Headquarters

Boston, MA United States

Most Recent Award

Oct. 7, 2025

Calculating

##### Show Quick Stats (See Federal Award Analysis for Full Details)

Federal contract and grant recipient BOSTON 
- boston geospatial (2026-06-25) https://bostongeospatial.com/
  # Boston Geospatial (Boston Geospatial, Inc.)

Boston Geospatial is a Information Services company. Boston Geospatial is a company that provides actionable, ready-to-use geospatial intelligence to identify, measure, and manage exposure to natural threats. They offer screening tools, monitoring, and alert solutions to help decision makers improve their outcomes. Boston Geospatial employs 2 people. Headquartered in Boston, Massachusetts, United States. Has $1.6M in total funding, with 3 prior fund

## Open roles
(no jobs data found)

---
# Context for the scoring agent (do not re-search)

## Barry Hess niche
Defense and intelligence technology companies in the $5M-$10M range whose proven prototype is stuck between demo and fielded deployment, guided by a CTO who fielded a patented tactical Android app platform with operators in theater and now engineers deployment architecture and acquisition advocacy from day one.

**EDP:** When months-to-field exceeds the funded months remaining on the contract, the program dies in the DoD valley of death - and for a $5M-$10M defense tech company, one dead transition is often the company.

## Deployed-reality checklist (score absence against this)
- **DDIL**: denied/disconnected/intermittent/low-bandwidth comms. Canonical example: an AI
  crowd-detection startup streamed 30fps video; the contested environment had bandwidth for
  1 frame every 5 seconds. One architecture change (frame-rate decoupling) saved the project.
- **SWaP-C**: size, weight, power, cost on the platform actually carried/mounted.
- **ATO/RMF**: Authority to Operate on government networks; STIGs, POA&Ms, IL4/IL5.
- **CMMC / NIST 800-171**: company-level compliance before the contract dies.
- **ITAR/export**: controlled data handling.
- **Operator training & sustainment**: who trains the E-5, who fixes it in the field, spares,
  battery logistics, GFE integration.
- **Advocacy chain**: a champion above the operator level; operators loving it is not adoption.
- **Prime/flow-down mechanics**: subcontract structure, government back-end integration.
- **TAK ecosystem**: if it touches situational awareness and never mentions ATAK/TAK, ask why.


## Doctrine
# Targeting doctrine brief (inject into every dossier & company agent — do NOT re-read the corpus)

Distilled from Jordan Crawford's Cannonball/Blueprint corpus, 2026-07-23.

1. **Pain-Qualified Segment beats ICP.** Never target firmographics ("50-200 person SaaS").
   Target companies in an observable painful SITUATION right now. The situation must be
   provable from public data.
2. **Existential Data Point (EDP).** For each niche, name the single number that kills the
   company if the problem stays unfixed (e.g. equipment utilization 60% vs 80% margins;
   fleet utilization <70% = crisis). Every dossier must state the EDP of its CTO's target
   situation: "what does it cost a company, in what metric, to leave this unfixed?"
3. **Jobs are confessions.** A job posting is a company publicly describing its own pain.
   We match jobs whose EXISTENCE signals a problem the CTO has already fixed — not jobs the
   CTO would apply to. "First Head of Engineering" = no technical leadership. "Integration
   Engineer (Epic, HL7)" = drowning in healthcare plumbing.
4. **The message is a redescription of the targeting.** If targeting is precise, the message
   writes itself by describing what we found. The reader should think "how do they know that?"
5. **Message-as-gift (GTM Shift letter standard).** Conversational, unhurried, 4–6 short
   sentences. Opens with a verified surprising fact. Every claim traces to a named public
   source. Valuable even if never answered. One light question, under 15 words. Bans: "I
   noticed", flattery, "hope you're well", meeting asks, personal details about the recipient
   as the opener. Lead with the CTO's expertise matched to the company's evident problem.
6. **Niche = Industry × Situation × Unique Angle** — all three specific (Ben Horowitz
   principle: not best botanist; best Japanese botanist specializing in Zen gardens).
7. **Insight tests**: counterintuitive (would other CTOs argue?), specific (exact tools/
   patterns), quantifiable (before/after numbers), demonstrable (showable in 5 minutes).
8. **Evidence honesty.** Actions > words > database records. Aggregator/API data (Blitz,
   FullEnrich) never corroborates itself — load-bearing claims need the person's or
   company's OWN public artifacts. Date every claim; label estimates as estimates.
9. **Dropping a bad fit is success.** Never pad a list to hit a count.
