# Evidence pack: Gambit AI
slug: gambit-ai | domain: gambit.us | HQ: Los Angeles, California US | employees(LI): 37 | founded: 2023 | stage_band: preseed_seed
sources: exa-agent:ai-ml-isr
industry(LI): Software Development | specialties: 
channel notes: Seed $9.8M total seed funding reported. dom: ai_ml status: pilot dod: Gambit conducted a defense field exercise with AWS and Anduril that validated its ALIEN AI platform for autonomous swarm coordination. hq: Los Angeles, CA emp: 19 src: https://gambit.us/
own-language word count: ~1363

## Their own words (website via Exa, livecrawl-preferred)

URL: https://gambit.us
Gambit

# adaptive intelligencefor autonomoussystems

# Adaptive intelligencefor autonomy

True autonomy doesn’t just need more machines. It needs adaptive intelligence.

watch the demo

watch the demo

product

## PhysicalAI Stack.

Gambit is a software-first, platform-agnostic intelligence stack that enables teams to build, deploy, and evolve autonomous behaviors across any hardware environment.

### Behavior Builder

Create and train behaviors.

### Adaptive Intelligenceat the Edge

Run adaptive intelligence in real time.

### Intuitive Interface

Monitor and control missions.

Unified intelligence

## Not automation. Adaptation.

Not control, but coordination. Not isolation, but orchestration.Bringing machines together through unified intelligence.

Model

Learn behaviors across missions.

Adapt

Real-time multi-robot control.

operate

Cross-platfrom execution (air, land, sea).

interface

Human intent to collective action.

Levels of Robotic Intelligence

Autonomous Intelligence

## Autonomyspectrum.

Autonomy evolves from simple automation to fully adaptive, unified systems that learn and coordinate together.

### Automated

Systems that follow scripts but cannot think.

### autonomous

Systems that navigate independently but don’t collaborate.

### collaborative

Systems that coordinate but only within a controlled stack.

gambit’s peak autonomy

### adaptive

Systems that learn, collaborate, and adapt as a single brain.

adaptive

Systems that learn, collaborate, and adapt as a single brain.

Gambit is the highest level of autonomy, that can learn, collaborate, and adapt as unified intelligence.

collaborative

Systems that coordinate but only within a controlled stack.

autonomous

Systems that navigate independently but don’t collaborate.

Automated

Systems that follow scripts but cannot think.

Levels of Autonomous Intelligence

Robotic Intelligence

## Autonomyspectrum.

Autonomy evolves from simple automation to fully adaptive, unified systems that learn and coordinate together.

### Automated

Systems that follow scripts but cannot think.

### autonomous

Systems that navigate independently but don’t collaborate.

### collaborative

Systems that coordinate but only within a controlled stack.

gambit’s peak autonomy

### adaptive

Systems that learn, collaborate, and adapt as a single brain.

Industries

## Validated in defense. Designed for scale.

From industry to infrastructure, we scale wherever adaptive intelligent systems are needed.

Unlock your autonomous systems potential

Unlock your autonomous systems potential

Backed by

## ourinvestors.

Supported by forward-thinking organizations committed to shaping the future alongside us.

### GenLab

### Mana Ventures

### Next Frontier Capital

### Eclipse

### ethos

### riphean investments

### marlinspike

### GenLab

### Mana Ventures

### Next Frontier Capital

### Eclipse

### ethos

### riphean investments

### marlinspike

through understanding of human intent, gambit achieves real-world results today - unifying machine intelligence across all domains.

Our background

## Proven intough industries

Our team has worked across aerospace, defense, robotics, and advanced software companies.

### Virgin Hyperloop

### unity

### Space x

### U.S. Marine Corps

### U.S. Army

### U.S. Navy

### Cisco

### toyota

### northrop grumman

### GM

### lockheed martin

### DIU

### L3 Harris

### persistent systems

we know what it takes

## The teambehind it.

We’ve built rockets, autonomous vehicles, high fidelity simulators, and mission systems. We know what it takes to make hard tech work in the field.

linkedin

Josh giegel

founder & ceo

linkedin

ben richardson

CCo

linkedin

andrew kemendo

CTO

Josh Giegel

Founder & CEO

Ben Richardson

CCo

Andrew Kemendo

CTO

trusted by

## Partners and‍Customers

who advises us

## advisoryboard.

We’ve brought together top military and government leaders to guide our strategy. High level expertise spanning National Security, Acquisition, and Special Operations.

H.R. McMaster

Former national security advisor

Katharina McFarland

government procurement expert

Jim Smith

USSOCOM Acquisition Executive

H.R. McMaster

Former national security advisor

Katharina McFarland

government procurement expert

Jim Smith

USSOCOM Acquisition Executive

news

## News andupdates.

#### The World Cup Needs Adaptive Autonomy

GambitBlogreadread

#### Reach Power and Gambit Win OECIF Funding to Build Drone Swarms That Never Need to Land

Gambit & ReachNewsSWARM pairs adaptive intelligence with wireless power beaming to eliminate battery limits on autonomous operationsreadread

#### The Right Drone for the Right Mission Is Only Half the Problem

GambitBlogThe Navy is asking the right questions about what autonomous system fits which mission. The next question is what makes them work together when it counts.readread

#### Gambit Awarded Two Multi-Million Dollar Contracts by U.S. Air Force to Advance Adaptive Intelligence for Autonomous Systems

Newsreadread

#### Gambit and VICTUS Partner to Advance Coordinated Autonomy in Contested Environments

Newsreadread

#### From Systems to Systems-of-Systems: What the DAWG Budget Really Signals and the New Frontier of Autonomy

GambitBlogThere’s a lot of attention right now on the Pentagon’s proposed FY27 $54.6 billion budget for the Defense Autonomous Warfare Group (DAWG). A year ago, that figure was $225 million, making this a roughly 240x increase in a single budget cycle. Most of the commentary frames it as a massive bet on drones—and it is—but that is only part of the story.readread

#### Gambit Awarded OTA by U.S. Army GVSC to Advance Integrated C-UAS Simulation and Coordinated Autonomy

gambit aiBlogGambit has been awarded an OTA agreement by the U.S. Army Ground Vehicle Systems Center (GVSC) to integrate its adaptive AI platform into advanced simulation environments, enabling scalable testing of coordinated autonomous systems in complex C-UA

URL: https://gambit.us/
Gambit

# adaptive intelligencefor autonomoussystems

# Adaptive intelligencefor autonomy

True autonomy doesn’t just need more machines. It needs adaptive intelligence.

watch the demo

watch the demo

product

## PhysicalAI Stack.

Gambit is a software-first, platform-agnostic intelligence stack that enables teams to build, deploy, and evolve autonomous behaviors across any hardware environment.

### Behavior Builder

Create and train behaviors.

### Adaptive Intelligenceat the Edge

Run adaptive intelligence in real time.

### Intuitive Interface

Monitor and control missions.

Unified intelligence

## Not automation. Adaptation.

Not control, but coordination. Not isolation, but orchestration.Bringing machines together through unified intelligence.

Model

Learn behaviors across missions.

Adapt

Real-time multi-robot control.

operate

Cross-platfrom execution (air, land, sea).

interface

Human intent to collective action.

Levels of Robotic Intelligence

Autonomous Intelligence

## Autonomyspectrum.

Autonomy evolves from simple automation to fully adaptive, unified systems that learn and coordinate together.

### Automated

Systems that follow scripts but cannot think.

### autonomous

Systems that navigate independently but don’t collaborate.

### collaborative

Systems that coordinate but only within a controlled stack.

gambit’s peak autonomy

### adaptive

Systems that learn, collaborate, and adapt as a single brain.

adaptive

Systems that learn, collaborate, and adapt as a single brain.

Gambit is the highest level of autonomy, that can learn, collaborate, and adapt as unified intelligence.

collaborative

Systems that coordinate but only within a controlled stack.

autonomous

Systems that navigate independently but don’t collaborate.

Automated

Systems that follow scripts but cannot think.

Levels of Autonomous Intelligence

Robotic Intelligence

## Autonomyspectrum.

Autonomy evolves from simple automation to fully adaptive, unified systems that learn and coordinate together.

### Automated

Systems that follow scripts but cannot think.

### autonomous

Systems that navigate independently but don’t collaborate.

### collaborative

Systems that coordinate but only within a controlled stack.

gambit’s peak autonomy

### adaptive

Systems that learn, collaborate, and adapt as a single brain.

Industries

## Validated in defense. Designed for scale.

From industry to infrastructure, we scale wherever adaptive intelligent systems are needed.



URL: https://www.linkedin.com/in/josh-giegel/
# Joshua Giegel

Gambit Founder/CEO

Los Angeles, California, United States (US)

500 connections • 9,774 followers

## About

Founder & CEO of Gambit. Former co-founder Virgin Hyperloop, ex-SpaceX.

Total Experience: 19 years and 3 months

## Experience

### Founder CEO - [Gambit](https://www.linkedin.com/company/gambitdefense) (Current)

Feb 2023 - Present (3 years and 5 months) in Greater Los Angeles, California, United States

Department: General Management • Level: Founder

Gambit AI is a Software Development company. Gambit is a company that provides platform solutions for user experience and scalability. They focus on simplifying the user experience and supporting the accelerated adoption of attritable platforms across various sectors. Gambit AI has 10-20 employees (+50% YoY), founded in 2023. Headquartered in Los Angeles, California, United States. Has $9.8M in total funding, with 1 prior funding round.

### Strategic Advisor - Armada (Current)

Dec 2022 - Present (3 years and 6 months)

Department: Consulting • Level: Specialist

### CEO and Co-Founder - Virgin Hyperloop

Feb 2021 - Oct 2021 (8 months) in Los Angeles Metropolitan Area

Department: General Management • Level: Founder

### Chief Technology Officer - Virgin Hyperloop

Oct 2017 - Feb 2021 (3 years and 4 months)

Department: Engineering and Technical • Level: C-Level

### President of Engineering - Virgin Hype

## LinkedIn about

We\'re proving that true autonomy doesn\'t just need more machines. It needs unified, adaptive intelligence.

## News (Exa, top 3)
- Gambit Awarded Two Multi-Million Dollar Contracts by U.S. Air Force to ... (2026-04-22) https://www.gambit.us/news-and-blogs/gambit-awarded-two-multi-million-dollar-contracts-by-u-s-air-force-to-advance-adaptive-intelligence-for-autonomous-systems
  Gambit Awarded Two Multi-Million Dollar Contracts by U.S. Air Force to Advance Adaptive Intelligence for Autonomous Systems

blog

# Gambit Awarded Two Multi-Million Dollar Contracts by U.S. Air Force to Advance Adaptive Intelligence for Autonomous Systems

by

April 22, 2026

Share this article

link copied!

Los Angeles, Calif. (April 22, 2026) – Gambit, developer of a hardware-agnostic autonomy orchestration platform for robotic systems, announced it has been awarded two multi-million dollar 
- Award | SBIR () https://www.sbir.gov/awards/216352
  Award | SBIR Skip to main content

# Award

Portfolio Data

Back to Award Search

## Dedicated Counter UAS Opposing Forces Utilizing AI/ML: CORTEX

### Awardee

#### Gambit Defense, Inc.

4817 RADFORD AVE PMB 98160VALLEY VILLAGE, CA, 91607-3720USA

Award Year: 2025

UEI: XC2HQCWYPMW4

HUBZone Owned: No

Woman Owned: No

Socially and Economically Disadvantaged: No

Congressional District: N/A

Tagged as:

STTR

Phase II

### Awarding Agency

DOD

Branch: USAF

Total Award Amount: $1,788,872

Cont
- Gambit Emerges from Stealth with Adaptive Intelligence Layer for Autonomous Military Systems (2026-01-07) https://www.prnewswire.com/news-releases/gambit-emerges-from-stealth-with-adaptive-intelligence-layer-for-autonomous-military-systems-302654449.html
  Gambit Emerges from Stealth with Adaptive Intelligence Layer for Autonomous Military Systems Accessibility Statement Skip Navigation

Former Virgin Hyperloop CEO/CTO and DoW Critical Technologies Director deploy physical AI operating system for U.S. Department of War; contract vehicles worth $500M+ as demand for multi-system coordination surges

LOS ANGELES, Jan. 7, 2026 /PRNewswire/ -- Gambit, the dual-use defense company building adaptive intelligence for autonomous systems, emerged from steal

## Open roles
(no jobs data found)

---
# Context for the scoring agent (do not re-search)

## Barry Hess niche
Defense and intelligence technology companies in the $5M-$10M range whose proven prototype is stuck between demo and fielded deployment, guided by a CTO who fielded a patented tactical Android app platform with operators in theater and now engineers deployment architecture and acquisition advocacy from day one.

**EDP:** When months-to-field exceeds the funded months remaining on the contract, the program dies in the DoD valley of death - and for a $5M-$10M defense tech company, one dead transition is often the company.

## Deployed-reality checklist (score absence against this)
- **DDIL**: denied/disconnected/intermittent/low-bandwidth comms. Canonical example: an AI
  crowd-detection startup streamed 30fps video; the contested environment had bandwidth for
  1 frame every 5 seconds. One architecture change (frame-rate decoupling) saved the project.
- **SWaP-C**: size, weight, power, cost on the platform actually carried/mounted.
- **ATO/RMF**: Authority to Operate on government networks; STIGs, POA&Ms, IL4/IL5.
- **CMMC / NIST 800-171**: company-level compliance before the contract dies.
- **ITAR/export**: controlled data handling.
- **Operator training & sustainment**: who trains the E-5, who fixes it in the field, spares,
  battery logistics, GFE integration.
- **Advocacy chain**: a champion above the operator level; operators loving it is not adoption.
- **Prime/flow-down mechanics**: subcontract structure, government back-end integration.
- **TAK ecosystem**: if it touches situational awareness and never mentions ATAK/TAK, ask why.


## Doctrine
# Targeting doctrine brief (inject into every dossier & company agent — do NOT re-read the corpus)

Distilled from Jordan Crawford's Cannonball/Blueprint corpus, 2026-07-23.

1. **Pain-Qualified Segment beats ICP.** Never target firmographics ("50-200 person SaaS").
   Target companies in an observable painful SITUATION right now. The situation must be
   provable from public data.
2. **Existential Data Point (EDP).** For each niche, name the single number that kills the
   company if the problem stays unfixed (e.g. equipment utilization 60% vs 80% margins;
   fleet utilization <70% = crisis). Every dossier must state the EDP of its CTO's target
   situation: "what does it cost a company, in what metric, to leave this unfixed?"
3. **Jobs are confessions.** A job posting is a company publicly describing its own pain.
   We match jobs whose EXISTENCE signals a problem the CTO has already fixed — not jobs the
   CTO would apply to. "First Head of Engineering" = no technical leadership. "Integration
   Engineer (Epic, HL7)" = drowning in healthcare plumbing.
4. **The message is a redescription of the targeting.** If targeting is precise, the message
   writes itself by describing what we found. The reader should think "how do they know that?"
5. **Message-as-gift (GTM Shift letter standard).** Conversational, unhurried, 4–6 short
   sentences. Opens with a verified surprising fact. Every claim traces to a named public
   source. Valuable even if never answered. One light question, under 15 words. Bans: "I
   noticed", flattery, "hope you're well", meeting asks, personal details about the recipient
   as the opener. Lead with the CTO's expertise matched to the company's evident problem.
6. **Niche = Industry × Situation × Unique Angle** — all three specific (Ben Horowitz
   principle: not best botanist; best Japanese botanist specializing in Zen gardens).
7. **Insight tests**: counterintuitive (would other CTOs argue?), specific (exact tools/
   patterns), quantifiable (before/after numbers), demonstrable (showable in 5 minutes).
8. **Evidence honesty.** Actions > words > database records. Aggregator/API data (Blitz,
   FullEnrich) never corroborates itself — load-bearing claims need the person's or
   company's OWN public artifacts. Date every claim; label estimates as estimates.
9. **Dropping a bad fit is success.** Never pad a list to hit a count.
