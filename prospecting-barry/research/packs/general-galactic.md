# Evidence pack: General Galactic
slug: general-galactic | domain: gengalactic.com | HQ: El Segundo, California US | employees(LI): 17 | founded: 2023 | stage_band: unknown
sources: vc:harpoon
industry(LI): Space Research and Technology | specialties: 
channel notes: in-space mobility / fuel synthesis
own-language word count: ~983

## Their own words (website via Exa, livecrawl-preferred)

URL: https://gengalactic.com
General Galactic

# Built for the high frontier.

General Galactic is building a railroad to Mars - and creating a new class of spacecraft for rapid logistics around the Earth and across the solar system.

Fly With USFly With US

s

s

g

u

z

d

d

g

s

r

Vision

Our platforms offer best-in-class thrust and efficiency, which means more delta-V for your mission at a lower cost.We deliver faster and cheaper access to orbits, enabling transformative commercial opportunities and renewed space superiority.

Learn more about our spacecraft

TEchnology

Advanced satellites & propulsion that change the equation for space operations.

Learn more about Genesis

Upcoming MissionS

Trinity

LEO

First spaceflight demonstration of Genesis. Delivering unprecedented delta-V and agility.

Request infoRequest info

Future

GEO

Showcasing sustained maneuver, RPO, and life extension capabilities in Geostationary orbit.

FLY WITh USFLY WITh US

Future

CISLUNAR

Delivering cargo to the lunar surface and other high-energy orbits, faster and cheaper than ever before.

FLY WITH USFLY WITH US

Backed by leading hard-tech investors

CAreers

Strategy & OperationsEl Segundo, CAFull Time

VP of Government RelationsEl Segundo, CAFull Time

Director of EngineeringEl Segundo, CAFull Time

Test / Automation EngineerEl Segundo, CAFull Time

Structures EngineerEl Segundo, CAFull Time

Senior GNC & Mission Design EngineerEl Segundo, CAFull Time

Propulsion EngineerEl Segundo, CAFull Time

Power Electronics Engineer (Electric Propulsion)El Segundo, CAFull Time

Mechanical EngineerEl Segundo, CAFull Time

Mechanical Design Engineer (Electric Propulsion)El Segundo, CAFull Time

Fluids EngineerEl Segundo, CAFull Time

Embedded Software EngineerEl Segundo, CAFull Time

Electrolyzer Mechanical Design EngineerEl Segundo, CAFull Time

Director of SoftwareEl Segundo, CAFull Time

Digital Content & Marketing CoordinatorEl Segundo, CAFull Time

Choose your own adventureEl Segundo, CAFull Time

Embedded Software EngineerEl Segundo, CAFull Time

Didn't see your dream job? Send us your resume: [email]

Updates

General Galactic Aims to Become the Galaxy's Energy and Logistics CompanyNews05.03.2026

General Galactic Aims to Become the Galaxy's Energy and Logistics CompanyNews05.03.2026

This Startup Thinks It Can Make Rocket Fuel From Water. Stop LaughingNews09.02.2026

This Startup Thinks It Can Make Rocket Fuel From Water. Stop LaughingNews09.02.2026

Railroad to Mars: Payload PocastInsight04.12.2025

Railroad to Mars: Payload PocastInsight04.12.2025

113 Sierra StreetEl Segundo, California 90245

[phone] [email]

Tech

Propulsion Spacecraft Nuclear PropellantGeneration

Company

About Updates Careers

Contact

Copyright © 2026 | general galactic technologies corporation

Privacy Policy Terms & Conditions

URL: https://gengalactic.com/missions
Missions

Mission Manifest

s

s

g

u

z

d

d

g

Leaving the planet should be easy. We're developing an entirely new way to fly in space, and making the journey simple.

development Roadmap

Expanding human activities across the inner solar system, within our lifetime.

2024

Prototype electrolyzers and Sabatier reactors

2025

H2/O2 chemical and electric thrusters developed

2026

First spaceflight

2027

Operational missions start in LEO/MEO/GEO

2028

Missions to lunar orbit begin

2030

Cargo missions to Mars

2035

Infrastructure deployed on the Moon and Mars

2122

General Galactic ship Nostromo disappears

Upcoming MissionS

Trinity

LEO

First spaceflight demonstration of Genesis. Delivering unprecedented delta-V and agility.

Request infoRequest info

Future

GEO

Showcasing sustained maneuver, RPO, and life extension capabilities in Geostationary orbit.

FLY WITh USFLY WITh US

Future

CISLUNAR

Delivering cargo to the lunar surface and other high-energy orbits, faster and cheaper than ever before.

FLY WITH USFLY WITH US

Latest Updates

General Galactic Aims to Become the Galaxy's Energy and Logistics CompanyNews05.03.2026

General Galactic Aims to Become the Galaxy's Energy and Logistics CompanyNews05.03.2026

This Startup Thinks It Can Make Rocket Fuel From Water. Stop LaughingNews09.02.2026

This Startup Thinks It Can Make Rocket Fuel From Water. Stop LaughingNews09.02.2026

Railroad to Mars: Payload PocastInsight04.12.2025

Railroad to Mars: Payload PocastInsight04.12.2025

113 Sierra StreetEl Segundo, California 90245

[phone] [email]

Tech

Propulsion Spacecraft Nuclear PropellantGeneration

Company

About Updates Careers

Contact

Copyright © 2026 | general galactic technologies corporation

Privacy Policy Terms & Conditions

URL: https://gengalactic.com/tech/spacecraft
Spacecraft | Tech | General Galactic

space vehicle development

# Spacecraft for a new generation.

## A galactic fleet of highly capable space vehicles.

s

s

g

u

z

d

d

g

Space is no longer a benign environment. Spacecraft must be agile, more capable, and more resilient. The General Galactic fleet will be the fastest, most cost efficient, and most flexible way to operate in the space domain. Our armada will host and transit payloads for applications across the commercial, science, and defense sectors.

# Seldon Spacecraft

# FULL PLATE / ESPA CLASS

b

km/s of high thrust delta-V

b

400kg+ payload capacity

b

Rapid transit to GEO & beyond

b

1.5kW+ payload power

b

Autonomous RPO-ready

b

Multi-mission reusability

# Dreadnought Spacecraft

# MULTI-TONne CARGO CLASS

In development.

113 Sierra StreetEl Segundo, California 90245

[phone] [email]

Tech

Propulsion Spacecraft Nuclear PropellantGeneration

Company

About Updates Careers

Contact

Copyright © 2026 | general galactic technologies corporation

Privacy Policy Terms & Conditions

URL: https://gengalactic.com/tech/propulsion
Propulsion | Tech | General Galactic

Genesis Propulsion

# Building Genesis

## Changing the fundamental equation for space mission economics

s

s

g

u

z

d

d

g

Space travel is limited by propulsive capability. Today's spacecraft engines are often less efficient than their predecessors from the first space race. General Galactic is building the propulsion platform for the next generation of space missions, infrastructure, & exploration.

b

km/s of high thrust delta-V

b

Chem & EP multi-mode

b

Rapid transit to GEO & beyond

Game Changing Impulse

True multi-mode capability means Genesis can provide delta-V in whatever form an operator needs, whenever it is needed – opening up completely new mission architectures.

Abundant Propellant

General Galactic's Genesis propulsion technology uses water as its only fuel. Unlike previous water-based engines, this technology outperforms toxic legacy propellants.

Pushing the Limits

We design, analyze, test, and manufacture our engines in house in El Segundo, California. We've demonstrated successful operation of our high-thrust chemical and high-efficiency electric engine prototypes.

113 Sierra StreetEl Segundo, California 90245

[phone] [email]

Tech

Propulsion Spacecraft Nuclear PropellantGeneration

Company

About Updates Careers

Contact

Copyright © 2026 | general galactic technologies corporation

Privacy Policy Terms & Conditions

## LinkedIn about

General Galactic is a deep-tech startup manufacturing platforms for space mobility and propellant generation.  

## News (Exa, top 3)
- Hypergiant Galactic Systems Awarded $61.4 Million U.S. Air Force Contract - SiliconHills (2023-05-05) https://www.siliconhillsnews.com/2023/05/04/hypergiant-galactic-systems-awarded-61-4-million-u-s-air-force-contract/
  Hypergiant Galactic Systems Awarded $61.4 Million U.S. Air Force Contract - SiliconHills

# Hypergiant Galactic Systems Awarded $61.4 Million U.S. Air Force Contract

Hypergiant Galactic Systems has received a $61.4 million Small Business Innovation Research Phase III contract from the U.S. Air Force.

The company, based in Blanco, received the contract to provide user interface and user experience development services for cloud-based command and control.

Hypergiant will perform the work in Bla
- General Galactic Raises $8M to Advance Fossil-Free Fuel Revolution (2024-11-12) https://www.businesswire.com/news/home/20241112893754/en/General-Galactic-Raises-%248M-to-Advance-Fossil-Free-Fuel-Revolution
  General Galactic Raises $8M to Advance Fossil-Free Fuel Revolution

Nov 12, 2024 7:00 AM Eastern Standard Time

# General Galactic Raises $8M to Advance Fossil-Free Fuel Revolution

Share

---

- Scalable solution for producing fossil-free hydrocarbon fuel from CO2 emissions has secured new funding from Harpoon Ventures, Refactor Capital and other investors
- Founded by former SpaceX and Varda Space Industries engineers Halen Mattison and Luke Neise, bringing aerospace industry engineering princ
- Umbra and Hypergiant among firms selected for Air Force JADC2 contract - SpaceNews (2021-07-16) https://spacenews.com/jadc2-idiq-950-million/
  Umbra and Hypergiant among firms selected for Air Force JADC2 contract - SpaceNews

Home/ Umbra and Hypergiant among firms selected for Air Force JADC2 contract

Umbra announced July 14 that its first synthetic aperture radar satellite's antenna successfully deployed in orbit. The patented antenna is comprised of a series of ribs attached to a central hub that opens to deploy the reflector. Cedit: Umbra

SAN FRANCISCO – The U.S. Air Force awarded indefinite-delivery, indefinite-quantity contract

## Open roles (Blitz, live)
- Choose your own adventure ({'city': 'El Segundo', 'country_code': 'US'})
- Embedded Software Engineer ({'city': 'El Segundo', 'country_code': 'US'})
- Digital Content & Marketing Coordinator ({'city': 'El Segundo', 'country_code': 'US'})
- Director of Software ({'city': 'El Segundo', 'country_code': 'US'})
- Embedded Software Engineer ({'city': 'El Segundo', 'country_code': 'US'})
- Electrolyzer Mechanical Design Engineer ({'city': 'El Segundo', 'country_code': 'US'})
- Fluids Engineer ({'city': 'El Segundo', 'country_code': 'US'})
- Mechanical Engineer ({'city': 'El Segundo', 'country_code': 'US'})
- Mechanical Design Engineer (Electric Propulsion) ({'city': 'El Segundo', 'country_code': 'US'})
- Power Electronics Engineer (Electric Propulsion) ({'city': 'El Segundo', 'country_code': 'US'})

---
# Context for the scoring agent (do not re-search)

## Barry Hess niche
Defense and intelligence technology companies in the $5M-$10M range whose proven prototype is stuck between demo and fielded deployment, guided by a CTO who fielded a patented tactical Android app platform with operators in theater and now engineers deployment architecture and acquisition advocacy from day one.

**EDP:** When months-to-field exceeds the funded months remaining on the contract, the program dies in the DoD valley of death - and for a $5M-$10M defense tech company, one dead transition is often the company.

## Deployed-reality checklist (score absence against this)
- **DDIL**: denied/disconnected/intermittent/low-bandwidth comms. Canonical example: an AI
  crowd-detection startup streamed 30fps video; the contested environment had bandwidth for
  1 frame every 5 seconds. One architecture change (frame-rate decoupling) saved the project.
- **SWaP-C**: size, weight, power, cost on the platform actually carried/mounted.
- **ATO/RMF**: Authority to Operate on government networks; STIGs, POA&Ms, IL4/IL5.
- **CMMC / NIST 800-171**: company-level compliance before the contract dies.
- **ITAR/export**: controlled data handling.
- **Operator training & sustainment**: who trains the E-5, who fixes it in the field, spares,
  battery logistics, GFE integration.
- **Advocacy chain**: a champion above the operator level; operators loving it is not adoption.
- **Prime/flow-down mechanics**: subcontract structure, government back-end integration.
- **TAK ecosystem**: if it touches situational awareness and never mentions ATAK/TAK, ask why.


## Doctrine
# Targeting doctrine brief (inject into every dossier & company agent — do NOT re-read the corpus)

Distilled from Jordan Crawford's Cannonball/Blueprint corpus, 2026-07-23.

1. **Pain-Qualified Segment beats ICP.** Never target firmographics ("50-200 person SaaS").
   Target companies in an observable painful SITUATION right now. The situation must be
   provable from public data.
2. **Existential Data Point (EDP).** For each niche, name the single number that kills the
   company if the problem stays unfixed (e.g. equipment utilization 60% vs 80% margins;
   fleet utilization <70% = crisis). Every dossier must state the EDP of its CTO's target
   situation: "what does it cost a company, in what metric, to leave this unfixed?"
3. **Jobs are confessions.** A job posting is a company publicly describing its own pain.
   We match jobs whose EXISTENCE signals a problem the CTO has already fixed — not jobs the
   CTO would apply to. "First Head of Engineering" = no technical leadership. "Integration
   Engineer (Epic, HL7)" = drowning in healthcare plumbing.
4. **The message is a redescription of the targeting.** If targeting is precise, the message
   writes itself by describing what we found. The reader should think "how do they know that?"
5. **Message-as-gift (GTM Shift letter standard).** Conversational, unhurried, 4–6 short
   sentences. Opens with a verified surprising fact. Every claim traces to a named public
   source. Valuable even if never answered. One light question, under 15 words. Bans: "I
   noticed", flattery, "hope you're well", meeting asks, personal details about the recipient
   as the opener. Lead with the CTO's expertise matched to the company's evident problem.
6. **Niche = Industry × Situation × Unique Angle** — all three specific (Ben Horowitz
   principle: not best botanist; best Japanese botanist specializing in Zen gardens).
7. **Insight tests**: counterintuitive (would other CTOs argue?), specific (exact tools/
   patterns), quantifiable (before/after numbers), demonstrable (showable in 5 minutes).
8. **Evidence honesty.** Actions > words > database records. Aggregator/API data (Blitz,
   FullEnrich) never corroborates itself — load-bearing claims need the person's or
   company's OWN public artifacts. Date every claim; label estimates as estimates.
9. **Dropping a bad fit is success.** Never pad a list to hit a count.
