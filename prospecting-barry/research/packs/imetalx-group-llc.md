# Evidence pack: IMETALX GROUP, LLC
slug: imetalx-group-llc | domain: imetalx.com | HQ: Sausalito, California US | employees(LI): 9 | founded: 2013 | stage_band: revenue_stage
sources: sbir
industry(LI): Space Research and Technology | specialties: 
channel notes: SBIR PhII x1 [2023] Air Force latest: Space Based Sensors for Safety and Situational Awareness for Future ISAM Vehicle Rendezvous hq: San Francisco,CA emp: 2
own-language word count: ~492

## Their own words (website via Exa, livecrawl-preferred)

URL: https://imetalx.com
iMetalx Space Systems

Seamless

---

Intelligent

---

Realtime

---

## VISION

### Decision Advantage in Characterization and Maneuver-Driven Space Operations

#### Our mission is to enable real-time perception, coordination, and resilient execution across dynamic orbital environments.

#### We develop high-confidence space object characterization and perception software aligned with U.S. national security mission priorities.

Our work extends to edge-based decision support, coordinated maneuver planning, resilient command and control, and secure integration across sensing and navigation architectures.

Space Battle Management

Space Control

Space Clearing

#### PORTFOLIO

#### Perception (THOR)

#### Simulation (ASGARD)

#### Intelligence (HEIMDALL)

#### Autonomy (ODIN)

LEADERSHIP TEAM

## Nehal Gajjar

CEO and Founder

Nehal Gajjar’s 20+ year career includes founding and leading three profitable high-tech startups, taking technology innovations from concept to commercialization in aerospace, defense, unmanned systems, renewables, power generation, and software space.

BS in Aerospace Engineering, Georgia-Tech

MBA, Carnegie Mellon University

## Alex Fleming

CTO & Cofounder

Alex Fleming has demonstrated over 20 years of innovation in unmanned undersea vehicles, vertical flight aircraft, military jet engines, propulsion and alternative energy systems. Alex also developed multiple UAS systems demonstrating a BLOS self-healing mesh network in 2016.

BS in Aerospace Engineering, Georgia-Tech

MBA, Carnegie Mellon University

### Our Office

Sausalito Shipyard and Marina

2350 Marinship Way, Sausalito, CA 94965

URL: https://imetalx.com/#about
iMetalx Space Systems

Seamless

---

Intelligent

---

Realtime

---

## VISION

### Decision Advantage in Characterization and Maneuver-Driven Space Operations

#### Our mission is to enable real-time perception, coordination, and resilient execution across dynamic orbital environments.

#### We develop high-confidence space object characterization and perception software aligned with U.S. national security mission priorities.

Our work extends to edge-based decision support, coordinated maneuver planning, resilient command and control, and secure integration across sensing and navigation architectures.

Space Battle Management

Space Control

Space Clearing

#### PORTFOLIO

#### Perception (THOR)

#### Simulation (ASGARD)

#### Intelligence (HEIMDALL)

#### Autonomy (ODIN)

LEADERSHIP TEAM

## Nehal Gajjar

CEO and Founder

Nehal Gajjar’s 20+ year career includes founding and leading three profitable high-tech startups, taking technology innovations from concept to commercialization in aerospace, defense, unmanned systems, renewables, power generation, and software space.

BS in Aerospace Engineering, Georgia-Tech

MBA, Carnegie Mellon University

## Alex Fleming

CTO & Cofounder

Alex Fleming has demonstrated over 20 years of innovation in unmanned undersea vehicles, vertical flight aircraft, military jet engines, propulsion and alternative energy systems. Alex also developed multiple UAS systems demonstrating a BLOS self-healing mesh network in 2016.

BS in Aerospace Engineering, Georgia-Tech

MBA, Carnegie Mellon University

### Our Office

Sausalito Shipyard and Marina

2350 Marinship Way, Sausalito, CA 94965

URL: https://imetalx.com/cart
iMetalx

iMetalx
[
0
](/cart)
[
Skip to Content
](#page)
[
![iMetalx](//images.squarespace-cdn.com/content/v1/627e99341baa1c678575c803/bb302633-1502-4b80-bfb5-2baa922fd293/MicrosoftTeams-image+%281%29.png?format=1500w)
](/)
Open Menu
Close Menu
Open Menu
Close Menu
[
![iMetalx](//images.squarespace-cdn.com/content/v1/627e99341baa1c678575c803/bb302633-1502-4b80-bfb5-2baa922fd293/MicrosoftTeams-image+%281%29.png?format=1500w)
](/)

## LinkedIn about

IMETALX\'s vision is to create a future where space is accessible and sustainable for all. Our mission is to provide in space domain awareness and in space servicing solutions for governments and businesses.

## SBIR/STTR record (their own abstracts)

### Space Based Sensors for Safety and Situational Awareness for Future ISAM Vehicle Rendezvous — Air Force 2023 ($1,680,508)
According to iARPA less than 1 percent of debris objects that could cause mission-endning damage are currently tracked and we have reached the point of no return of a self sustaining Kessler Syndrome. Space hazards of debris will potentially impact the gr

## News (Exa, top 3)
- Imetalx (GKPLZJBD9NY6) () https://www.highergov.com/awardee/imetalx-inc-10048393/
  Imetalx (GKPLZJBD9NY6)

HigherGov

Notify

- New Contract Awards
- New Grant Awards

Favorite

Export

- CSV
- Excel

Share

- Copy URL
- Email
- Tweet

# Imetalx

UEI: GKPLZJBD9NY6 &#x2022 CAGE: 6ZBZ3

- List
- Text

## Overview

Awardee Type

Parent

Federal Capability Statement

With average experience of an engineer over 20 plus years, iMetalx Group can help your organization solve your R&D engineering challenges quickly. We specialize in the fields of aerospace (from jet engines to drones) 
- PSIONIC and iMETALX Announce Strategic Collaboration (2026-02-11) https://www.globenewswire.com/news-release/2026/02/11/3236628/0/en/PSIONIC-and-iMETALX-Announce-Strategic-Collaboration-Supporting-RPOD-Space-Control-and-Space-Battle-Management-for-U-S-Space-Force-and-SDA-Missions.html
  PSIONIC and iMETALX Announce Strategic Collaboration

# PSIONIC and iMETALX Announce Strategic Collaboration Supporting RPOD, Space Control, and Space Battle Management for U.S. Space Force and SDA Missions

## Enabling autonomous decision advantage, distributed space control, and resilient command and control across contested orbital environments for multi-domain missions

February 11, 2026 15:34 ET | Source: iMetalx, Inc. iMetalx, Inc.

---

Washington, D.C. , Feb. 11, 2026 (GLOBE NEWSWIRE) --
- imetalx inc. (2026-06-26) https://imetalx.com/
  # iMETALX Inc. (IMETALX GROUP, LLC)

iMETALX Inc. is a Space Research and Technology company. iMetalX is a Space Systems Company serving US Space Force, US Air Force and Commercial Customers. They provide in-space servicing and space environment management solutions for governments and businesses. They have a proven record of rapid development in extreme environment autonomous systems. They have expertise in sensors fusion, edge compute, interoperable communication gateways, datacenters, overlay

## Open roles (Blitz, live)
- Senior-Principal Computer Vision Engineer ({'city': 'Sausalito', 'country_code': 'US'})
- Data Scientist Intern ({'city': None, 'country_code': None})
- Mission Analysis and Simulation Internship ({'city': 'Colorado Springs', 'country_code': 'US'})
- Data Scientist Intern ({'city': 'Colorado Springs', 'country_code': 'US'})
- Technical Marketing, Presentation & Branding Intern ({'city': 'Sausalito', 'country_code': 'US'})
- DevSecOps Engineer ({'city': 'Dayton', 'country_code': 'US'})
- Senior/Staff Software Engineer (Computer Vision Team) ({'city': 'Sausalito', 'country_code': 'US'})
- CyberSecurity & DevSecOps Engineer - Summer Internship 2026 ({'city': 'Sausalito', 'country_code': 'US'})
- Simulation Realism Engineer Internship - Summer 2026 ({'city': 'Sausalito', 'country_code': 'US'})
- Chief Software Architect ({'city': 'Sausalito', 'country_code': 'US'})

---
# Context for the scoring agent (do not re-search)

## Barry Hess niche
Defense and intelligence technology companies in the $5M-$10M range whose proven prototype is stuck between demo and fielded deployment, guided by a CTO who fielded a patented tactical Android app platform with operators in theater and now engineers deployment architecture and acquisition advocacy from day one.

**EDP:** When months-to-field exceeds the funded months remaining on the contract, the program dies in the DoD valley of death - and for a $5M-$10M defense tech company, one dead transition is often the company.

## Deployed-reality checklist (score absence against this)
- **DDIL**: denied/disconnected/intermittent/low-bandwidth comms. Canonical example: an AI
  crowd-detection startup streamed 30fps video; the contested environment had bandwidth for
  1 frame every 5 seconds. One architecture change (frame-rate decoupling) saved the project.
- **SWaP-C**: size, weight, power, cost on the platform actually carried/mounted.
- **ATO/RMF**: Authority to Operate on government networks; STIGs, POA&Ms, IL4/IL5.
- **CMMC / NIST 800-171**: company-level compliance before the contract dies.
- **ITAR/export**: controlled data handling.
- **Operator training & sustainment**: who trains the E-5, who fixes it in the field, spares,
  battery logistics, GFE integration.
- **Advocacy chain**: a champion above the operator level; operators loving it is not adoption.
- **Prime/flow-down mechanics**: subcontract structure, government back-end integration.
- **TAK ecosystem**: if it touches situational awareness and never mentions ATAK/TAK, ask why.


## Doctrine
# Targeting doctrine brief (inject into every dossier & company agent — do NOT re-read the corpus)

Distilled from Jordan Crawford's Cannonball/Blueprint corpus, 2026-07-23.

1. **Pain-Qualified Segment beats ICP.** Never target firmographics ("50-200 person SaaS").
   Target companies in an observable painful SITUATION right now. The situation must be
   provable from public data.
2. **Existential Data Point (EDP).** For each niche, name the single number that kills the
   company if the problem stays unfixed (e.g. equipment utilization 60% vs 80% margins;
   fleet utilization <70% = crisis). Every dossier must state the EDP of its CTO's target
   situation: "what does it cost a company, in what metric, to leave this unfixed?"
3. **Jobs are confessions.** A job posting is a company publicly describing its own pain.
   We match jobs whose EXISTENCE signals a problem the CTO has already fixed — not jobs the
   CTO would apply to. "First Head of Engineering" = no technical leadership. "Integration
   Engineer (Epic, HL7)" = drowning in healthcare plumbing.
4. **The message is a redescription of the targeting.** If targeting is precise, the message
   writes itself by describing what we found. The reader should think "how do they know that?"
5. **Message-as-gift (GTM Shift letter standard).** Conversational, unhurried, 4–6 short
   sentences. Opens with a verified surprising fact. Every claim traces to a named public
   source. Valuable even if never answered. One light question, under 15 words. Bans: "I
   noticed", flattery, "hope you're well", meeting asks, personal details about the recipient
   as the opener. Lead with the CTO's expertise matched to the company's evident problem.
6. **Niche = Industry × Situation × Unique Angle** — all three specific (Ben Horowitz
   principle: not best botanist; best Japanese botanist specializing in Zen gardens).
7. **Insight tests**: counterintuitive (would other CTOs argue?), specific (exact tools/
   patterns), quantifiable (before/after numbers), demonstrable (showable in 5 minutes).
8. **Evidence honesty.** Actions > words > database records. Aggregator/API data (Blitz,
   FullEnrich) never corroborates itself — load-bearing claims need the person's or
   company's OWN public artifacts. Date every claim; label estimates as estimates.
9. **Dropping a bad fit is success.** Never pad a list to hit a count.
