# Evidence pack: INHANCE DIGITAL CORPORATION
slug: inhance-digital-corporation | domain: inhance.com | HQ: Los Angeles, California US | employees(LI): 58 | founded: 1997 | stage_band: revenue_stage
sources: sbir
industry(LI): Media Production | specialties: interactive content design, modeling, ai, immersive technology, complex visualization, visual effects, creative storytelling, ux, video production, virtual environments, executive briefing  innovation centers, 3d animation, ui, software development, metaverse, immersive br experiences, training simu
channel notes: SBIR PhII x1 [2022] Air Force latest: Comprehensive Virtual Health/Combat Care Platform (VHP) hq: Los Angeles,CA emp: 38
own-language word count: ~929

## Their own words (website via Exa, livecrawl-preferred)

URL: https://inhance.com
Inhance

# IllustratingIntricateIdeas

Inhance is a Los Angeles-based interactive software agency specializing in immersive digital experiences. Our award-winning team delivers tailored solutions in content creation, AI/ML-based virtual training, activations, and XR (extended reality) technology to enhance storytelling and engagement across technically and scientifically complex industries.

---

## AR/VR/XR

## Modeling, Simulation, & Training

## Visualization

## Interactives

## EBCs and Innovation Centers

## Games

## AR/VR/XR

## Modeling, Simulation, & Training

## Visualization

## Interactives

## EBCs and Innovation Centers

## Games

At Inhance, we specialize in crafting innovative, interactive experiences that bring brands to life. Leveraging cutting-edge technologies like AI/ML, AR/VR, and immersive content, we transform ideas into powerful stories that captivate, engage and educate audiences. With a commitment to pushing creative and engineering boundaries, our tailored solutions redefine communications and training and forge deeper connections across industries. Let us help you tell your story in a way that inspires and leaves a lasting impact.

We use cookies to improve your experience and analyze traffic. You can opt out at any time.

Accept Decline

URL: https://inhance.com/about
Inhance

We use cookies to improve your experience and analyze traffic. You can opt out at any time.

Accept Decline

Inhance
We use cookies to improve your experience and analyze traffic. You can opt out at any time.
Accept Decline

URL: https://inhance.com/
Inhance

# IllustratingIntricateIdeas

Inhance is a Los Angeles-based interactive software agency specializing in immersive digital experiences. Our award-winning team delivers tailored solutions in content creation, AI/ML-based virtual training, activations, and XR (extended reality) technology to enhance storytelling and engagement across technically and scientifically complex industries.

---

## AR/VR/XR

## Modeling, Simulation, & Training

## Visualization

## Interactives

## EBCs and Innovation Centers

## Games

## AR/VR/XR

## Modeling, Simulation, & Training

## Visualization

## Interactives

## EBCs and Innovation Centers

## Games

At Inhance, we specialize in crafting innovative, interactive experiences that bring brands to life. Leveraging cutting-edge technologies like AI/ML, AR/VR, and immersive content, we transform ideas into powerful stories that captivate, engage and educate audiences. With a commitment to pushing creative and engineering boundaries, our tailored solutions redefine communications and training and forge deeper connections across industries. Let us help you tell your story in a way that inspires and leaves a lasting impact.

We use cookies to improve your experience and analyze traffic. You can opt out at any time.

Accept Decline

URL: https://inhance.com/portfolio
Inhance

## Interactivity, creativity, and passion. A curated selection of our work.

---

## Bean Riot™

## U.S. Navy 250th Anniversary Museum Experience

## Above and Beyond Experience

## Protected SATCOM

## F-35 XR Flight Simulator

## FM LPGA VR Golf Simulator

## CJADC2 - High Fight and Low Fight

## Honeywell Interactive Mural

## ST CES Immersive Theater Experience

## F135 2024

## Ultra 3D Interactive Platform

## Ultra ORION Product Launch

## Helix Executive Briefing Center - AI Rotunda

## V-280 Flight Simulator

## AH-6i Maintenance Task Trainer

## Myelofibrosis MOD

## Asthma + TSLP Virtual Experience

## L3Harris Global Innovation Center

## Otis Elevator Simulator

## Intel AI-Driven IoT

## Trumpf Smart Factory EBC

## Hologic EBC Lobby

## Illumina Skywalker Virtual Tour

## MasterCraft Virtual Showroom

## Quantum SI Protein Sequencer

## AFRL Aircraft MRO In Hololens

## Sundance Hero VR Experience

## Amgen MRD AR Game

## Minimal Residual Disease Discovery Game

## Pratt & Whitney PW127XT Engine Interactive

## World of Nitto AR/VR Experience

## Monster Power Recruiter Challenge

## Bean Riot™

## U.S. Navy 250th Anniversary Museum Experience

## Above and Beyond Experience

## Protected SATCOM

## F-35 XR Flight Simulator

## FM LPGA VR Golf Simulator

## CJADC2 - High Fight and Low Fight

## Honeywell Interactive Mural

## ST CES Immersive Theater Experience

## F135 2024

## Ultra 3D Interactive Platform

## Ultra ORION Product Launch

## Helix Executive Briefing Center - AI Rotunda

## V-280 Flight Simulator

## AH-6i Maintenance Task Trainer

## Myelofibrosis MOD

## Asthma + TSLP Virtual Experience

## L3Harris Global Innovation Center

## Otis Elevator Simulator

## Intel AI-Driven IoT

## Trumpf Smart Factory EBC

## Hologic EBC Lobby

## Illumina Skywalker Virtual Tour

## MasterCraft Virtual Showroom

## Quantum SI Protein Sequencer

## AFRL Aircraft MRO In Hololens

## Sundance Hero VR Experience

## Amgen MRD AR Game

## Minimal Residual Disease Discovery Game

## Pratt & Whitney PW127XT Engine Interactive

## World of Nitto AR/VR Experience

## Monster Power Recruiter Challenge

We use cookies to improve your experience and analyze traffic. You can opt out at any time.

Accept Decline

## LinkedIn about

Inhance is a cutting-edge creative agency celebrated for transforming complex ideas into captivating, immersive experiences. Our team of visionary strategists, innovative designers, skilled programmers, and experienced producers continuously redefine how audiences engage, creating award-winning content that sets new standards in interaction. Collaborating with some of the world\'s most influential companies, Inhance crafts multimedia solutions that don’t just capture attention—they foster deep engagement and drive meaningful customer learning. Across industries like aerospace, defense, healthc

## SBIR/STTR record (their own abstracts)

### Comprehensive Virtual Health/Combat Care Platform (VHP) — Air Force 2022 ($744,901)
Our proposed solution, Virtual Health Platform (VHP) will enable next-generation digital medical triage, specifically providing end-users the ability to transmit patient’s biomedical data, utilizing wearable biosensors to respective medical/emergency care facilities. The mission impact of this project on the DAF and DoD will be to develop a medical triage and patient tracking tool that bridges the gap from incident/emergency to medical/emergency care facilities. We believe technology development under this effort will contribute to this future mission need fulfillment. VHP will integrate commercial, off-the-shelf wearable biosensors and other IoT devices to measure and transmit patient’s biomedical data.  It visually integrates data into a common-operating picture to provide situational awareness of patients in medical emergencies to help EMTs and other emergency responders. This tool will enable digital triage and patient tracking in mass casualty events. VHP will enable healthcare provider-to-provider virtual consultations, allowing sharable biometrics, medical data, geolocation, and video feeds with emergency departments. VHP will help streamline medical emergency response as well as mass casualty and triage and healthcare provider communications during triage, treatment, and recovery.

## News (Exa, top 3)
- Inhance Digital Corporation () https://www.highergov.com/awardee/inhance-digital-corporation-55367635/
  Inhance Digital Corporation

HigherGov

Notify

- New Contract Awards
- New Grant Awards

Favorite

Export

- CSV
- Excel

Share

- Copy URL
- Email
- Tweet

Search Awardees Open Help Docs

# Inhance Digital Corporation

UEI: C172H549QPM7 &#x2022 CAGE: 43WE6

- List
- Text

## Overview

Awardee Type

Parent

Company Description

Inhance Digital: Leading Agency in Delivering AR/VR/XR and custom Interactive Brand Experiences that Engage and Inform Users

Name

Inhance Digital Corporation

Website

- Radiance Technologies Wins $151B SHIELD Contract for ... (2025-12-19) https://www.linkedin.com/posts/radiance-technologies_radiance-technologies-inc-is-pleased-to-activity-7407904706351493120-C1Ys
  Radiance Technologies Wins $151B SHIELD Contract for Missile Defense Agency | Radiance Technologies posted on the topic | LinkedIn

Agree & Join LinkedIn

By clicking Continue to join or sign in, you agree to LinkedIn’s User Agreement, Privacy Policy, and Cookie Policy.

# Radiance Technologies Wins $151B SHIELD Contract for Missile Defense Agency

https://www.linkedin.com/company/radiance-technologies

Radiance Technologies

20,172 followers

7mo

- Report this post

Radiance Technologies, Inc.
- inhance (2026-06-26) https://inhance.com/
  # Inhance

Inhance is a Media Production company. Inhance Digital is an interactive agency that provides content generation, visual effects, physical brand activation/immersive experiences, and virtual brand exhibits and events. They offer custom, cutting-edge interactive experiences for both in-person brand activations as well as virtual and online customer engagement. Inhance employs 17 people (-1.7% YoY, -1 person) and has an annual revenue of $17M, founded in 1997. Headquartered in Los Angel

## Open roles (Blitz, live)
- Sales and Business Development Director – Government Sector ({'city': 'Los Angeles', 'country_code': 'US'})
- Sales Development Representative - Meeting Booker ({'city': 'Los Angeles', 'country_code': 'US'})
- Business Development Director – Experiential & Immersive ({'city': 'Los Angeles', 'country_code': 'US'})
- Business Development Director – Experiential & Immersive ({'city': 'Los Angeles', 'country_code': 'US'})
- Sales & Business Development Director – Government Sector ({'city': 'Los Angeles', 'country_code': 'US'})
- Business Development Director – Experiential & Immersive ({'city': 'Los Angeles', 'country_code': 'US'})

---
# Context for the scoring agent (do not re-search)

## Barry Hess niche
Defense and intelligence technology companies in the $5M-$10M range whose proven prototype is stuck between demo and fielded deployment, guided by a CTO who fielded a patented tactical Android app platform with operators in theater and now engineers deployment architecture and acquisition advocacy from day one.

**EDP:** When months-to-field exceeds the funded months remaining on the contract, the program dies in the DoD valley of death - and for a $5M-$10M defense tech company, one dead transition is often the company.

## Deployed-reality checklist (score absence against this)
- **DDIL**: denied/disconnected/intermittent/low-bandwidth comms. Canonical example: an AI
  crowd-detection startup streamed 30fps video; the contested environment had bandwidth for
  1 frame every 5 seconds. One architecture change (frame-rate decoupling) saved the project.
- **SWaP-C**: size, weight, power, cost on the platform actually carried/mounted.
- **ATO/RMF**: Authority to Operate on government networks; STIGs, POA&Ms, IL4/IL5.
- **CMMC / NIST 800-171**: company-level compliance before the contract dies.
- **ITAR/export**: controlled data handling.
- **Operator training & sustainment**: who trains the E-5, who fixes it in the field, spares,
  battery logistics, GFE integration.
- **Advocacy chain**: a champion above the operator level; operators loving it is not adoption.
- **Prime/flow-down mechanics**: subcontract structure, government back-end integration.
- **TAK ecosystem**: if it touches situational awareness and never mentions ATAK/TAK, ask why.


## Doctrine
# Targeting doctrine brief (inject into every dossier & company agent — do NOT re-read the corpus)

Distilled from Jordan Crawford's Cannonball/Blueprint corpus, 2026-07-23.

1. **Pain-Qualified Segment beats ICP.** Never target firmographics ("50-200 person SaaS").
   Target companies in an observable painful SITUATION right now. The situation must be
   provable from public data.
2. **Existential Data Point (EDP).** For each niche, name the single number that kills the
   company if the problem stays unfixed (e.g. equipment utilization 60% vs 80% margins;
   fleet utilization <70% = crisis). Every dossier must state the EDP of its CTO's target
   situation: "what does it cost a company, in what metric, to leave this unfixed?"
3. **Jobs are confessions.** A job posting is a company publicly describing its own pain.
   We match jobs whose EXISTENCE signals a problem the CTO has already fixed — not jobs the
   CTO would apply to. "First Head of Engineering" = no technical leadership. "Integration
   Engineer (Epic, HL7)" = drowning in healthcare plumbing.
4. **The message is a redescription of the targeting.** If targeting is precise, the message
   writes itself by describing what we found. The reader should think "how do they know that?"
5. **Message-as-gift (GTM Shift letter standard).** Conversational, unhurried, 4–6 short
   sentences. Opens with a verified surprising fact. Every claim traces to a named public
   source. Valuable even if never answered. One light question, under 15 words. Bans: "I
   noticed", flattery, "hope you're well", meeting asks, personal details about the recipient
   as the opener. Lead with the CTO's expertise matched to the company's evident problem.
6. **Niche = Industry × Situation × Unique Angle** — all three specific (Ben Horowitz
   principle: not best botanist; best Japanese botanist specializing in Zen gardens).
7. **Insight tests**: counterintuitive (would other CTOs argue?), specific (exact tools/
   patterns), quantifiable (before/after numbers), demonstrable (showable in 5 minutes).
8. **Evidence honesty.** Actions > words > database records. Aggregator/API data (Blitz,
   FullEnrich) never corroborates itself — load-bearing claims need the person's or
   company's OWN public artifacts. Date every claim; label estimates as estimates.
9. **Dropping a bad fit is success.** Never pad a list to hit a count.
