# Evidence pack: Intalus
slug: intalus | domain: intalustech.com | HQ: West Lafayette, Indiana US | employees(LI): 12 | founded: 2019 | stage_band: unknown
sources: vc:scout
industry(LI): Manufacturing | specialties: refractory metals, transition metals, ceramics
channel notes: ceramic-infused metals for extreme environments
own-language word count: ~759

## Their own words (website via Exa, livecrawl-preferred)

URL: https://intalustech.com
Intalus

# Permanent performance.

INTALUS integrates ceramics into metal to create engineered material architectures for demanding applications across a wide range of industries.

INTALUS integrates ceramics into metal to create engineered material architectures for demanding applications across a wide range of industries.

## Beyond surface engineering, where failure is not an option.

INTALUS develops a patented ceramic infusion process that does not sit on the surface. It becomes part of the metal itself. No boundary layer. No delamination. No reapplication. Permanent performance, engineered within.

## From undersea to space.

OEMs and manufacturers trust INTALUS to perform in demanding environments.

### Shape the new era of materials

### Work with us

Whether you're solving for wear, corrosion, or thermal resistance — start here.

Name

Email

I'm reaching out as a...

Select one...CustomerInvestorTalentOther

Message (optional)

Thank you! Your submission has been received!

Oops! Something went wrong while submitting the form.

About

Careers

Contact

Connect

[email]

© 2025 Intalus. All rights reserved· Privacy Policy

URL: https://intalustech.com/about
About

### About INTALUS

INTALUS is an advanced materials and manufacturing technology company developing a new class of metal-ceramic components.Our proprietary Targeted Phase Infusion process integrates ceramic performance into metal components, creating an intrinsic metal-ceramic structure designed for demanding applications where durability must be built into the component itself.The result is a metallurgically integrated metal-ceramic architecture engineered for performance-critical applications without adding a conventional coating. INTALUS works with advanced metal components across aerospace, defense, space, energy, nuclear, industrial, maritime, mobility, medical, and select consumer product applications.

## Gradient ceramic infusion with spatial & compositional precision.

Our technology is applied after manufacturing and is designed to preserve part geometry while enhancing material performance.

#### Legacy coating

Just sits on top

#### INTALUS infusion

Controlled ceramic infusion and engineered functional zones.

## Proven Usecases

##### 01

#### Defense & Aerospace

Propulsion, flight-critical systems, structural hardware, and mission-critical components.

##### 02

#### Space Systems

Spaceflight, orbital, and re-entry-related components exposed to demanding operating environments.

##### 03

#### Industrial & Maritime

Pump components, shafts, fasteners, exposed structures, and hardware requiring resistance to wear, corrosion, fouling, and harsh operating conditions.

##### 04

#### Medical

Biocompatible metal-ceramic components for surgical, implant-related, and biomedical applications.

##### 05

#### Energy & Nuclear

Energy infrastructure, nuclear-adjacent systems, and high-reliability industrial components.

##### 06

#### Mobility

Automotive, motorsports, UAVs, robotics, and high-duty-cycle components where friction, wear, repeated motion, and environmental exposure are key considerations.

### Engineer performance that doesn’t wear out

Take materials beyond surface limits. Integrate durability, resistance, and reliability directly into your components.

Talk to Our Team

About

Careers

Contact

Connect

[email]

© 2025 Intalus. All rights reserved· Privacy Policy

URL: https://intalustech.com/
Intalus

# Permanent performance.

INTALUS integrates ceramics into metal to create engineered material architectures for demanding applications across a wide range of industries.

INTALUS integrates ceramics into metal to create engineered material architectures for demanding applications across a wide range of industries.

## Beyond surface engineering, where failure is not an option.

INTALUS develops a patented ceramic infusion process that does not sit on the surface. It becomes part of the metal itself. No boundary layer. No delamination. No reapplication. Permanent performance, engineered within.

## From undersea to space.

OEMs and manufacturers trust INTALUS to perform in demanding environments.

### Shape the new era of materials

### Work with us

Whether you're solving for wear, corrosion, or thermal resistance — start here.

Name

Email

I'm reaching out as a...

Select one...CustomerInvestorTalentOther

Message (optional)

Thank you! Your submission has been received!

Oops! Something went wrong while submitting the form.

About

Careers

Contact

Connect

[email]

© 2025 Intalus. All rights reserved· Privacy Policy

URL: https://intalustech.com/blog-news
Blog & News

### News & Blogs

The INTALUS News & Blog

News

Jun 25, 2026

### INTALUS moves into production with new facility at Purdue's HAMTC

>

INTALUS

New Purdue facility will scale production of Intalus’ s metal-ceramic mat erials for government and industrial use

Read

ALL

BLOGS

NEWS

UPDATES

PRESS

### INTALUS moves into production with new facility at Purdue's HAMTC

NewsJun 25, 2026>INTALUSNew Purdue facility will scale production of Intalus’ s metal-ceramic mat erials for government and industrial use

### INTALUS announces $11M Seed Raise

NewsOct 4, 2023>INTALUS

News

Jun 25, 2026

### INTALUS moves into production with new facility at Purdue's HAMTC

>

INTALUS

New Purdue facility will scale production of Intalus’ s metal-ceramic mat erials for government and industrial use

News

Oct 4, 2023

### INTALUS announces $11M Seed Raise

>

INTALUS

No items found.

No items found.

About

Careers

Contact

Connect

LinkedIn

[email]

© 2025 Intalus. All rights reserved· Privacy Policy

## LinkedIn about

Intalus harnesses integrative materials technology in the pursuit of the next generation of manufacturing. We do not follow the old playbook. We are industry disruptors, reshaping the landscape of material enhancement. Our proprietary methods transcend conventional coatings and alloys, providing unparalleled solutions for corrosion, erosion, wear, and thermal distortion. We amplify the true potential of a metal across diverse applications in defense, general industry, energy, space, and aerospace. Our pioneering processes and materials integration put us in a whole new category for manufacturi

## News (Exa, top 3)
- Exclusive: INTALUS Raises $20M for Metal-Ceramic Integration (2026-07-21) https://www.tectonicdefense.com/exclusive-intalus-raises-20m-for-metal-ceramic-integration/
  # Exclusive: INTALUS Raises $20M for Metal-Ceramic Integration

Author: Erin O'Brien
Published: 2026-07-21T13:47:48+00:00
Source: tectonicdefense.com (tectonicdefense.com)
Language: en

## Story

Exclusive: INTALUS Raises $20M for Metal-Ceramic Integration

Skip to content

[![TectonicDefense](https://www.tectonicdefense.com/wp-content/themes/tectonicdefens/assets/images/logo.png?v=1.0.3)](https://www.tectonicdefense.com/) [Subscribe](https://www.tectonicdefense.com/subscribe/) Menu

Search for:
- Origin Ventures is pleased to announce that INTALUS ... (2026-07-21) https://www.linkedin.com/posts/origin-ventures_origin-ventures-is-pleased-to-announce-that-activity-7485377021045469185-VrRI
  # Origin Ventures is pleased to announce that INTALUS has raised an additional $20M on the backs of our $11M Seed financing alongside Lockheed Martin and Scout Ventures earlier this year, bringing the … | Origin Ventures · LinkedIn · 2026-07-21

**Origin Ventures**: Origin Ventures is a company that invests in bold founders leading humanity into the fifth economic era - the artificial intelligence economy - transforming how we live, work, and play. Origin Ventures has 10-20 employees (+30% YoY) 
- Little-known materials startup Intalus raises $11M seed round | Semafor (2026-05-06) https://www.semafor.com/article/05/06/2026/little-known-materials-startup-intalus-raises-11m-seed-round
  Little-known materials startup Intalus raises $11M seed round | Semafor

Intelligence for the New World Economy

---

Semafor World Economy

---

From Semafor Technology

In your inbox, 2x per week

Sign up

# Exclusive / Little-known materials startup Intalus raises $11M seed round

Reed Albergotti

Tech Editor, Semafor

May 6, 2026, 1:01pm EDT

Share

Metal preparation area at an aluminum smelter. Benoit Tessier/Reuters.

Startup Intalus raised an $11 million seed round to expand production of

## Open roles
(no jobs data found)

---
# Context for the scoring agent (do not re-search)

## Barry Hess niche
Defense and intelligence technology companies in the $5M-$10M range whose proven prototype is stuck between demo and fielded deployment, guided by a CTO who fielded a patented tactical Android app platform with operators in theater and now engineers deployment architecture and acquisition advocacy from day one.

**EDP:** When months-to-field exceeds the funded months remaining on the contract, the program dies in the DoD valley of death - and for a $5M-$10M defense tech company, one dead transition is often the company.

## Deployed-reality checklist (score absence against this)
- **DDIL**: denied/disconnected/intermittent/low-bandwidth comms. Canonical example: an AI
  crowd-detection startup streamed 30fps video; the contested environment had bandwidth for
  1 frame every 5 seconds. One architecture change (frame-rate decoupling) saved the project.
- **SWaP-C**: size, weight, power, cost on the platform actually carried/mounted.
- **ATO/RMF**: Authority to Operate on government networks; STIGs, POA&Ms, IL4/IL5.
- **CMMC / NIST 800-171**: company-level compliance before the contract dies.
- **ITAR/export**: controlled data handling.
- **Operator training & sustainment**: who trains the E-5, who fixes it in the field, spares,
  battery logistics, GFE integration.
- **Advocacy chain**: a champion above the operator level; operators loving it is not adoption.
- **Prime/flow-down mechanics**: subcontract structure, government back-end integration.
- **TAK ecosystem**: if it touches situational awareness and never mentions ATAK/TAK, ask why.


## Doctrine
# Targeting doctrine brief (inject into every dossier & company agent — do NOT re-read the corpus)

Distilled from Jordan Crawford's Cannonball/Blueprint corpus, 2026-07-23.

1. **Pain-Qualified Segment beats ICP.** Never target firmographics ("50-200 person SaaS").
   Target companies in an observable painful SITUATION right now. The situation must be
   provable from public data.
2. **Existential Data Point (EDP).** For each niche, name the single number that kills the
   company if the problem stays unfixed (e.g. equipment utilization 60% vs 80% margins;
   fleet utilization <70% = crisis). Every dossier must state the EDP of its CTO's target
   situation: "what does it cost a company, in what metric, to leave this unfixed?"
3. **Jobs are confessions.** A job posting is a company publicly describing its own pain.
   We match jobs whose EXISTENCE signals a problem the CTO has already fixed — not jobs the
   CTO would apply to. "First Head of Engineering" = no technical leadership. "Integration
   Engineer (Epic, HL7)" = drowning in healthcare plumbing.
4. **The message is a redescription of the targeting.** If targeting is precise, the message
   writes itself by describing what we found. The reader should think "how do they know that?"
5. **Message-as-gift (GTM Shift letter standard).** Conversational, unhurried, 4–6 short
   sentences. Opens with a verified surprising fact. Every claim traces to a named public
   source. Valuable even if never answered. One light question, under 15 words. Bans: "I
   noticed", flattery, "hope you're well", meeting asks, personal details about the recipient
   as the opener. Lead with the CTO's expertise matched to the company's evident problem.
6. **Niche = Industry × Situation × Unique Angle** — all three specific (Ben Horowitz
   principle: not best botanist; best Japanese botanist specializing in Zen gardens).
7. **Insight tests**: counterintuitive (would other CTOs argue?), specific (exact tools/
   patterns), quantifiable (before/after numbers), demonstrable (showable in 5 minutes).
8. **Evidence honesty.** Actions > words > database records. Aggregator/API data (Blitz,
   FullEnrich) never corroborates itself — load-bearing claims need the person's or
   company's OWN public artifacts. Date every claim; label estimates as estimates.
9. **Dropping a bad fit is success.** Never pad a list to hit a count.
