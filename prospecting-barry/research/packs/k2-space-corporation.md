# Evidence pack: K2 SPACE CORPORATION
slug: k2-space-corporation | domain: k2space.com | HQ: Los Angeles, California US | employees(LI): 289 | founded: 2022 | stage_band: revenue_stage | LATE(151-300)
sources: sbir
industry(LI): Defense and Space Manufacturing | specialties: 
channel notes: SBIR PhII x2 [2023] Air Force latest: Precision Pointing for High Resolution Optical and RF Remote Sensing Satellites hq: San Francisco,CA emp: 10
own-language word count: ~1541

## Their own words (website via Exa, livecrawl-preferred)

URL: https://k2space.com
High-Power Satellite Platforms | K2 Space | Build Bigger

# Build Bigger

Build Bigger

## Scale changes what’s possible. Bigger satellites deliver more power to payloads, mass on orbit, and resilience at lower cost. Bigger isn’t excess, it’s access.

## The frontier runs on hardware

### Factory

80% of each satellite is built in-house. Vertical integration drives reliability, accelerates production, and enables continuous innovation

### Launch

By stacking 10 Mega Class satellites per launch vehicle, we maximize power to orbit (200kW / launch).

### Orbit

Engineered to reach every orbit and withstand even the harshest radiation environments.

### Heading

This is some text inside of a div block.

### Heading

This is some text inside of a div block.

### Heading

This is some text inside of a div block.

## K2 Space is building the biggest satellites on orbit

Payload Mass

200 KG

Power

2KW

Payload Mass

3000 KG

Power

20KW

01

Satellite Power

The Mega Class platform delivers 10x the power of other constellation class satellites (20kW vs. 2kW), unlocking industry-leading payload performance.

02

Resilient Design

Radiation-tolerant architectures, redundant avionics, and high-Δv propulsion means our satellites are built for resilience and longevity across every orbit.

03

Scale Ready

A design optimized for mass production combined with a fully vertically integrated hardware stack.

Learn more →

## Our satellites are built to thrive where others can’t

LEO

MEO

GEO

Temperature

545Fº

Deeper orbits are where

coverage and comms become

Distance from earth

564.545.000 km

Deeper orbits are where

coverage and comms become

Low Earth Orbit is well suited for missions that rely on proximity to Earth. With K2’s high-power platforms and advanced propulsion systems, LEO becomes a powerful domain for sensing and for applications where ultra-low latency still matters.

Medium Earth Orbit is the sweet spot. MEO provides global reach while maintaining strong network performance. K2’s MEO systems unlock efficient, resilient, and scalable constellations, without the complexity of operating hundreds of LEO spacecrafts.

Temperature

545Fº

Deeper orbits are where

coverage and comms become

Distance from earth

564.545.000 km

Deeper orbits are where

coverage and comms become

Geostationary orbit anchors the world’s most trusted broadcast and communications. Our GEO satellites deliver high power, long life, and continuous regional coverage, extending K2 capability all the way to 36,000 km.

Temperature

545Fº

Deeper orbits are where

coverage and comms become

Distance from earth

564.545.000 km

Deeper orbits are where

coverage and comms become

LEO

MEO

GEO

Temperature

545Fº

Deeper orbits are where

coverage and comms become

Distance from earth

564.545.000 km

Deeper orbits are where

coverage and comms become

Low Earth Orbit is well suited for missions that rely on proximity to Earth. With K2’s high-power platforms and advanced propulsion systems, LEO becomes a powerful domain for sensing and for applications where ultra-low latency still matters.

Medium Earth Orbit is the sweet spot. With dramatically fewer satellites, MEO provides true global reach while maintaining strong network performance. K2’s MEO systems unlock efficient, resilient, and scalable constellations, without the complexity of operating hundreds or thousands of LEO spacecraft

Temperature

545Fº

Deeper orbits are where

coverage and comms become

Distance from earth

564.545.000 km

Deeper orbits are where

coverage and comms become

Geostationary orbit anchors the world’s most trusted broadcast and communications . Our GEO satellites deliver high power, long life, and continuous regional coverage, extending K2 capability all the way to 36,000 km.

Temperature

545Fº

Deeper orbits are where

coverage and comms become

Distance from earth

564.545.000 km

Deeper orbits are where

coverage and comms become

Low Earth Orbit is well suited for missions that rely on proximity to Earth. With K2’s high-power platforms and advanced propulsion systems, LEO becomes a powerful domain for sensing and for applications where ultra-low latency still matters.

Temperature

545Fº

Deeper orbits are where

coverage and comms become

Distance from earth

564.545.000 km

Deeper orbits are where

coverage and comms become

Medium Earth Orbit is the sweet spot. MEO provides global reach while maintaining strong network performance. K2’s MEO systems unlock efficient, resilient, and scalable constellations, without the complexity of operating hundreds of LEO spacecrafts.

Temperature

545Fº

Deeper orbits are where

coverage and comms become

Distance from earth

564.545.000 km

Deeper orbits are where

coverage and comms become

Geostationary orbit anchors the world’s most trusted broadcast and communications. Our GEO satellites deliver high power, long life, and continuous regional coverage, extending K2 capability all the way to 36,000 km.

Temperature

545Fº

Deeper orbits are where

coverage and comms become

Distance from earth

564.545.000 km

Deeper orbits are where

coverage and comms become

"The industry went small. We asked: what happens if you go the other way?"

Neel Kunjur, Chief Technical Officer

Learn more →

## In the News

March 24, 2026

SES Partners with K2 Space to Build Cheaper Satellite Network

Read full article →

July 7, 2025

K2 Space announces plans for three-orbit demonstration mission

Read full article →

Dec 11, 2025

This Startup Is Building Huge Satellites For An Underused Interstellar Sweet Spot

Read full article →

September 16, 2025

SES and K2 Space to Accelerate Development of Next-Generation MEO Network

Read full article →

July 7, 2025

K2 Hotfire Paves a Path to Multi-Orbit Missions

Read full article →

February 13, 2025

Bloomberg Technology: K2 Space Raises $110M Series B

Read full article →

January 15, 2025

K2 Space raises $110 million to scale up satellite production

Read

URL: https://k2space.com/about
About K2 Space | Satellite Innovation Leaders | Building the Future of Orbital Infrastructure

# Where builders belong.

Small teams own the hardest problems. Every subsystem powers a mission that counts.

Small teams own the hardest problems. Every subsystem powers a mission that counts.

## We believe in ownership: if you design it, you see it fly. Pride comes from real hardware, not slide decks. Our teams move fast with autonomy, balancing pace with integrity.

Careers →

## Greenfield challenges await

Every subsystem contributes to bigger missions, different orbits, and new science. Every design challenge is a doorway to the future.

Today

Power

Propulsion

Autonomy

Thermal

Pointing

Deployment

Multi-Orbit Constellations

Deep Space Relay

Next Great Observatories

Nuclear Electric Propulsion

Deep Space Data Centers

Tomorrow

## Beyond

What Comes After the Frontier?

The work you do today unlocks tomorrow’s orbits. From lunar data hubs to Webb-scale science at a fraction of cost. We build the hardware that flies the future.

### Flagship Science and Exploration

Enabling frequent, complex missions to search for life and explore the outer solar system.

### Telescopes

Assembling giant telescopes and telescope arrays to characterize exoplanet surfaces in unprecedented detail.

### Megastructures for off-world settlement

Building large-scale space infrastructure to support human outposts across the solar system, laying the groundwork for a Type II civilization.

### Interstellar Probes

Creating interstellar-capable probes designed to cross light-year distances within a human lifetime.

### Flagship Science and Exploration

Enabling frequent, complex missions to search for life and explore the outer solar system.

### Telescopes

Assembling giant telescopes and telescope arrays to characterize exoplanet surfaces in unprecedented detail.

### Interstellar Probes

Creating interstellar-capable probes designed to cross light-year distances within a human lifetime.

### Megastructures for off-world settlement

Building large-scale space infrastructure to support human outposts across the solar system, laying the groundwork for a Type II civilization.

## The people behind the missions

Play video

### Karan Kunjur

CEO, Co-Founder

Helped lead AI startup from seed stage to 9-figure exit after a 10yr career at BCG

>

Play video

### Neel Kunjur

CTO, Co-Founder

Helped develop and integrate avionics systems for various Dragon spacecraft at SpaceX. 

URL: https://k2space.com/
High-Power Satellite Platforms | K2 Space | Build Bigger

# Build Bigger

Build Bigger

## Scale changes what’s possible. Bigger satellites deliver more power to payloads, mass on orbit, and resilience at lower cost. Bigger isn’t excess, it’s access.

## The frontier runs on hardware

### Factory

80% of each satellite is built in-house. Vertical integration drives reliability, accelerates production, and enables continuous innovation

### Launch

By stacking 10 Mega Class satellites per launch vehicle, we maximize power to orbit (200kW / launch).

### Orbit

Engineered to reach every orbit and withstand even the harshest radiation environments.

### Heading

This is some text inside of a div block.

### Heading

This is some text inside of a div block.

### Heading

This is some text inside of a div block.

## K2 Space is building the biggest satellites on orbit

Payload Mass

200 KG

Power

2KW

Payload Mass

3000 KG

Power

20KW

01

Satellite Power

The Mega Class platform delivers 10x the power of other constellation class satellites (20kW vs. 2kW), unlocking industry-leading payload performance.

02

Resilient Design

Radiation-tolerant architectures, redundant avionics, and high-Δv propulsion means our satellites are built for resilience and longevity across every orbit.

03

Scale Ready

A design optimized for mass production combined with a fully vertically integrated hardware stack

## LinkedIn about

Making previously impossible missions possible

## SBIR/STTR record (their own abstracts)

### Precision Pointing for High Resolution Optical and RF Remote Sensing Satellites — Air Force 2023 ($1,249,626)
K2 Space is developing the Giga Class satellite bus, an ultra-high-power spacecraft capable of delivering over 115kW of power and 15,000kg of payload mass to enable previously unattainable capabilities spanning ISR (Intelligence, Surveillance, and Reconna
### High-power Multi-function Orbital Logistics Hub for Inspection, Servicing, and Refueling — Air Force 2023 ($1,249,644)
K2 Space is developing the Giga Class satellite bus, an ultra high-power spacecraft capable of delivering up to 115kW of power and 15,000kg of payload mass to enable previously unattainable capabilities spanning ISR (Intelligence, Surveillance, and Reconn

## News (Exa, top 3)
- Award | SBIR () https://www.sbir.gov/awards/216128
  Award | SBIR Skip to main content

# Award

Portfolio Data

Back to Award Search

## K2 Space Mega Class Satellite

### Awardee

#### K2 SPACE CORPORATION

https://www.sbir.gov/portfolio/2233431 1355 Sepulveda BoulevardTORRANCE, CA, 90501USA

Award Year: 2025

UEI: ZXRFDXLXU575

HUBZone Owned: No

Woman Owned: No

Socially and Economically Disadvantaged: No

Congressional District: N/A

Tagged as:

SBIR

Phase II

### Awarding Agency

DOW

Branch: USAF

Total Award Amount: $18,000,000

Contract 
- CONTRACT to K2 SPACE CORPORATION () https://www.usaspending.gov/award/CONT_AWD_FA880925CB003_9700_-NONE-_-NONE-
  USAspending.gov
- K2 Space Awarded $60M STRATFI Contract for Groundbreaking ... (2024-12-19) https://www.prnewswire.com/news-releases/k2-space-awarded-60m-stratfi-contract-for-groundbreaking-proliferated-meo-mission-302335650.html
  K2 Space Awarded $60M STRATFI Contract for Groundbreaking Proliferated MEO Mission Accessibility Statement Skip Navigation

Breakthrough Multi-Orbit Platform to Launch in February 2026

TORRANCE, Calif., Dec. 19, 2024 /PRNewswire/ -- K2 Space has signed a contract with the U.S. Space Force to launch its first Mega Class satellite on a mission code-named "Gravitas." The contract, with a total value of $60 million, includes government funds, Small Business Innovation Research (SBIR) matching funds

## Open roles (Blitz, live)
- Senior Site Reliability Engineer ({'city': 'Los Angeles', 'country_code': 'US'})
- Senior Build Reliability Engineer ({'city': 'Los Angeles', 'country_code': 'US'})
- Thermal Analyst ({'city': 'Los Angeles', 'country_code': 'US'})
- Thermal Control System (TCS) Design Engineer ({'city': 'Los Angeles', 'country_code': 'US'})
- Senior Thermal Control System (TCS) Design Engineer ({'city': 'Los Angeles', 'country_code': 'US'})
- Senior Thermal Design Engineer ({'city': 'Los Angeles', 'country_code': 'US'})
- Principal Thermal Design Engineer ({'city': 'Los Angeles', 'country_code': 'US'})
- Senior Development Test Technician ({'city': 'Los Angeles', 'country_code': 'US'})
- Senior Application Architect ({'city': 'Los Angeles', 'country_code': 'US'})
- Senior Optomechanical Engineer ({'city': 'Los Angeles', 'country_code': 'US'})

---
# Context for the scoring agent (do not re-search)

## Barry Hess niche
Defense and intelligence technology companies in the $5M-$10M range whose proven prototype is stuck between demo and fielded deployment, guided by a CTO who fielded a patented tactical Android app platform with operators in theater and now engineers deployment architecture and acquisition advocacy from day one.

**EDP:** When months-to-field exceeds the funded months remaining on the contract, the program dies in the DoD valley of death - and for a $5M-$10M defense tech company, one dead transition is often the company.

## Deployed-reality checklist (score absence against this)
- **DDIL**: denied/disconnected/intermittent/low-bandwidth comms. Canonical example: an AI
  crowd-detection startup streamed 30fps video; the contested environment had bandwidth for
  1 frame every 5 seconds. One architecture change (frame-rate decoupling) saved the project.
- **SWaP-C**: size, weight, power, cost on the platform actually carried/mounted.
- **ATO/RMF**: Authority to Operate on government networks; STIGs, POA&Ms, IL4/IL5.
- **CMMC / NIST 800-171**: company-level compliance before the contract dies.
- **ITAR/export**: controlled data handling.
- **Operator training & sustainment**: who trains the E-5, who fixes it in the field, spares,
  battery logistics, GFE integration.
- **Advocacy chain**: a champion above the operator level; operators loving it is not adoption.
- **Prime/flow-down mechanics**: subcontract structure, government back-end integration.
- **TAK ecosystem**: if it touches situational awareness and never mentions ATAK/TAK, ask why.


## Doctrine
# Targeting doctrine brief (inject into every dossier & company agent — do NOT re-read the corpus)

Distilled from Jordan Crawford's Cannonball/Blueprint corpus, 2026-07-23.

1. **Pain-Qualified Segment beats ICP.** Never target firmographics ("50-200 person SaaS").
   Target companies in an observable painful SITUATION right now. The situation must be
   provable from public data.
2. **Existential Data Point (EDP).** For each niche, name the single number that kills the
   company if the problem stays unfixed (e.g. equipment utilization 60% vs 80% margins;
   fleet utilization <70% = crisis). Every dossier must state the EDP of its CTO's target
   situation: "what does it cost a company, in what metric, to leave this unfixed?"
3. **Jobs are confessions.** A job posting is a company publicly describing its own pain.
   We match jobs whose EXISTENCE signals a problem the CTO has already fixed — not jobs the
   CTO would apply to. "First Head of Engineering" = no technical leadership. "Integration
   Engineer (Epic, HL7)" = drowning in healthcare plumbing.
4. **The message is a redescription of the targeting.** If targeting is precise, the message
   writes itself by describing what we found. The reader should think "how do they know that?"
5. **Message-as-gift (GTM Shift letter standard).** Conversational, unhurried, 4–6 short
   sentences. Opens with a verified surprising fact. Every claim traces to a named public
   source. Valuable even if never answered. One light question, under 15 words. Bans: "I
   noticed", flattery, "hope you're well", meeting asks, personal details about the recipient
   as the opener. Lead with the CTO's expertise matched to the company's evident problem.
6. **Niche = Industry × Situation × Unique Angle** — all three specific (Ben Horowitz
   principle: not best botanist; best Japanese botanist specializing in Zen gardens).
7. **Insight tests**: counterintuitive (would other CTOs argue?), specific (exact tools/
   patterns), quantifiable (before/after numbers), demonstrable (showable in 5 minutes).
8. **Evidence honesty.** Actions > words > database records. Aggregator/API data (Blitz,
   FullEnrich) never corroborates itself — load-bearing claims need the person's or
   company's OWN public artifacts. Date every claim; label estimates as estimates.
9. **Dropping a bad fit is success.** Never pad a list to hit a count.
