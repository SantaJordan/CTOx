# Evidence pack: Kalogon, Inc
slug: kalogon-inc | domain: kalogon.com | HQ: Melbourne, Florida US | employees(LI): 35 | founded: 2019 | stage_band: revenue_stage
sources: sbir
industry(LI): Medical Equipment Manufacturing | specialties: seating solutions, health tech, seat cushions, wheelchair tech, smart wheelchair cushions, clinical validation, mobility tech, smart seating technology, wheelchair seating, pressure injury prevention, adaptive tech, dod seating solutions, med tech, dual use tech, seating, aviation seating, custom se
channel notes: SBIR PhII x1 [2022] Air Force latest: Kalogon Project Pluto (Smart Cushion) hq: Melbourne,FL emp: 12
own-language word count: ~1612

## Their own words (website via Exa, livecrawl-preferred)

URL: https://kalogon.com
Kalogon | The Future of Smart Seating - Wheelchairs Users, Pilots, Drivers, and Beyond

top of page

## Eliminating the stresses of sitting and improving health through novel seating solutions.

Kalogon is a trusted leader in seated health, driven by clinical research, deep customer empathy, and rapid product iterations to meet customer needs. We build advanced seating technologies for people who must sit — enhancing comfort, protecting health, and improving outcomes.

## Our Award Winning, Medicare-Coded Products

## Bondar E2617

## Orbiter Med E2609

## Booster K0108

## Verro E2609

Explore our Medical Products

## Machine Learning

## Patent-pending air cell technology

## Kalogon's Technology

## Our Advanced Pressure Management System (APMS) is a clinically validated technology that improves blood flow and increases seating tolerance.

Learn More

## Our Other Industries

## Kalogon is building advanced seating solutions for people who must sit — enhancing comfort, protecting health, and improving outcomes.

DoD

Interiors

## User Testimonials

Play Video

Play Video

01:09

### Meet David Kellam, Kalogon Customer

Play Video

Play Video

03:16

### A Conversation with John Miller, Kalogon Customer

Play Video

Play Video

00:50

### Meet Dalkin Gomez, Kalogon Customer

## Become A Part of Our Community

## Kalogon is always looking for ways to get involved with the community and organizations that align with our mission to eliminate the stresses of sitting and improve health through inventive seating solutions.

Learn More

## Interested in learning more about Kalogon?

Contact Us

bottom of page

URL: https://www.kalogon.com/medical
Medical Products | Kalogon

top of page

# Experience The Future of Smart Seating

# Revolutionized seating for extended sitting

Schedule An In-Service

## Cushions

## Orbiter Med – a Medicare-approved custom wheelchair cushion for pressure management, combines Kalogon’s Advanced Pressure Management System (APMS) with the individualized postural support of a custom cushion reimbursable by insurance.

Learn More

Accessories

## Orbiter – the world's first smart cushion that uses AI and patent-pending air cell technology to analyze real-time pressure data and intelligently adapt to meet your unique comfort needs.

Learn More

Accessories

## Verro – E2609 Medicare-coded cushion built for a versatile, intimate fit. Provides skin protection by maximizing patient contact area and spreading pressure evenly with customized load/offload areas. Features two material options: Kalogon's groundbreaking variable-density lattice or field modification ready dual-density foam.

Learn More

## Back Supports

## Bondar - E2617 Medicare-coded custom fabricated wheelchair back cushion designed to be an easy-to-install solution for people who need additional postural correction or support from their wheelchairs.

Learn More

Accessories

## Accessories

## Booster - a controller that solves maintenance issues with single-cell air floatation cushions.

Learn More

Orbiter Med’s APMS technology provides over 200 automated offload cycles per day*

*while in 2 minute redistribution mode

Clinical guidelines recommend offloading every

## 15 minutes

for 2 minutes each to avoid pressure injuries, but most users are able to far less frequently

## 91%

## of pressure injuries are considered preventable

At Kalogon, we focus on clinically-validated seating solutions, fortified by pending patents on core technologies, product efficacy, and a dedication to rapid iteration so our users get the best, first.

Research

Total U.S. healthcare costs per year for pressure injuries exceed

## $11B

## 2.5M

Americans will develop a pressure injury this year

## Interested in learning more about our products?

Schedule An In-Service

bottom of page

URL: https://www.kalogon.com/post/kalogon-launches-verro-a-new-medicare-coded-custom-wheelchair-cushion
Kalogon Launches Verro, a New Medicare-coded Custom Wheelchair Cushion Built for the Most Complex Support Needs

top of page

The new cushion line introduces a 3D-printed, variable-density lattice technology alongside a dual-density foam option, purpose-built for wheelchair users with significant postural needs

Melbourne, Fla. (July 20, 2026) – Kalogon, the market leader in smart seating solutions, today announced the launch of Verro– a new Medicare-coded custom wheelchair cushion line designed for people with significant postural support needs. Available in two models – Verro variable-density lattice (Verro Lattice) and Verro dual-density foam (Verro Foam) – Verro is built for wheelchair users whose bodies require a precise, body-conforming fit that standard cushions can’t deliver, including those with windswept posture, pelvic obliquity, or asymmetry.

At the core of Verro Lattice and new to the complex rehab tech market is Kalogon’s patent-pending variable-density lattice technology – a 3D-printed elastomer construction that varies firmness across the cushion surface: firmer where the body needs to be held in position, and softer where pressure needs to be diffused. For those who want more adaptability after fitting, Verro Foam is also available, with a dense foam base and soft foam topper (dual-density).

“We kept hearing from clinicians that their most complex patients were falling through the cracks, so we developed a technology that lets us 3D print the cushion with precise, localized control over where the body gets support and where it gets relief,” said Tim Balz, CEO and founder of Kalogon. “With our variable-density lattice, we can create subtle variations across the surface and sides of the cushion that simply aren’t possible with traditional materials. The customization options are hyper-personalized to provide the best comfort, support and fit for every customer.”

Verro's Two Cushion Constructions for Diverse Needs: 3D-Printed Variable-Density Lattice Technology and Dual-Density Foam

Seating specialists can order Verro in two construction types suited to different user needs and clinical approaches:

Verro Lattice is 3D-printed from a high-performance, lightweight elastomer material, in a proprietary variable-density lattice that varies firmness across the surface. The open lattice lets air move freely through the cushion, helping with temperature and moisture management and making the cushion up to two pounds lighter than the average cush

URL: https://www.kalogon.com/orbiter
Orbiter | Kalogon

top of page

# Feel Better, Sit Longer

# Kalogon's Orbiter cushion relieves pain, improves posture and increases blood flow to ensure you can live your best active, seated life

Learn More

Are you looking for Orbiter Med, our custom, Medicare-coded cushion?

## Higher Seating Tolerance

Machine learning and patent-pending air cell technology analyze pressure points in real time

System adapts to ambient conditions

## Improves Posture and Stability

Take control of your seating with personalized settings in the Kalogon app

Share data with your clinician to optimize for your health

## Wound Prevention

AI helps cushion to adapt and redistribute weight intelligently

Maintain blood flow with research based redistribution system

Automatic leak detection and alerts, with auto-adjustments to compensate for air loss

## Live a More Active, Seated Life

Custom to your needs

CRT and Manual wheelchair compatible

Battery life allows for a full day on the go

| Cushion Structure | Base Dimensions | Accessories | Coding |
| --- | --- | --- | --- |
| Top -X-Soft Non-FRG (1") | 14-24” | Additional covers | E2610 |
| Middle - Medium FRG 1” | Weight 5 to 7 lbs | Extender | |
| Bottom - X-Soft Non FRG 1” | | XLR Charger | |
| Air cell layer underneath foam | | Mounting Kit | |
| | | Mount Pouch | |

## Interested in learning more about our products?

Schedule An In-Service

Frequently Asked Questions

bottom of page

## LinkedIn about

Kalogon® is best known for pioneering smart seating solutions that reduce the stresses of sitting and improve seated health. Our patent-pending Advanced Pressure Management System (APMS) powers products with adaptive customizations, ambient adjustments, and personalized settings that enhance comfort and reduce fatigue for wheelchair users, pilots, and others who spend extended time seated. With four Medicare-coded products on the market, we prioritize user-centered design to ensure true market fit and lasting impact. By listening closely to the needs of wheelchair users, clinicians, and profes

## SBIR/STTR record (their own abstracts)

### Kalogon Project Pluto (Smart Cushion) — Air Force 2022 ($1,248,368)
The Air Force Global Strike Command has a national defense-related mission need in the areas of long-range strategic strike, aircrew performance, and mission readiness. Specifically, long-range strike missions require bomber and Intercontinental Ballistic Missile (ICBM) crews to be seated for extended durations, often exceeding 12 or more hours on both global bomber missions and during multi-day ICBM alert tours. In these conditions, pilots experience significant fatigue and physical pain that distract from their performance during critical phases of operation. This decrease in human performance factors negatively impacts mission effectiveness and leads to an increased risk of safety-related incidents. Kalogon contends that our innovative, Machine Learning Enabled Smart Seating technology will greatly reduce crew fatigue and thus increase combat effectiveness. The mission impact of implementing Kalogon’s Smart Seating technology on the DAF and DoD will be to greatly increase the effectiveness of aircraft and ICBM seating systems currently employed for long-duration missions by enabling the crew to remain comfortably seated for longer periods. This automated and dynamic redistribution of pressure zones encourages the flow of blood to areas that would otherwise not be offloaded. This will reduce long-term fatigue that crews experience, and it will also improve levels of comfort, enabling increased mission performance due to reduced physical pain.   The central problem that this project (internally dubbed “Project Pluto”) seeks to solve is that missions requiring extended time spent seated (12 - 24 hours) are painful for pilots, distracting from their performance, and decreasing mission readiness. We have developed a Smart Cushion that relieves pain and fatigue experienced by individuals who are seated for extended periods of time, allowing them to remain seated for longer. It accomplishes this by dynamically redistributing the user’s pressure, which causes improved blood flow and prevents the pain and fatigue that occur when blood flow is cut off while seated. We have already completed feasibility research, and in this project, we will adapt our existing off-the-shelf solution to the needs of the Air Force while continuing the development of the product for all other markets. This low-cost low-risk solution to the issues mentioned above will be done by making the following improvements: The incorporation of the external electronics enclosure into the seating surface of the cushion, so that the cushion is one fully integrated unit, as opposed to two pieces connected together. The adaption of the electronics and cushion materials to adhere to the relevant Air Force standards for use during non-critical stages of flight. The addition of the ability to power the cushion from ports available on aircraft. Investigation into the feasibility of incorporating our Smart Seating technology into an ejection seat.

## News (Exa, top 3)
- DoD (2018-12-11) https://www.kalogon.com/dod
  DoD | Kalogon

top of page

# Smart seating for mission-ready performance

Tim Balz and Amanda Krantz from Kalogon present a B-52H Stratofortress smart seat cushion prototype to Brig. Gen. Ty W. Neuman, Director of Strategic Plans, Programs and Requirements, Air Force Global Strike Command and other A5/8 leadership and members December 6, 2023, at Barksdale Air Force Base, Louisiana. Kalogon, a company that builds seat cushions for wheelchair patients, has been working with the A5/8 as they crea
- History (2018-12-11) https://www.kalogon.com/history
  History | Kalogon

top of page

# Learn How Kalogon Was Built

​​​3rd SBIR US Air Force contract ($1.6M) for E-4B cushion

Kicked off August 2024

$585K grant from National Institute on Disability, Independent Living, and Rehabilitation Research

Awarded July 2024

$1.2M Convertible Note

Closed May 2024

Medicare-coded cushion

Began shipping May 2024

Paid proof of concept for business class seat

Debuted at Aircraft Interiors Expo May 2024

Accessory sold by Kalogon and Etac, one of the world
- AFGSC partners with Kalogon to modernize innovative B-52H Stratofortress upgrades () https://www.afgsc.af.mil/News/Photos/igphoto/2003357487/
  AFGSC partners with Kalogon to modernize innovative B-52H Stratofortress upgrades

An official website of the United States government

Here's how you know

Official websites use .mil

A .mil website belongs to an official U.S. Department of Defense organization in the United States.

Secure .mil websites use HTTPS

A lock ( ) or https:// means you’ve safely connected to the .mil website. Share sensitive information only on official, secure websites.

Skip to main content (Press Enter).

# AFGSC

## Open roles (Blitz, live)
- Production Technician ({'city': 'Melbourne', 'country_code': 'US'})
- Mechanical Engineer ({'city': 'Kubolong', 'country_code': 'IN'})
- Manufacturing Engineer ({'city': 'Kubolong', 'country_code': 'IN'})
- Product Manager, Seating Solutions (Healthcare Hardware) ({'city': 'Kubolong', 'country_code': 'IN'})
- Mechanical Engineer ({'city': 'Emeryville', 'country_code': 'US'})
- Product Manager, Seating Solutions (Healthcare Hardware) ({'city': 'Emeryville', 'country_code': 'US'})
- Manufacturing Engineer ({'city': 'Emeryville', 'country_code': 'US'})
- Research Lead ({'city': 'Emeryville', 'country_code': 'US'})
- Research Lead ({'city': 'Kubolong', 'country_code': 'IN'})
- Product Manager, Seating Solutions (Healthcare Hardware) ({'city': 'Kubolong', 'country_code': 'IN'})

---
# Context for the scoring agent (do not re-search)

## Barry Hess niche
Defense and intelligence technology companies in the $5M-$10M range whose proven prototype is stuck between demo and fielded deployment, guided by a CTO who fielded a patented tactical Android app platform with operators in theater and now engineers deployment architecture and acquisition advocacy from day one.

**EDP:** When months-to-field exceeds the funded months remaining on the contract, the program dies in the DoD valley of death - and for a $5M-$10M defense tech company, one dead transition is often the company.

## Deployed-reality checklist (score absence against this)
- **DDIL**: denied/disconnected/intermittent/low-bandwidth comms. Canonical example: an AI
  crowd-detection startup streamed 30fps video; the contested environment had bandwidth for
  1 frame every 5 seconds. One architecture change (frame-rate decoupling) saved the project.
- **SWaP-C**: size, weight, power, cost on the platform actually carried/mounted.
- **ATO/RMF**: Authority to Operate on government networks; STIGs, POA&Ms, IL4/IL5.
- **CMMC / NIST 800-171**: company-level compliance before the contract dies.
- **ITAR/export**: controlled data handling.
- **Operator training & sustainment**: who trains the E-5, who fixes it in the field, spares,
  battery logistics, GFE integration.
- **Advocacy chain**: a champion above the operator level; operators loving it is not adoption.
- **Prime/flow-down mechanics**: subcontract structure, government back-end integration.
- **TAK ecosystem**: if it touches situational awareness and never mentions ATAK/TAK, ask why.


## Doctrine
# Targeting doctrine brief (inject into every dossier & company agent — do NOT re-read the corpus)

Distilled from Jordan Crawford's Cannonball/Blueprint corpus, 2026-07-23.

1. **Pain-Qualified Segment beats ICP.** Never target firmographics ("50-200 person SaaS").
   Target companies in an observable painful SITUATION right now. The situation must be
   provable from public data.
2. **Existential Data Point (EDP).** For each niche, name the single number that kills the
   company if the problem stays unfixed (e.g. equipment utilization 60% vs 80% margins;
   fleet utilization <70% = crisis). Every dossier must state the EDP of its CTO's target
   situation: "what does it cost a company, in what metric, to leave this unfixed?"
3. **Jobs are confessions.** A job posting is a company publicly describing its own pain.
   We match jobs whose EXISTENCE signals a problem the CTO has already fixed — not jobs the
   CTO would apply to. "First Head of Engineering" = no technical leadership. "Integration
   Engineer (Epic, HL7)" = drowning in healthcare plumbing.
4. **The message is a redescription of the targeting.** If targeting is precise, the message
   writes itself by describing what we found. The reader should think "how do they know that?"
5. **Message-as-gift (GTM Shift letter standard).** Conversational, unhurried, 4–6 short
   sentences. Opens with a verified surprising fact. Every claim traces to a named public
   source. Valuable even if never answered. One light question, under 15 words. Bans: "I
   noticed", flattery, "hope you're well", meeting asks, personal details about the recipient
   as the opener. Lead with the CTO's expertise matched to the company's evident problem.
6. **Niche = Industry × Situation × Unique Angle** — all three specific (Ben Horowitz
   principle: not best botanist; best Japanese botanist specializing in Zen gardens).
7. **Insight tests**: counterintuitive (would other CTOs argue?), specific (exact tools/
   patterns), quantifiable (before/after numbers), demonstrable (showable in 5 minutes).
8. **Evidence honesty.** Actions > words > database records. Aggregator/API data (Blitz,
   FullEnrich) never corroborates itself — load-bearing claims need the person's or
   company's OWN public artifacts. Date every claim; label estimates as estimates.
9. **Dropping a bad fit is success.** Never pad a list to hit a count.
