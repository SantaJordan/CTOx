# Evidence pack: Kirkwall
slug: kirkwall | domain: kirkwall.io | HQ: Sioux Falls, South Dakota US | employees(LI): 13 | founded: 2022 | stage_band: preseed_seed
sources: exa-agent:defense-cyber-more
industry(LI): Computer and Network Security | specialties: risk management, risk mitigation, iot monitoring, critical infrastructure, operational security
channel notes: Seed $565K total funding reported. dom: cyber status: fielded dod: Kirkwall markets cybersecurity work for defense and critical infrastructure, and has a DOE CESER cybersecurity subcontract. hq: Sioux Falls, SD emp: 8 src: https://kirkwall.io/
own-language word count: ~1170

## Their own words (website via Exa, livecrawl-preferred)

URL: https://kirkwall.io
Kirkwall | Operational security software

top of page

# Operational security software that thinks ahead.

Proactive protection for critical environments — because downtime isn't an option.

Learn More

Serving

Manufacturing

Heavy Industry

Energy & Fuels Sector

Municipal Water Facilities

Agricultural Co-ops

Datacenters

Biomedical Facilities

and more ...

Back to Top

## We safeguard the systems that keep critical industries running.

Give your team the clarity to protect what matters most, before costly failures occur.

Sensor-Agnostic Integration

Connect legacy or next-generation sensors — no hardware swaps required.

Proactive Alerts in 30 Seconds

Detected anomaly and issue alerts via SMS or email within 30 seconds of occurrence.

Real-Time Data

Aggregate live data from sensors, including infrared, electro-optical, acoustic, chemical, biological, magnetic, MEMS, and more.

Non-Disruptive Deployment

Integrates seamlessly without downtime or workflow interruption.

Natural Language Query

No need for technical speak — ask status questions in plain language and cut down response time.

Machine Learning Detection

Monitor data streams to catch early signs of equipment failure, human error, external attack, or environmental threat.

CASE STUDY

## Critical Cold Chain Intervention for a Multinational Biotech Firm

“Kirkwall prevented the loss of four years’ worth of archived cancer research samples, DNA, and reagents on day one. Our reagents alone were worth tens of thousands of dollars and we can’t even begin to put a price on those cancer research samples.”

— Laboratory & Quality Manager

Thanks to Kirkwall’s rapid alerting and visibility platform, a critical system error was identified and corrected. Approximately $1 million in time and resources, more than $50,000 worth of sensitive chemical reagents, and 4 years of cancer research protected.

DOWNLOAD CASE STUDY

Click here to download

## Kirkwall Overwatch

Our proprietary operational security platform, providing centralized visibility across operation-critical systems by aggregating sensor data into a unified operational dashboard.

Custom Configuration

1

Detects early signs of equipment degradation, external tampering, human error, environmental exposure, and maintenance gaps

2

Delivers warning notifications and actionable alerts via text message and secure channels within 30 seconds of detecting an issue.

Multiple Sensors

3

Enables rapid operator response before problems escalate, to mitigate downtime, production loss, and costly cascading failures across critical operations.

Expanded Insights

## Enabled by our natural language query chat feature.

Comprehensive Sensor Audit Logs

Have a question or ready to talk?

SUBMIT

Thanks for submitting!

We will be in contact soon.

bottom of page

URL: https://www.kirkwall.io/about
ABOUT | Kirkwall Defense

top of page

DOWNLOAD OUR SPEC SHEET

Click here to download

# About Us

Founded by security experts with a history of protecting national infrastructure

Kirkwall is an operational security software company dedicated to preserving the integrity of the systems that keep critical industries running. Built by former intelligence and national security professionals, Kirkwall helps manufacturers, energy cooperatives, agricultural co-ops, and critical infrastructure operators protect the assets and systems vital to their operations.

By unifying real-time sensor data, behavioral analytics, and environmental monitoring into a centralized platform, Kirkwall empowers teams to detect risks early — from equipment failures to environmental threats and human error — and respond proactively before small issues become costly disruptions. Delivering actionable insights, predictive alerts, and rapid notifications, Kirkwall ensures operators have the visibility and confidence to act before downtime, production loss, or asset failure impacts operations.

Agile & Scalable

Our SaaS infrastructure is designed to adapt and grow with your operations, ensuring long-term reliability.

Secure

Offering industry-leading cybersecurity measures to protect your assets and data.

Cutting Edge

Our always-expanding state-of-the-art technology keeps you ahead of potential threats.

Centralized

Streamlined, all-in-one solution for optimal visibility into your equipment performance.

Sensor Agnostic

Supporting legacy and emerging sensors within your operations.

## Homegrown

Based out of Midwest

## We are

Predictive & Proactive

Our proprietary machine learning frameworks anticipate, identify, and address operational anomalies and potential equipment defects, enabling timely repairs before failures occur.

## Our Team

William Cromarty

Founder & CEO

Based in Sioux Falls, South Dakota, Will is a former CIA officer and national security expert in space systems and counterproliferation, bringing decades of intelligence and strategic development experience to his role. He leads the company’s mission to safeguard critical infrastructure across agriculture, defense, and manufacturing.

Justin Morales

Lead Security Engineer

A cybersecurity and IT leader with a focus on government compliance and cyber resilience driving initiatives around CMMC and other regulatory standards. With hands-on expertise in securing infrastructure and leading cyber-focused projects, J

URL: https://www.kirkwall.io/
Kirkwall | Operational security software

top of page

# Operational security software that thinks ahead.

Proactive protection for critical environments — because downtime isn't an option.

Learn More

Serving

Manufacturing

Heavy Industry

Energy & Fuels Sector

Municipal Water Facilities

Agricultural Co-ops

Datacenters

Biomedical Facilities

and more ...

Back to Top

## We safeguard the systems that keep critical industries running.

Give your team the clarity to protect what matters most, before costly failures occur.

Sensor-Agnostic Integration

Connect legacy or next-generation sensors — no hardware swaps required.

Proactive Alerts in 30 Seconds

Detected anomaly and issue alerts via SMS or email within 30 seconds of occurrence.

Real-Time Data

Aggregate live data from sensors, including infrared, electro-optical, acoustic, chemical, biological, magnetic, MEMS, and more.

Non-Disruptive Deployment

Integrates seamlessly without downtime or workflow interruption.

Natural Language Query

No need for technical speak — ask status questions in plain language and cut down response time.

Machine Learning Detection

Monitor data streams to catch early signs of equipment failure, human error, external attack, or environmental threat.

CASE STUDY

## Critical Cold Chain Intervention for a Multinational Biotech Firm

“Kirkwall prevented the loss of four years’ worth of archived cancer research samples, DNA, and reagents on day one. Our reagents alone were worth tens of thousands of dollars and we can’t even begin to put a price on those cancer research samples.”

— Laboratory & Quality Manager

Thanks to Kirkwall’s rapid alerting and visibility platform, a critical system error was identified and corrected. Approximately $1 million in time and resources, more than $50,000 worth of sensitive chemical reagents, and 4 years of cancer research protected.

DOWNLOAD CASE STUDY

Click here to download

## Kirkwall Overwatch

Our proprietary operational security platform, providing centralized visibility across operation-critical systems by aggregating sensor data into a unified operational dashboard.

Custom Configuration

1

Detects early signs of equipment degradation, external tampering, human error, environmental exposure, and maintenance gaps

2

Delivers warning notifications and actionable alerts via text message and secure channels within 30 seconds of detecting an issue.

Multiple Sensors

3

Enables rapid operator response before problems escalate,

URL: https://www.kirkwall.io/resources
Resources | Kirkwall Defense

top of page

We're migrating our previous posts, check back soon!

Search

# Newsroom & Updates

## Reflections from SOFWeek 2025

May 16, 2025

## FARGO Inc! Profiles Our CTO Ujjwal Adhikari

Mar 5, 2025

## Sara Vosberg Joins Kirkwall as VP of Operations

Feb 2, 2025

## Kirkwall CEO profiled by FARGO INC!

Jan 30, 2025

## Governor’s Giant Vision winners announced | Kirkwall named winner

Aug 12, 2024

## A Reckoning Day for Securing America’s Critical Infrastructure

Jul 24, 2024

bottom of page

## LinkedIn about

Kirkwall is an operational security partner specializing in risk management to safeguard critical business operations from downtime. Leveraging cutting-edge technology, Kirkwall monitors critical IoT infrastructure and equipment for cybersecurity, environmental, and human factors to mitigate risk and protect operations from economic loss from downtime. Our proprietary software detects anomalies and notifies key operators in real-time to deploy corrective actions, ensuring our clients have peace of mind that their operations are always running optimally.

## News (Exa, top 3)
-  () https://www.find-tender.service.gov.uk/Notice/010917-2024/PDF
  Page 1 to 7
 
This is a published notice on the Find a Tender service: https://www.findtender.service.gov.uk/Notice/010917-2024
Contract
Replacement DVOR Electronics at Kirkwall Airport
Highlands and Islands Airports Limited
F06: Contract award notice – utilities
Notice identifier: 2024/S 000-010917
Procurement identifier (OCID): ocds-h6vhtk-037ff2 
Published 4 April 2024, 12:12pm
Section I: Contracting entity
I.1) Name and addresses
Highlands and Islands Airports Limited
Head Office, Inverness
- 40-bed Kirkwall Care Facility, Orkney - Find a Tender () https://www.find-tender.service.gov.uk/Notice/024877-2022
  40-bed Kirkwall Care Facility, Orkney - Find a Tender

Skip to main content

Help us improve Find a Tender Close

Sign up for user testing

BETA Contact the Find a Tender team (opens in new tab) if you have feedback, questions or suggestions.

Contract

# 40-bed Kirkwall Care Facility, Orkney

- Orkney Islands Council

F03: Contract award notice

Notice identifier: 2022/S 000-024877

Procurement identifier (OCID): ocds-h6vhtk-02b472

Published 6 September 2022, 8:55am

Watch this notice

---

--
- Orkney Harbours Masterplan Phase 1 - Scapa Deep Water Quay (SDWQ) - Find a Tender () https://www.find-tender.service.gov.uk/Notice/014391-2025
  Orkney Harbours Masterplan Phase 1 - Scapa Deep Water Quay (SDWQ) - Find a Tender

Skip to main content

Help us improve Find a Tender Close

Sign up for user testing

BETA Contact the Find a Tender team (opens in new tab) if you have feedback, questions or suggestions.

Contract

# Orkney Harbours Masterplan Phase 1 - Scapa Deep Water Quay (SDWQ)

- Orkney Islands Council (Harbour Authority)

F06: Contract award notice – utilities

Notice identifier: 2025/S 000-014391

Procurement identifier (O

## Open roles
(no jobs data found)

---
# Context for the scoring agent (do not re-search)

## Barry Hess niche
Defense and intelligence technology companies in the $5M-$10M range whose proven prototype is stuck between demo and fielded deployment, guided by a CTO who fielded a patented tactical Android app platform with operators in theater and now engineers deployment architecture and acquisition advocacy from day one.

**EDP:** When months-to-field exceeds the funded months remaining on the contract, the program dies in the DoD valley of death - and for a $5M-$10M defense tech company, one dead transition is often the company.

## Deployed-reality checklist (score absence against this)
- **DDIL**: denied/disconnected/intermittent/low-bandwidth comms. Canonical example: an AI
  crowd-detection startup streamed 30fps video; the contested environment had bandwidth for
  1 frame every 5 seconds. One architecture change (frame-rate decoupling) saved the project.
- **SWaP-C**: size, weight, power, cost on the platform actually carried/mounted.
- **ATO/RMF**: Authority to Operate on government networks; STIGs, POA&Ms, IL4/IL5.
- **CMMC / NIST 800-171**: company-level compliance before the contract dies.
- **ITAR/export**: controlled data handling.
- **Operator training & sustainment**: who trains the E-5, who fixes it in the field, spares,
  battery logistics, GFE integration.
- **Advocacy chain**: a champion above the operator level; operators loving it is not adoption.
- **Prime/flow-down mechanics**: subcontract structure, government back-end integration.
- **TAK ecosystem**: if it touches situational awareness and never mentions ATAK/TAK, ask why.


## Doctrine
# Targeting doctrine brief (inject into every dossier & company agent — do NOT re-read the corpus)

Distilled from Jordan Crawford's Cannonball/Blueprint corpus, 2026-07-23.

1. **Pain-Qualified Segment beats ICP.** Never target firmographics ("50-200 person SaaS").
   Target companies in an observable painful SITUATION right now. The situation must be
   provable from public data.
2. **Existential Data Point (EDP).** For each niche, name the single number that kills the
   company if the problem stays unfixed (e.g. equipment utilization 60% vs 80% margins;
   fleet utilization <70% = crisis). Every dossier must state the EDP of its CTO's target
   situation: "what does it cost a company, in what metric, to leave this unfixed?"
3. **Jobs are confessions.** A job posting is a company publicly describing its own pain.
   We match jobs whose EXISTENCE signals a problem the CTO has already fixed — not jobs the
   CTO would apply to. "First Head of Engineering" = no technical leadership. "Integration
   Engineer (Epic, HL7)" = drowning in healthcare plumbing.
4. **The message is a redescription of the targeting.** If targeting is precise, the message
   writes itself by describing what we found. The reader should think "how do they know that?"
5. **Message-as-gift (GTM Shift letter standard).** Conversational, unhurried, 4–6 short
   sentences. Opens with a verified surprising fact. Every claim traces to a named public
   source. Valuable even if never answered. One light question, under 15 words. Bans: "I
   noticed", flattery, "hope you're well", meeting asks, personal details about the recipient
   as the opener. Lead with the CTO's expertise matched to the company's evident problem.
6. **Niche = Industry × Situation × Unique Angle** — all three specific (Ben Horowitz
   principle: not best botanist; best Japanese botanist specializing in Zen gardens).
7. **Insight tests**: counterintuitive (would other CTOs argue?), specific (exact tools/
   patterns), quantifiable (before/after numbers), demonstrable (showable in 5 minutes).
8. **Evidence honesty.** Actions > words > database records. Aggregator/API data (Blitz,
   FullEnrich) never corroborates itself — load-bearing claims need the person's or
   company's OWN public artifacts. Date every claim; label estimates as estimates.
9. **Dropping a bad fit is success.** Never pad a list to hit a count.
