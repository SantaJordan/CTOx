# Evidence pack: Long Wall
slug: long-wall | domain: lwall.com | HQ: Long Beach, California US | employees(LI): 55 | founded: 2024 | stage_band: unknown
sources: vc:silent
industry(LI): Defense and Space Manufacturing | specialties: 
channel notes: defense technology
own-language word count: ~681

## Their own words (website via Exa, livecrawl-preferred)

URL: https://lwall.com
Long Wall

## Accelerate Missile Defense

# Mission

The world today faces grave threats. Enemies of freedom seek to violently disrupt the world order. Missiles fly towards civilians across multiple continents. Hypersonic strike weapons are disrupting the balance of power. Nuclear arsenals are growing and modernizing. Diplomacy is challenged. To preserve freedom and protect lives, a new approach to missile defense is needed.

Long Wall was founded to meet this challenge.

# Our Products

01.

##### Cyclops

Mass-producible exoatmospheric interceptor to defeat advanced threats.

02.

##### RSX

Low-cost liquid boosters for flight testing and threat replication.

03.

##### Ironwood

Modern GSE to support global launch and test with advanced C2, power, and comms.

# About Us

Building on seven years of R&D at ABL, Long Wall leverages proven technology, infrastructure, and operations to deliver breakthroughs for our customers. Ironwood systems have been deployed in four seasons in the most demanding environments.

RSX carries years of design iteration, subsystem testing, and flight ops. Missile defense systems are developed, built, and operated with proven systems for engineering, quality, licensing, and flight.

# Careers

We are growing our team of exceptional engineers, technicians, operators, and business people to accelerate towards our goals. If you have skills to contribute and are looking for a new mission, consider joining up. At Long Wall, your work will serve to protect people across the globe, and bend the arc of history away from conflict and towards peace.

URL: https://lwall.com/news
Long Wall Announces Cyclops: the Mass-Producible Interceptor | Long Wall

# News

Missions, programs, updates – here’s what we’ve been up to.

12.15.2025

Long Wal

#### LONG WALL ANNOUNCES CYCLOPS: THE MASS-PRODUCIBLE INTERCEPTOR

Today, Long Wall is announcing Cyclops: a new, surface-launched, exoatmospheric interceptor to defeat missiles in the midcourse phase of flight. Cyclops will provide high-performance, low-cost magazine depth for homeland and regional missile defense.

READ MORE

02.19.2025

Dan

#### Letter From the CEO, 2025

In recent years, the post-Cold War world order has cracked and broken. The Golden Arches theory of conflict prevention failed, leaving the U.S. and our allies exposed, with deep economic dependency on authoritarian rivals. We now face frequent, explicit threats – not just from rogue states and non-state actors, but from “near-peer” competitors. Despite motivated attempts at prevention, those threats have materialized into attacks, and free societies are falling under military domination.

READ MORE

URL: https://jobs.lever.co/longwall
Long Wall

Location type

Location

- All
- El Segundo, California
- Long Beach, California

Team

- All
- Corporate
- - Corporate
- Legal
- Engineering
- - Electronic Systems
- Engineering
- GNC & Trajectory
- Hardware Engineering
- Test & Operations
- Manufacturing
- - Manufacturing

Work type

Corporate

Corporate

Apply

##### Director, Sites and Facilities

On-site — Full-timeLong Beach, California

Legal

Apply

##### Sr. Corporate Counsel, Supply Chain

On-site — Full-timeLong Beach, California

Engineering

Electronic Systems

Apply

##### Principal Power Electronics Engineer

On-site — Full-timeLong Beach, California

Apply

##### Sr. Electronics Design Engineer

On-site — Full-timeLong Beach, California

Engineering

Apply

##### Director, Test and Operations

On-site — Full-timeLong Beach, California

Apply

##### Flight Analyst

On-site — Full-timeLong Beach, California

Apply

##### FPGA Design Engineer

On-site — Full-timeLong Beach, California

Apply

##### Head of GNC

On-site — Full-timeLong Beach, California

Apply

##### Head of Launch Systems

On-site — Full-timeLong Beach, California

GNC & Trajectory

Apply

##### Sr. GNC Engineer

On-site — Full-timeLong Beach, California

Hardware Engineering

Apply

##### Fluid Components Engineer

On-site — Full-timeLong Beach, California

Apply

##### Head of Vehicle Engineering

On-site — Full-timeLong Beach, California

Apply

##### Sr. Hardware Development Engineer

On-site — Full-timeLong Beach, California

Test & Operations

Apply

##### Lead Test Engineer

On-site — Long Beach, California

Apply

##### Test & Operations Engineer

On-site — Full-timeLong Beach, California

Manufacturing

Manufacturing

Apply

##### Assembly Technician

On-site — Full-timeEl Segundo, California

Apply

##### CNC Machinist

On-site — Full-timeEl Segundo, California

Apply

##### Sr. Avionics Development Technician

On-site — Full-timeEl Segundo, California

Apply

##### Sr. CNC Programmer and Machinist

On-site — Full-timeEl Segundo, California

Apply

##### Sr. Integration Technician

On-site — Full-timeEl Segundo, California

Long Wall Home Page

Jobs powered by

## LinkedIn about

Long Wall Company was created in 2024 to accelerate missile defense technology for the U.S. and our allies. Long Wall develops containerized, mass-producible missile defense systems and offers rapid, low-cost flight testing. We believe that the future of global security and stability require defensive technology that is lower cost, higher performing, better tested, and more broadly distributed than is possible today.

## News (Exa, top 3)
- Long Wall Unveils Surface-Launched Missile Following Company Revamp | Aviation Week (2025-12-15) https://aviationweek.com/defense/missile-defense-weapons/long-wall-unveils-surface-launched-missile-following-company-revamp
  Long Wall Unveils Surface-Launched Missile Following Company Revamp | Aviation Week

## Share

- Share on Email
- Share on Linkedin
- Share on Twitter
- Share on Facebook

# Long Wall Unveils Surface-Launched Missile Following Company Revamp

## Share

- Share on Email
- Share on Linkedin
- Share on Twitter
- Share on Facebook

Vivienne Machi December 15, 2025

The company wants to build interceptors for midcourse missile defense at a fraction of existing systems' price points.

Credit: Long Wal
- ABL Space renamed Long Wall as it shifts focus to defense market (2025-02-21) https://spacenews.com/abl-space-renamed-long-wall-as-it-shifts-focus-to-defense-market/
  ABL Space renamed Long Wall as it shifts focus to defense market - SpaceNews

Home/ ABL Space renamed Long Wall as it shifts focus to defense market

Credit: Long Wall

WASHINGTON — ABL Space, once an emerging contender in the commercial launch industry, is rebranding as Long Wall as it pivots away from launching small satellites to focus on defense applications, including missile defense and hypersonic flight testing.

The company, founded in 2017 in El Segundo, California, initially sought to 
- LONG WALL ANNOUNCES CYCLOPS: THE MASS-PRODUCIBLE INTERCEPTOR | Long Wall () https://www.lwall.com/news/announcing-cyclops-the-mass-producible-interceptor
  LONG WALL ANNOUNCES CYCLOPS: THE MASS-PRODUCIBLE INTERCEPTOR | Long Wall

## LONG WALL ANNOUNCES CYCLOPS: THE MASS-PRODUCIBLE INTERCEPTOR

12.15.2025

Long Wall

Today, Long Wall is announcing Cyclops: a new, surface-launched, exoatmospheric interceptor to defeat missiles in the midcourse phase of flight. Cyclops will provide high-performance, low-cost magazine depth for homeland and regional missile defense.

###### A MISSION TO DETER AND DEFEAT

As Long Wall’s flagship interceptor, Cyclops und

## Open roles (Blitz, live)
- FPGA Design Engineer ({'city': 'Long Beach', 'country_code': 'US'})
- Manager, AP ({'city': 'Long Beach', 'country_code': 'US'})
- Sr. Hardware Development Engineer ({'city': 'Long Beach', 'country_code': 'US'})
- Head of Hardware Development ({'city': 'Long Beach', 'country_code': 'US'})
- Head of Vehicle Engineering ({'city': 'Long Beach', 'country_code': 'US'})
- Director, Sites and Facilities ({'city': 'Long Beach', 'country_code': 'US'})
- Head of Launch Systems ({'city': 'Long Beach', 'country_code': 'US'})
- Director, Test and Operations ({'city': 'Long Beach', 'country_code': 'US'})
- Lead Test Engineer ({'city': 'Long Beach', 'country_code': 'US'})
- Manager, Corporate Accounting ({'city': 'Long Beach', 'country_code': 'US'})

---
# Context for the scoring agent (do not re-search)

## Barry Hess niche
Defense and intelligence technology companies in the $5M-$10M range whose proven prototype is stuck between demo and fielded deployment, guided by a CTO who fielded a patented tactical Android app platform with operators in theater and now engineers deployment architecture and acquisition advocacy from day one.

**EDP:** When months-to-field exceeds the funded months remaining on the contract, the program dies in the DoD valley of death - and for a $5M-$10M defense tech company, one dead transition is often the company.

## Deployed-reality checklist (score absence against this)
- **DDIL**: denied/disconnected/intermittent/low-bandwidth comms. Canonical example: an AI
  crowd-detection startup streamed 30fps video; the contested environment had bandwidth for
  1 frame every 5 seconds. One architecture change (frame-rate decoupling) saved the project.
- **SWaP-C**: size, weight, power, cost on the platform actually carried/mounted.
- **ATO/RMF**: Authority to Operate on government networks; STIGs, POA&Ms, IL4/IL5.
- **CMMC / NIST 800-171**: company-level compliance before the contract dies.
- **ITAR/export**: controlled data handling.
- **Operator training & sustainment**: who trains the E-5, who fixes it in the field, spares,
  battery logistics, GFE integration.
- **Advocacy chain**: a champion above the operator level; operators loving it is not adoption.
- **Prime/flow-down mechanics**: subcontract structure, government back-end integration.
- **TAK ecosystem**: if it touches situational awareness and never mentions ATAK/TAK, ask why.


## Doctrine
# Targeting doctrine brief (inject into every dossier & company agent — do NOT re-read the corpus)

Distilled from Jordan Crawford's Cannonball/Blueprint corpus, 2026-07-23.

1. **Pain-Qualified Segment beats ICP.** Never target firmographics ("50-200 person SaaS").
   Target companies in an observable painful SITUATION right now. The situation must be
   provable from public data.
2. **Existential Data Point (EDP).** For each niche, name the single number that kills the
   company if the problem stays unfixed (e.g. equipment utilization 60% vs 80% margins;
   fleet utilization <70% = crisis). Every dossier must state the EDP of its CTO's target
   situation: "what does it cost a company, in what metric, to leave this unfixed?"
3. **Jobs are confessions.** A job posting is a company publicly describing its own pain.
   We match jobs whose EXISTENCE signals a problem the CTO has already fixed — not jobs the
   CTO would apply to. "First Head of Engineering" = no technical leadership. "Integration
   Engineer (Epic, HL7)" = drowning in healthcare plumbing.
4. **The message is a redescription of the targeting.** If targeting is precise, the message
   writes itself by describing what we found. The reader should think "how do they know that?"
5. **Message-as-gift (GTM Shift letter standard).** Conversational, unhurried, 4–6 short
   sentences. Opens with a verified surprising fact. Every claim traces to a named public
   source. Valuable even if never answered. One light question, under 15 words. Bans: "I
   noticed", flattery, "hope you're well", meeting asks, personal details about the recipient
   as the opener. Lead with the CTO's expertise matched to the company's evident problem.
6. **Niche = Industry × Situation × Unique Angle** — all three specific (Ben Horowitz
   principle: not best botanist; best Japanese botanist specializing in Zen gardens).
7. **Insight tests**: counterintuitive (would other CTOs argue?), specific (exact tools/
   patterns), quantifiable (before/after numbers), demonstrable (showable in 5 minutes).
8. **Evidence honesty.** Actions > words > database records. Aggregator/API data (Blitz,
   FullEnrich) never corroborates itself — load-bearing claims need the person's or
   company's OWN public artifacts. Date every claim; label estimates as estimates.
9. **Dropping a bad fit is success.** Never pad a list to hit a count.
