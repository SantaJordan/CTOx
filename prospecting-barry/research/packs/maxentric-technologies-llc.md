# Evidence pack: MaXentric Technologies LLC
slug: maxentric-technologies-llc | domain: maxentric.com | HQ: Fort Lee, New Jersey US | employees(LI): 48 | founded: 2003 | stage_band: unknown
sources: exa-agent:sensors-data-collection-more
industry(LI): Wireless Services | specialties: v-b wlan, advanced dsp, biomedical technology, dpd, software defined radios, high efficiency pa systems, wireless networking, radars, biosensors, wireless system, digital signal processing platforms, envelope tracking
channel notes: SBIR Army SBIR Phase I/II support. dom: sensors status: prototype dod: MaXentric is developing a stretchable electronic patch to monitor soldiers' heart rate and respiration. hq: Fort Lee, New Jersey emp: 40-50 src: https://www.sbir.gov/awards/209580
own-language word count: ~884

## Their own words (website via Exa, livecrawl-preferred)

URL: https://maxentric.com
Products See all current Product lines Explore category Explore category

LabX All LabX technologies.. Explore category Explore category

Advanced R&D See what we are working on next Explore category Explore category

All Technologies See all MaXentric Technologies See all See all

# About Us

At MaXentric Technologies, we develop high-performance electronics for defense, medical, and commercial industries. Our teams focus on problem-solving and innovation to deliver technologies that create a safer, smarter, and more connected world.

Read our Story

## Featured Products

Browse all products

MaX-ADAPTR

RCSUS

MaXSDR-SoC

MAESTRO

DepLife

# Latest News and Publications

Browse all articles

DepLife at NTOA 2025 in Washington DC Another successful year at NTOA! Oct 20, 2025

New Paper Published by Dr. Paul Theilmann by IEEE This new paper presents research that surrounds our VitalWave technology development to create no-contact patient vital sign monitoring Jan 24, 2024

---

## Navigation

- About
- Products
- LabX
- Advanced R&D
- Careers
- News
- Contact
- Products
- [[phone]](tel:(201) 242 - 9800)
- [[email]](mailto: [email])

URL: https://maxentric.com/store/products
# Products

All

[Products]

LabX

Advanced R&D

STRETCH Compact Wearable Health Monitor for Tactical Triage Product

MaX-ADAPTR Detect, capture, locate, and track RF signals. Anywhere. Product

DepLife See-Through Wall Radar for First Responders Product

Maple Seeds Stand-In Sensing System Product

MaXSDR-SoC Software-Defined Radio System and Mobile Processing Unit for Military Radios Product

RCSUS NSA Type 1 Encrypted Software Defined Radio for Unmanned Aerial Systems Product

---

## Navigation

- About
- Products
- LabX
- Advanced R&D
- Careers
- News
- Contact
- Products
- [[phone]](tel:(201) 242 - 9800)
- [[email]](mailto: [email])

URL: https://maxentric.com/about
# Our Mission Statement

At MaXentric Technologies, we develop high-performance electronics for defense, medical, and commercial industries. Our teams focus on problem-solving and innovation to deliver technologies that create a safer, smarter, and more connected world.

# Core Values

### Collaboration is key

With a diverse group of team members, MaXentric encourages creativity and embraces team work to build a strong community and identity.

### Ideas into reality

Our team members have the space to explore their research interest and pursue projects that are meaningful to them.

### Engineering Excellence

MaXentric looks for team members who love what they do and help them develop their passions into innovative solutions.

# Who We Are

MaXentric Technologies researches, develops, and manufactures next generation electronic solutions for defense, medical, and commercial industries. Founded in 2003 as a research and development company, MaXentric has spent 20 years developing innovative products and services through a research-focused collaborative lab environment. MaXentric’s talented engineering, sales, and support teams ensure the highest quality standards in software and hardware throughout every phase of production across all three of our US locations.

Meet Our Leadership

# Locations

##### Fort Lee

##### New Jersey

2071 Lemoine Ave 302
Fort Lee, NJ 07024

##### San Diego

##### California

7590 Fay Ave, Suite 301
La Jolla, CA 92037

##### Boulder

##### Colorado

5055 Chaparral Court, Suite 101
Boulder, CO 80301

## We create high performance solutions for niche markets.

Please click below to see the products we are proud to have created.

Browse Products

# Our Management Team

### Kamran Mahbobi

##### Co-Founder & Managing Director

##### Fort Lee

Read Bio

### Houman Ghajari

##### Director of Government Systems & Technology

##### San Diego

Read Bio

# Leadership

### Paul Theilmann

##### VP, Low Power Wireless Systems

Read Bio

### Johana Yan

##### VP, Advanced RF Technology

Read Bio

### Per Johansson

##### Vice President of Engineering

Read Bio

### Brian Woods

##### Principal Engineer, Radar & Comms

Read Bio

### Brandon Beresini

##### Chief Information Officer

Read Bio

### Toshifumi Nakatani

##### Principal Engineer, Transceivers & HW Systems

Read Bio

---

## Navigation

- About
- Products
- LabX
- Advanced R&D
- Careers
- News
- Contact
- Products
- [[phone]](tel:(201) 242 - 9800)
- [[email]](mail

URL: https://maxentric.com/store/all
# All

[All]

Products

LabX

Advanced R&D

STRETCH Compact Wearable Health Monitor for Tactical Triage Product

MaX-ADAPTR Detect, capture, locate, and track RF signals. Anywhere. Product

BentoBox Lite Power Amplifier System and Waveform Generator LabX

DepLife See-Through Wall Radar for First Responders Product

DARWIN LPI/D High Speed Narrow Beam Data Connection LabX

GBPS Modem Gigabit-Per-Second Modem Software Family Advanced R&D

IBIS Ingestible Self-Powered Biosensor Advanced R&D

MAESTRO High Power High Efficiency Microwave Power Amplifier Advanced R&D

Load More

---

## Navigation

- About
- Products
- LabX
- Advanced R&D
- Careers
- News
- Contact
- Products
- [[phone]](tel:(201) 242 - 9800)
- [[email]](mailto: [email])

## LinkedIn about

MaXentric Technologies, LLC, founded in 2003, is an advanced research, development, and consulting company. Along the way, we have built a portfolio of technologies that service both commercial and government markets. Today, we are working on several components of the next generation of mobile platforms. We believe that intelligent engineering through the development of innovative products and services can significantly enhance and create value for our clients. Our technology attracts 4G cellular base-station integrators, NASA, the Army, the Navy/Marines and DARPA. Our team spans multiple disc

## SBIR/STTR record (their own abstracts)

### Enhanced Multi Band Reconfigurable Communications for Small Unmanned Systems Secure Radio Terminal (EMB-RCSUS SRT) — Army 2023 ($1,749,866)
Redacted
### Physiological Status Monitoring Nanorobotics — Army 2022 ($1,099,976)
To meet the demands of the A20B-T021 SBIR solicitation (“Physiological Status Monitoring Nanorobotics”), we propose an ingestible sensor which utilizes a novel self-powered sensing approach. The sensor is both powered by and senses metabolites in the digestive system.  Physiological data is streamed to a nearby end user device for data capture and visualization. Our approach leverages scaled CMOS technology to meet the power, size, and cost requirements for an ingestible, disposable sensor. During Phase I, we performed a study involving analysis, modeling, and simulation to design, fabricate, and test a preliminary prototype of the proposed sensor. During Phase II, we aim to demonstrate the sensor’s capabilities through animal model in-vivo testing in addition to iterating upon the prototype, focusing on sensor miniaturization, enhancing sensing capabilities, and system level platform development.

## News (Exa, top 3)
- Award | SBIR () https://www.sbir.gov/awards/209580
  Award | SBIR Skip to main content

# Award

Portfolio Data

Back to Award Search

## STretchable Robust Electronic Tracker and Continuous Health monitoring system (STRETCH)

### Awardee

#### MAXENTRIC TECHNOLOGIES LLC

2071 LEMOINE AVE STE 302FORT LEE, NJ, 07024-6007USA

Award Year: 2024

UEI: JL9DW4GMNJ53

HUBZone Owned: No

Woman Owned: No

Socially and Economically Disadvantaged: No

Congressional District: 5

Tagged as:

SBIR

Phase II

### Awarding Agency

DOD

Branch: ARMY

Total Award Am
- Award | SBIR () https://www.sbir.gov/awards/209407
  Award | SBIR Skip to main content

# Award

Portfolio Data

Back to Award Search

## Ultra-Low SWaP Reprogrammable Radio and Compute SoC for Autonomous Systems

### Awardee

#### MAXENTRIC TECHNOLOGIES LLC

https://www.sbir.gov/portfolio/226666 2071 LEMOINE AVE STE 302FORT LEE, NJ, 07024-6007USA

Award Year: 2024

UEI: JL9DW4GMNJ53

HUBZone Owned: No

Woman Owned: No

Socially and Economically Disadvantaged: No

Congressional District: 5

Tagged as:

SBIR

Phase I

### Awarding Agency

DOW

Bran
- Award | SBIR () https://www.sbir.gov/awards/207330
  Award | SBIR Skip to main content

# Award

Portfolio Data

Back to Award Search

## HERMES

### Awardee

#### MAXENTRIC TECHNOLOGIES LLC

2071 LEMOINE AVE STE 302FORT LEE, NJ, 07024-6007USA

Award Year: 2023

UEI: JL9DW4GMNJ53

HUBZone Owned: No

Woman Owned: No

Socially and Economically Disadvantaged: No

Congressional District: 5

Tagged as:

SBIR

Phase II

### Awarding Agency

DOD

Branch: NAVY

Total Award Amount: $1,749,675

Contract Number: N68335-23-C-0676

Agency Tracking Number: F2D-

## Open roles (Blitz, live)
- Sr. Embedded Software Engineer ({'city': 'San Diego', 'country_code': 'US'})
- Senior Software Engineer ({'city': 'San Diego', 'country_code': 'US'})
- Sr. Embedded Software Engineer ({'city': 'San Diego', 'country_code': 'US'})

---
# Context for the scoring agent (do not re-search)

## Barry Hess niche
Defense and intelligence technology companies in the $5M-$10M range whose proven prototype is stuck between demo and fielded deployment, guided by a CTO who fielded a patented tactical Android app platform with operators in theater and now engineers deployment architecture and acquisition advocacy from day one.

**EDP:** When months-to-field exceeds the funded months remaining on the contract, the program dies in the DoD valley of death - and for a $5M-$10M defense tech company, one dead transition is often the company.

## Deployed-reality checklist (score absence against this)
- **DDIL**: denied/disconnected/intermittent/low-bandwidth comms. Canonical example: an AI
  crowd-detection startup streamed 30fps video; the contested environment had bandwidth for
  1 frame every 5 seconds. One architecture change (frame-rate decoupling) saved the project.
- **SWaP-C**: size, weight, power, cost on the platform actually carried/mounted.
- **ATO/RMF**: Authority to Operate on government networks; STIGs, POA&Ms, IL4/IL5.
- **CMMC / NIST 800-171**: company-level compliance before the contract dies.
- **ITAR/export**: controlled data handling.
- **Operator training & sustainment**: who trains the E-5, who fixes it in the field, spares,
  battery logistics, GFE integration.
- **Advocacy chain**: a champion above the operator level; operators loving it is not adoption.
- **Prime/flow-down mechanics**: subcontract structure, government back-end integration.
- **TAK ecosystem**: if it touches situational awareness and never mentions ATAK/TAK, ask why.


## Doctrine
# Targeting doctrine brief (inject into every dossier & company agent — do NOT re-read the corpus)

Distilled from Jordan Crawford's Cannonball/Blueprint corpus, 2026-07-23.

1. **Pain-Qualified Segment beats ICP.** Never target firmographics ("50-200 person SaaS").
   Target companies in an observable painful SITUATION right now. The situation must be
   provable from public data.
2. **Existential Data Point (EDP).** For each niche, name the single number that kills the
   company if the problem stays unfixed (e.g. equipment utilization 60% vs 80% margins;
   fleet utilization <70% = crisis). Every dossier must state the EDP of its CTO's target
   situation: "what does it cost a company, in what metric, to leave this unfixed?"
3. **Jobs are confessions.** A job posting is a company publicly describing its own pain.
   We match jobs whose EXISTENCE signals a problem the CTO has already fixed — not jobs the
   CTO would apply to. "First Head of Engineering" = no technical leadership. "Integration
   Engineer (Epic, HL7)" = drowning in healthcare plumbing.
4. **The message is a redescription of the targeting.** If targeting is precise, the message
   writes itself by describing what we found. The reader should think "how do they know that?"
5. **Message-as-gift (GTM Shift letter standard).** Conversational, unhurried, 4–6 short
   sentences. Opens with a verified surprising fact. Every claim traces to a named public
   source. Valuable even if never answered. One light question, under 15 words. Bans: "I
   noticed", flattery, "hope you're well", meeting asks, personal details about the recipient
   as the opener. Lead with the CTO's expertise matched to the company's evident problem.
6. **Niche = Industry × Situation × Unique Angle** — all three specific (Ben Horowitz
   principle: not best botanist; best Japanese botanist specializing in Zen gardens).
7. **Insight tests**: counterintuitive (would other CTOs argue?), specific (exact tools/
   patterns), quantifiable (before/after numbers), demonstrable (showable in 5 minutes).
8. **Evidence honesty.** Actions > words > database records. Aggregator/API data (Blitz,
   FullEnrich) never corroborates itself — load-bearing claims need the person's or
   company's OWN public artifacts. Date every claim; label estimates as estimates.
9. **Dropping a bad fit is success.** Never pad a list to hit a count.
