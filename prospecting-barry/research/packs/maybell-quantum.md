# Evidence pack: Maybell Quantum
slug: maybell-quantum | domain: maybellquantum.com | HQ: Denver, Colorado US | employees(LI): 79 | founded:  | stage_band: unknown
sources: vc:decisive-point
industry(LI): Computer Hardware Manufacturing | specialties: 
channel notes: quantum computing infrastructure (dilution refrigerators)
own-language word count: ~1428

## Their own words (website via Exa, livecrawl-preferred)

URL: https://maybellquantum.com
The Quantum Infrastructure Company | Maybell

# The quantum infrastructure company

## Maybell builds dilution refrigerators that are intuitive, reliable, and obsessively engineered to meet the growing demands of the quantum industry.

Explore products →

## Dilution refrigerators

### The Fast Fridge

#### Quick turnaround, fast characterization

The Fast Fridge is affordable, flexible, and quick so you can iterate faster and get from idea to impact faster.

Explore the Fast Fridge →

### The Fridge

#### The original quantum workhorse, built to last

The Fridge offers 360˚ access and proven reliability with a footprint compact enough for almost any lab.

Explore the Fridge →

### The Fridge+

#### Upgrade your lab with premium performance

The Fridge+ offers everything you love about the Fridge plus faster sample swaps and 4x more sample capacity.

Explore the Fridge+ →

### The Big Fridge

#### Scale up your quantum research

The Big Fridge offers twice the power, twice the wire capacity, and five times more sample capacity for labs that demand scale.

Explore the Big Fridge →

## Decide which system is right for you

## Maybell builds beautiful, performant dilution refrigerators obsessively designed to support more qubits in a smaller footprint.

Take the quiz →

### Wiring solutions

#### Flexible, scalable wiring for every system

Our universally compatible wiring offers a lower thermal load in a more compact footprint.Explore wiring solutions →

### Magnets & accessories

#### Enhance your experimental environment

Maybell offers state-of-the-art HTS and LTS magnets and custom shielding solutionsExplore magnets & accessories →

## The latest Maybell news

### Maybell Quantum Launches ColdCloud®: A Patented Architecture for Scalable Quantum Cryogenics

March 12, 2026The ColdCloud® platform delivers up to 10x efficiency improvement, cooldown in hours instead of days, and datacenter-ready reliabilityRead more →

### QuantWare and Maybell partner to maximize compute-per-watt performance of VIO-40K systems

June 24, 2026Open-architecture partnership ensures Maybell’s ColdCloud® is commercially available for the VIO-40KTM system stack, as QuantWare prepares to deliver 10,000-qubit processors in 2028Read more →

### Maybell Quantum Announces $25M Series A Funding Led by Cerberus

March 4, 2024Maybell Quantum, the quantum infrastructure company, has closed a $25M funding round led by Cerberus Ventures. This investment marks the start of a pivotal chapter in Maybell’s development. With the financial backing of a leading global investment firm, Maybell intends to scale manufacturing and open a new production facility in 2024.Read more →

## Obsessively designed & tested

### Unmatched reliability

No scroll pumps, no solder joints or acid flux, no indium or epoxy, 90% reduction in rubber gaskets, and self-regenerating helium traps for maximum uptime

### User-centric design

Front-opening access, integrated workspace, and intuitive controls that let you focus on science, not babysitting your fridge

### Superior performance

Consistently achieving base temperatures below 10mK with the quietest experimental environment in the industry

### Expert support

Three-year comprehensive warranty with same-day response and a dedicated team of specialists who helped design your system

### American made

Designed and manufactured in Denver, Colorado with rigorous quality control and testing

Learn more → Contact us →

URL: https://maybellquantum.com/products/coldcloud
ColdCloud | Maybell

# MaybellColdCloud®

## Patented cryogenic infrastructure built for energy-efficient, reliable scaled quantum applications

Designed without compromise, Maybell’s ColdCloud® delivers ultra-low temperatures with efficiency, reliability, and performance impossible to achieve without our patented technology. This is the infrastructure that qubit builders need to reach utility-scale and beyond.

The world's first utility-scale cryogenic architecture, ColdCloud® decouples 4 Kelvin pre-cooling from the millikelvin stages, serving independent nodes below 10 mK from a centralized plant. It is an order of magnitude more efficient and more powerful than any existing sub-Kelvin system, and is protected by an extensive U.S. patent portfolio no other organization can replicate.

Contact us → About us →

## Scalable

### Operate your 4K stage at 8-15% of Carnot Efficiency, instead of the <1% typical in a pulse tube-based dilution refrigerator; reduced Helium 3 requirements by as much as 80%; spend up to 70% less installing and operating your lab.

## Reliable

### Scale cooling power without scaling failure points through centralized 4K and optimized mK systems; expected uptime: 96-99.9%

## Powerful

### Pair high-power 4K with high-performance sub-Kelvin systems; cool your nodes to base temperature in hours, not days

## Flexible

### ColdClouds can be optimized for base temperatures between <10mK and 6K+. System nodes can be warmed, cooled, and serviced independently, maximizing system availability.

## Scale from research hub to data center

### Each tier expands capacity with increasing thermodynamic efficiency. Scale from research hub to data center.

Scale

4K power

Nodes

Qubits

Research

25–80W

4–20

1k–20k+

Utility

80–250W

8–100

30 μW expected,28 μW guaranteed

20k–500k+

Data center

250W–1kW+

20–1,000+

1,100 μW expected,1,000 μW guaranteed

1M+

## Other ColdCloud features

## Node heterogeneity

Cool each node to a temperature ideal for a unique experiment or run the full cloud at one base temperature

## Unified control

Facility-level performance monitoring with real-time diagnostics and predictive maintenance systems

## Maybell’s Promise

The ColdCloud is designed with Maybell’s unmatched commitment to system excellence

## The latest Maybell news

### Maybell Quantum Launches ColdCloud®: A Patented Architecture for Scalable Quantum Cryogenics

March 12, 2026The ColdCloud® platform delivers up to 10x efficiency improvement,

URL: https://maybellquantum.com/products/fast-fridge
Fast Fridge | Maybell 

# TheFastFridge

## Fast turnaround, low cost, maximum flexibility

The Fast Fridge is affordable, flexible, and quick so you can iterate faster and get from idea to impact faster.

Contact us → View specs ↓

## Lowest cost

### Maximize your research budget with the most affordable path to millikelvin temperatures, without compromising performance.

Lower acquisition cost than competing dilution refrigerators

Reduced infrastructure requirements with compact footprint and integrated systems (e.g., no liquid nitrogen)

Minimal maintenance costs with self-regenerating traps and maintenance-free pumps

Energy-efficient design that cuts electricity consumption and cooling water requirements

## Maximum flexibility

### Adapt quickly to changing research priorities with a system designed for versatility and rapid reconfiguration.

Configurable 30mK or 10mK base temperature and custom shielding

Quick-swap wire loaders allows experiments to be prepared in parallel and swapped in minutes

Upgradable design lets you use the same wire tree for the Fridge+ or Big Fridge with no modifications

## Fastest cooldown

### The Fast Fridge is purpose-built for high-throughput experimentation and rapid characterization—all with minimal downtime.

24-hour turn around from room temperature to base temperature and back

Streamlined thermal path with reduced thermal mass for maximum efficiency

Automated cooldown sequences with state-based triggers so that every sample is cooled as quickly as possible

Specification

Fast Fridge (30 mK)

Fast Fridge (10 mK)

Fridge

Base temperature

<30 mK

<10 mK

<10 mK

Cooling power at 20 mK

N/A

14 μW expected,12 μW guaranteed

16 μW expected,14 μW guaranteed

Cooling power at 100 mK

300 μW expected,250 μW guaranteed

300 μW expected,250 μW guaranteed

500 μW expected,400 μW guaranteed

Mixing plate area

289 mm x 310 mm

289 mm x 310 mm

368 mm x 366 mm

Sample space

300 mm or 600 mm

300 mm or 600 mm

>300 mm

Wire insert area

>280 cm2

>280 cm2

>800 cm2

SMA coaxial line count

>100

>100

>250

SMPS Flexline count

>350

>350

>1,000

Next-gen Flexline count

>1,000

>1,000

>4,000

## Obsessively designed & tested

### Unmatched reliability

No scroll pumps, no solder joints or acid flux, no indium or epoxy, 90% reduction in rubber gaskets (compared with leading competitor), and self-regenerating helium traps

### User-centric design

Front-opening access, integrated workspace, and intuitive controls tha

URL: https://maybellquantum.com/products/fridge
Fridge | Maybell

# TheFridge

## The original Maybell workhorse, built to last

The Fridge offers 360˚ access and proven reliability with a footprint compact enough for almost any lab.

Contact us → View specs ↓

## Proven reliability

### The Fridge was built from the ground up to eliminate common failure points and maximize experimental uptime.

More than three years on market with a proven track record of outstanding performance

No solder joints or acid flux to degrade over time and cause leaks

Maintenance-free micro-root blowers replace failure-prone scroll pumps

Metal-to-metal and welded connections instead of rubber O-rings, indium, soft solder, and epoxy

Self-regenerating helium traps that never require manual refilling

Industry-leading service and support backing our confidence in system reliability

## 360° access

### Enjoy a revolutionary design that transforms how researchers interact with dilution refrigerators.

Open-door sample access provides unprecedented access to your experimental setup

Integrated chamber lift for full system disassembly and easy sample preparation and manipulation

Parallel front-loading wiring inserts allow complete wiring assemblies to be prepared outside the fridge

All-around accessibility eliminates blind spots and provides a similar experience to traditional systems

## Co

## LinkedIn about

Building the infrastructure for a quantum future so you can cool more with less

Our mission is to deliver the world’s best tools to solve the toughest quantum challenges, offering solutions that are more accessible, reliable, and affordable while building a work force and supply chain that advance U.S. quantum leadership. We listen to our customers, understand their needs, and employ cutting-edge science and human-centered engineering to deliver industry-leading performance and unrivaled user experience.

## News (Exa, top 3)
- CONTRACT to MAYBELL QUANTUM INDUSTRIES, INC. () https://www.usaspending.gov/award/CONT_AWD_FA875122C0007_9700_-NONE-_-NONE-
  USAspending.gov
- Award | SBIR () https://www.sbir.gov/awards/206815
  Award | SBIR Skip to main content

# Award

Portfolio Data

Back to Award Search

## Scaled Production of High-Density Cryogenic Flexible coAXial (FLAX) RF Ribbon Cables

### Awardee

#### MAYBELL QUANTUM INDUSTRIES, INC.

https://www.sbir.gov/portfolio/1831993 7100 Broadway Bldg 3 Unit D-EDenver, CO, 80221-2915USA

Award Year: 2023

UEI: DZCLWEMJRKU6

HUBZone Owned: No

Woman Owned: No

Socially and Economically Disadvantaged: No

Congressional District: N/A

Tagged as:

SBIR

Phase II

### Awa
- MAYBELL QUANTUM INDUSTRIES, INC. | Federal Award Recipient Profile () https://www.usaspending.gov/recipient/975e2571-a000-1f32-4291-d05ef3c167f4-C/latest
  USAspending.gov

# --

---

Loading

## Open roles (Blitz, live)
- Associate Product Engineer ({'city': 'Denver', 'country_code': 'US'})
- Vice President of Operations ({'city': 'Denver', 'country_code': 'US'})
- Technical Product Marketing Manager ({'city': 'Denver', 'country_code': 'US'})
- Validation Technician ({'city': 'Denver', 'country_code': 'US'})
- Director of Engineering ({'city': 'Denver', 'country_code': 'US'})
- Head of Strategic Finance ({'city': 'New York', 'country_code': 'US'})
- Head of Strategic Finance ({'city': 'Denver Metropolitan Area', 'country_code': 'US'})
- Head of Strategic Finance ({'city': 'Denver', 'country_code': 'US'})
- Product Marketing Manager ({'city': 'Denver', 'country_code': 'US'})
- Cryogenics R&D Scientist (Germany) ({'city': 'Münster', 'country_code': 'DE'})

---
# Context for the scoring agent (do not re-search)

## Barry Hess niche
Defense and intelligence technology companies in the $5M-$10M range whose proven prototype is stuck between demo and fielded deployment, guided by a CTO who fielded a patented tactical Android app platform with operators in theater and now engineers deployment architecture and acquisition advocacy from day one.

**EDP:** When months-to-field exceeds the funded months remaining on the contract, the program dies in the DoD valley of death - and for a $5M-$10M defense tech company, one dead transition is often the company.

## Deployed-reality checklist (score absence against this)
- **DDIL**: denied/disconnected/intermittent/low-bandwidth comms. Canonical example: an AI
  crowd-detection startup streamed 30fps video; the contested environment had bandwidth for
  1 frame every 5 seconds. One architecture change (frame-rate decoupling) saved the project.
- **SWaP-C**: size, weight, power, cost on the platform actually carried/mounted.
- **ATO/RMF**: Authority to Operate on government networks; STIGs, POA&Ms, IL4/IL5.
- **CMMC / NIST 800-171**: company-level compliance before the contract dies.
- **ITAR/export**: controlled data handling.
- **Operator training & sustainment**: who trains the E-5, who fixes it in the field, spares,
  battery logistics, GFE integration.
- **Advocacy chain**: a champion above the operator level; operators loving it is not adoption.
- **Prime/flow-down mechanics**: subcontract structure, government back-end integration.
- **TAK ecosystem**: if it touches situational awareness and never mentions ATAK/TAK, ask why.


## Doctrine
# Targeting doctrine brief (inject into every dossier & company agent — do NOT re-read the corpus)

Distilled from Jordan Crawford's Cannonball/Blueprint corpus, 2026-07-23.

1. **Pain-Qualified Segment beats ICP.** Never target firmographics ("50-200 person SaaS").
   Target companies in an observable painful SITUATION right now. The situation must be
   provable from public data.
2. **Existential Data Point (EDP).** For each niche, name the single number that kills the
   company if the problem stays unfixed (e.g. equipment utilization 60% vs 80% margins;
   fleet utilization <70% = crisis). Every dossier must state the EDP of its CTO's target
   situation: "what does it cost a company, in what metric, to leave this unfixed?"
3. **Jobs are confessions.** A job posting is a company publicly describing its own pain.
   We match jobs whose EXISTENCE signals a problem the CTO has already fixed — not jobs the
   CTO would apply to. "First Head of Engineering" = no technical leadership. "Integration
   Engineer (Epic, HL7)" = drowning in healthcare plumbing.
4. **The message is a redescription of the targeting.** If targeting is precise, the message
   writes itself by describing what we found. The reader should think "how do they know that?"
5. **Message-as-gift (GTM Shift letter standard).** Conversational, unhurried, 4–6 short
   sentences. Opens with a verified surprising fact. Every claim traces to a named public
   source. Valuable even if never answered. One light question, under 15 words. Bans: "I
   noticed", flattery, "hope you're well", meeting asks, personal details about the recipient
   as the opener. Lead with the CTO's expertise matched to the company's evident problem.
6. **Niche = Industry × Situation × Unique Angle** — all three specific (Ben Horowitz
   principle: not best botanist; best Japanese botanist specializing in Zen gardens).
7. **Insight tests**: counterintuitive (would other CTOs argue?), specific (exact tools/
   patterns), quantifiable (before/after numbers), demonstrable (showable in 5 minutes).
8. **Evidence honesty.** Actions > words > database records. Aggregator/API data (Blitz,
   FullEnrich) never corroborates itself — load-bearing claims need the person's or
   company's OWN public artifacts. Date every claim; label estimates as estimates.
9. **Dropping a bad fit is success.** Never pad a list to hit a count.
