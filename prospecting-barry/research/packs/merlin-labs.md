# Evidence pack: Merlin Labs
slug: merlin-labs | domain: merlinlabs.com | HQ: Boston, Massachusetts US | employees(LI): 224 | founded: 2019 | stage_band: unknown | LATE(151-300)
sources: vc:harpoon
industry(LI): Software Development | specialties: 
channel notes: autonomous flight for aircraft (NYSE: MRLN)
own-language word count: ~710

## Their own words (website via Exa, livecrawl-preferred)

URL: https://merlinlabs.com
Building Autonomous Aerospace today | Merlin Labs

2026 Copyright

Terms & Conditions

Privacy Policy

Building Autonomous Aerospace today

Watch Video Start Mission

Why Merlin

One Platform. Any Aircraft.

The Merlin Pilot is a single, aircraft-agnostic solution for autonomous aviation. Our software is designed for adaptation to any plane in operation today, and in the future. At Merlin, we build intelligence that flies.

Explore Pilot

$105M

USSCOM Contract Ceiling

C-130J

Prime Tactical Transport

100s

Hours Autonomous Flight

Mission Domains

Tactical Transport

Merlin is partnering with U.S. Special Operations Command to bring autonomy to the C-130J fleet with the goal of extending operational range for high-frequency routes.

Our Vision

The Future of American Aerospace is Autonomous

The Merlin Pilot is designed to extend the capabilities of existing fleets, provide robust safety, and unlock next generation missions. Merlin is creating the autonomous systems that will forge the next 100 years of American Aviation.

About Company

Our Collaborators

Careers

14 Open Roles

Join Merlin

URL: https://merlinlabs.com/
Building Autonomous Aerospace today | Merlin Labs

2026 Copyright

Terms & Conditions

Privacy Policy

Building Autonomous Aerospace today

Watch Video Start Mission

Why Merlin

One Platform. Any Aircraft.

The Merlin Pilot is a single, aircraft-agnostic solution for autonomous aviation. Our software is designed for adaptation to any plane in operation today, and in the future. At Merlin, we build intelligence that flies.

Explore Pilot

$105M

USSCOM Contract Ceiling

C-130J

Prime Tactical Transport

100s

Hours Autonomous Flight

Mission Domains

Tactical Transport

Merlin is partnering with U.S. Special Operations Command to bring autonomy to the C-130J fleet with the goal of extending operational range for high-frequency routes.

Our Vision

The Future of American Aerospace is Autonomous

The Merlin Pilot is designed to extend the capabilities of existing fleets, provide robust safety, and unlock next generation missions. Merlin is creating the autonomous systems that will forge the next 100 years of American Aviation.

About Company

Our Collaborators

Careers

14 Open Roles

Join Merlin

URL: https://merlinlabs.com/pilot/
Pilot | Merlin Labs

2026 Copyright

Terms & Conditions

Privacy Policy

Creating Certified Autonomy

Schedule Demo

Our Approach

One Brain. Infinite Missions.

The Merlin Pilot is a single system designed to be airframe and mission interoperable - able to be tailored to any use case. We intend to work directly with mission sponsors in the field to fine tune our software to their applications.

Start Mission

Our Collaborators

Tactical

Opportunity for crewed and uncrewed missions

Logistics

Opportunity to extend support for mission-critical flights around the clock

Commercial

Opportunity to reduce crew on commercial carriers to increase flight capacity

How It Works

High Agency Intelligence with Transparent Safety

The Merlin Pilot combines industry standards for safety with highly agile AI. These run in parallel to confirm and validate decisions.

Artificial Intelligence

Intelligent Planning

Our machine learning system adapts based on 100s of hours of flight to make decisions via real-time data and mission priorities.

Flight Systems

Deterministic Protocol

We employ hardcoded, known fallback systems that bolster safety and provide reliability through transparent means.

Our Roadmap

From First Principles to Full-Stack Autonomy

Merlin approaches autonomy as a series of questions about legacy constraints. This allows us to devise AI solutions from the ground up. We believe in methodically scaling these innovations through real world testing and certification.

Single Engine

Flight proven

Flight proven

Multi Engine

Flight proven

Flight proven

Transport Class

Developing...

Developing...

Tanker Class

Developing...

Developing...

Commercial Cargo

Future

Future

Drone

Future

Future

Careers

14 Open Roles

Join Merlin

URL: https://merlinlabs.com/news/
News | Merlin Labs

2026 Copyright

Terms & Conditions

Privacy Policy

Newsroom

AllUpdatePress ReleaseInvestor

Working with Israel Aerospace Industries to Advance Condor Update Jul 23, 2026

Merlin and IAI Sign Agreement to Advance AI-Powered Autonomy for Commercial Cargo AircraftPress Release ↗Jul 23, 2026

Autopilot to Autonomy: A Century of Handing Off the ControlsUpdate Jul 21, 2026

Merlin Completes Autonomous Landing at EAA AirVenture OshkoshPress Release ↗Jul 20, 2026

The Future of Flight Takes Center Stage: What Merlin Is Bringing to Oshkosh 2026Update Jul 17, 2026

Meet the Man Advancing Merlin's Safety Culture, Larry ColbyUpdate Jul 16, 2026

A Family Reunion: What Oshkosh Means to the Merlin TeamUpdate Jul 13, 2026

Oshkosh: The Story Behind One of the World's Greatest Aviation CelebrationsUpdate Jul 10, 2026

Why Merlin Is Called MerlinUpdate Jul 07, 2026

Show All

## LinkedIn about

Merlin is an aerospace and defense technology company building the operating system of record for autonomous flight. Our aircraft-agnostic, AI-powered software is purpose-built for military and civil programs, and is powering an expanding range of missions and aircraft, proven through hundreds of autonomous flights from test facilities across the globe. With $100M+ total in awarded contracts from military customers, Merlin is helping to solve national security challenges through safe, reliable autonomy.

## News (Exa, top 3)
- Merlin Awarded $105M Production Contract for USSOCOM Fixed-Wing ... (2024-06-11) https://www.businesswire.com/news/home/20240611723182/en/Merlin-Awarded-%24105M-Production-Contract-for-USSOCOM-Fixed-Wing-Autonomy
  Merlin Awarded $105M Production Contract for USSOCOM Fixed-Wing Autonomy

Jun 11, 2024 1:57 PM Eastern Daylight Time

# Merlin Awarded $105M Production Contract for USSOCOM Fixed-Wing Autonomy

Share

---

With an initial focus on the automation of the C-130J platform, this contract will expand Merlin’s technology into USSOCOM’s broader aircraft fleet over the next five years

Merlin Awarded $105M Production Contract for USSOCOM Fixed-Wing Autonomy (Photo: Business Wire)

Download media

BOSTON-
- Merlin Labs Secures $105M USSOCOM Tech Design, Integration Support Contract – ExecutiveBiz (2024-06-11) https://www.executivebiz.com/articles/merlin-labs-secures-105m-ussocom-tech-design-integration-support-contract
  Merlin Labs Secures $105M USSOCOM Tech Design, Integration Support Contract – ExecutiveBiz

Processing....

Hello, Guest!

Latest Federal & Government Contracting Companies' News Coverage

Menu

Menu

Latest Federal & Government Contracting Companies' News Coverage

Merlin Labs Logo / merlinlabs.com

Merlin Labs has received a potential five-year,$105 million contract from U.S. Special Operations Command to design, test and integrate a system that could demonstrate reduced aircrew capability.

T
- Special Operations Command Contracts Aircraft Design | AFCEA International () https://www.afcea.org/signal-media/special-operations-command-contracts-aircraft-design
  Special Operations Command Contracts Aircraft Design | AFCEA International

Enable breadcrumbs token at /includes/pageheader.html.twig

# Special Operations Command Contracts Aircraft Design

Jul 17, 2024

Merlin Labs Inc., Boston, Massachusetts, is being awarded an indefinite-delivery/indefinite-quantity contract (H9240824R0003) with a contract ceiling of $105,000,000. This contract is being awarded to design, integrate and test an operationally relevant system that will demonstrate reduced air

## Open roles (Blitz, live)
- Contract Recruiting Coordinator ({'city': 'Boston', 'country_code': 'US'})
- Engineering Manager, Autonomy Planning & Behaviors ({'city': None, 'country_code': 'US'})
- Engineering Manager, Autonomy Planning & Behaviors ({'city': 'Boston', 'country_code': 'US'})
- Senior Software Engineer- Flight Autonomy ({'city': 'Boston', 'country_code': 'US'})
- Senior Systems Engineer ({'city': 'Boston', 'country_code': 'US'})
- Senior Systems Engineer ({'city': 'Boston', 'country_code': 'US'})
- Program Operations Manager ({'city': 'Boston', 'country_code': 'US'})
- Senior Program Manager, Autonomy ({'city': 'Boston', 'country_code': 'US'})
- Senior Program Manager, Autonomy ({'city': 'Boston', 'country_code': 'US'})
- Program Operations Manager ({'city': 'Boston', 'country_code': 'US'})

---
# Context for the scoring agent (do not re-search)

## Barry Hess niche
Defense and intelligence technology companies in the $5M-$10M range whose proven prototype is stuck between demo and fielded deployment, guided by a CTO who fielded a patented tactical Android app platform with operators in theater and now engineers deployment architecture and acquisition advocacy from day one.

**EDP:** When months-to-field exceeds the funded months remaining on the contract, the program dies in the DoD valley of death - and for a $5M-$10M defense tech company, one dead transition is often the company.

## Deployed-reality checklist (score absence against this)
- **DDIL**: denied/disconnected/intermittent/low-bandwidth comms. Canonical example: an AI
  crowd-detection startup streamed 30fps video; the contested environment had bandwidth for
  1 frame every 5 seconds. One architecture change (frame-rate decoupling) saved the project.
- **SWaP-C**: size, weight, power, cost on the platform actually carried/mounted.
- **ATO/RMF**: Authority to Operate on government networks; STIGs, POA&Ms, IL4/IL5.
- **CMMC / NIST 800-171**: company-level compliance before the contract dies.
- **ITAR/export**: controlled data handling.
- **Operator training & sustainment**: who trains the E-5, who fixes it in the field, spares,
  battery logistics, GFE integration.
- **Advocacy chain**: a champion above the operator level; operators loving it is not adoption.
- **Prime/flow-down mechanics**: subcontract structure, government back-end integration.
- **TAK ecosystem**: if it touches situational awareness and never mentions ATAK/TAK, ask why.


## Doctrine
# Targeting doctrine brief (inject into every dossier & company agent — do NOT re-read the corpus)

Distilled from Jordan Crawford's Cannonball/Blueprint corpus, 2026-07-23.

1. **Pain-Qualified Segment beats ICP.** Never target firmographics ("50-200 person SaaS").
   Target companies in an observable painful SITUATION right now. The situation must be
   provable from public data.
2. **Existential Data Point (EDP).** For each niche, name the single number that kills the
   company if the problem stays unfixed (e.g. equipment utilization 60% vs 80% margins;
   fleet utilization <70% = crisis). Every dossier must state the EDP of its CTO's target
   situation: "what does it cost a company, in what metric, to leave this unfixed?"
3. **Jobs are confessions.** A job posting is a company publicly describing its own pain.
   We match jobs whose EXISTENCE signals a problem the CTO has already fixed — not jobs the
   CTO would apply to. "First Head of Engineering" = no technical leadership. "Integration
   Engineer (Epic, HL7)" = drowning in healthcare plumbing.
4. **The message is a redescription of the targeting.** If targeting is precise, the message
   writes itself by describing what we found. The reader should think "how do they know that?"
5. **Message-as-gift (GTM Shift letter standard).** Conversational, unhurried, 4–6 short
   sentences. Opens with a verified surprising fact. Every claim traces to a named public
   source. Valuable even if never answered. One light question, under 15 words. Bans: "I
   noticed", flattery, "hope you're well", meeting asks, personal details about the recipient
   as the opener. Lead with the CTO's expertise matched to the company's evident problem.
6. **Niche = Industry × Situation × Unique Angle** — all three specific (Ben Horowitz
   principle: not best botanist; best Japanese botanist specializing in Zen gardens).
7. **Insight tests**: counterintuitive (would other CTOs argue?), specific (exact tools/
   patterns), quantifiable (before/after numbers), demonstrable (showable in 5 minutes).
8. **Evidence honesty.** Actions > words > database records. Aggregator/API data (Blitz,
   FullEnrich) never corroborates itself — load-bearing claims need the person's or
   company's OWN public artifacts. Date every claim; label estimates as estimates.
9. **Dropping a bad fit is success.** Never pad a list to hit a count.
