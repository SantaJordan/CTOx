# Evidence pack: NPS Radar, LLC
slug: nps-radar-llc | domain: nps.ai | HQ: Pleasanton, California US | employees(LI): 14 | founded: 2018 | stage_band: preseed_seed
sources: exa-agent:sbir-graduates-raising
industry(LI): Computer Hardware Manufacturing | specialties: ai, defense, aerospace
channel notes: Series A Approximately $20M Series A/Seed round led by Cota Capital (2021). dom: sensors status: prototype dod: NPS Radar received a 2024 Army sequential Phase II SBIR for a multiband sensor suite and radar processing for autonomous military vehicles. hq: Pleasanton, CA emp: 11-50 employees src: https://www.sbir.gov/awards/209566
own-language word count: ~885

## Their own words (website via Exa, livecrawl-preferred)

URL: https://nps.ai
Neural Propulsion Systems - Digital Imaging Radar Technology

Home JZRdezineadmin 2026-05-14T15:45:07-07:00

# See the Unseen

Improved angular resolution, resolving closely spaced targets at distance and in any condition for enhanced situational awareness and efficient radar performance.

# See the Unseen

Improved angular resolution, resolving closely spaced targets at distance and in any condition for enhanced situational awareness and efficient radar performance.

###### Press Release

### NPS is Now Physical AI

# Multiple Targets. One Clear View.

Watch HROS in Action

## Precise Angular Resolution

Distinguishes tightly grouped targets more effectively than traditional radar modes.

## ImprovedTarget Clarity

Detects multiple objects under challenging conditions, day or night.

## EfficientSignal Processing

Delivers theoretical resolution limits with streamlined, high-speed data handling.

# Hyper-Definition Radar Operating System

Delivering significantly enhanced, reliable radar resolution with groundbreaking precision and responsiveness

LEARN MORE

###### TECHNOLOGY

## Atomic Norm is new math for 4D imaging radar that leads the way for defense.

LEARN MORE

###### COMPANY

## Empowering innovation and radar with mathematicians from Caltech, Stanford, UC Berkeley and Technical University of Munich

LEARN MORE

###### NEWS

###### VIEW MORE

https://nps.ai/news/

https://nps.ai/news/

News

#### Giving Vehicles Better Vision – Lidar-like Performance at Radar-like Cost

Published On: 03/15/2025

https://nps.ai/news/

News

#### Software-Defined Radar Is First Leap On SDV Path

Published On: 03/06/2025

https://nps.ai/news/

Press Release

#### The Dawn of AI Radar: NPS Delivers 10x Higher Resolution for ADAS, Ushering in a New Era of Automotive Safety

READ

Published On: 12/18/2024

# Are you ready to learn more?

GET IN TOUCH

Neural Propulsion Systems, Inc 7026 Koll Center Parkway, Pleasanton, CA 94566 [phone]

© NPS 2025. All Rights Reserved

URL: https://nps.ai/solutions/
Solutions - NPS

Solutions Josh Sandefer 2025-11-17T12:23:57-07:00

# Pioneering Next Generation Radar

Ground-breaking mathematical framework, AI and patented algorithms dramatically change how sensor data is processed and understood

# SOLUTIONS

### The Dawn of AI Radar

### The AtomicSense™ Platform

###### Advanced AI

###### 10X Higher Resolution

###### Unmatched Reliability

###### Precise Image Classification

###### DEFENSE

## Defense Applications

### Surface to Air

#### Ground/Space RadarECCM

### Air to Air

#### AESA RadarCouter-UAS

### Autonomy

#### Robotic Combat VehicleLogistics/Supply

###### OUTPERFORMS HUMAN VISION

# Radar that sees occluded objects

Vehicles, fence & tripod (600m)

#### Detects multiple points on all targets

#### Correctly estimates the speed of cross-traffic

Detects occluded objects

#### Uses multiple bands for obstruction mitigation

#### 5.8 GHz: white vehicle can be seen even when blocked by blue vehicle

Pedestrian next to a car

#### Detects driver as soon as vehicle door is opened

#### Gets multiple points oncar and driver

#### Tracks driver even when behind vehicle

# Are you ready to embrace groundbreaking radar performance?

GET IN TOUCH

###### NEWS

###### VIEW MORE

https://nps.ai/news/

https://nps.ai/news/

News

#### Giving Vehicles Better Vision – Lidar-like Performance at Radar-like Cost

Published On: 03/15/2025

https://nps.ai/news/

News

#### Software-Defined Radar Is First Leap On SDV Path

Published On: 03/06/2025

https://nps.ai/news/

Press Release

#### The Dawn of AI Radar: NPS Delivers 10x Higher Resolution for ADAS, Ushering in a New Era of Automotive Safety

READ

Published On: 12/18/2024

Neural Propulsion Systems, Inc 7026 Koll Center Parkway, Pleasanton, CA 94566 [phone]

© NPS 2025. All Rights Reserved

URL: https://nps.ai/about/
About NPS - Neural Propulsion Systems Team and Leadership

About Aldrin G 2025-11-17T12:26:58-07:00

###### Our Vision

# NPS Provides Crystal Clear Visibility for Defense Applications

## Founded in 2018 by Silicon Valley experts, NPS is a pioneer in next‑generation radar operating systems, delivering crystal‑clear visibility for mission‑critical defense applications. Its flagship product, Hyper‑Definition Radar Operating System (HROS), is an advanced radar solution that uses AI‑powered, patented mathematical frameworks to deliver unmatched clarity and responsiveness at a practical cost for large‑scale deployment. HROS currently supports multiple defense scenarios that rely on early threat detection and minimal false positives, giving military teams the awareness, confidence, and operational certainty required to understand what surrounds them in any mission-critical environment.

## Executive Team

#### BEHROOZ REZVANI

Ph.D. & CEO

#### Babak Hassibi

Ph.D. & CTO

#### Dr. Lawrence Burns

Exec. Advisor

#### Alysson Do

VP. Ops & Admin

#### Mohsen Moazami

Exec. Advisor

#### Daryoosh Rejaly

Director, HW Systems

#### Ljubo Mercep

PH.D. & Sr. Director, Perception

## Board of Directors

#### John Marren

##### VICE CHAIRMAN, NORTH AMERICA, TEMASEK

#### Elizabeth Cross

##### PRESIDENT, ARIAT INTERNATIONAL

#### Bobby Yazdani

##### FOUNDER & MANAGING PARTNER, COTA CAPITAL

#### Behrooz Rezvani, Ph.D

##### CEO & CHAIRMAN, NPS

## Observers

#### Rick Wagoner

##### FORMER GENERAL MOTORS CHAIRMAN & CEO

#### Marwan Fawaz

##### EXECUTIVE ADVISOR, GOOGLE & ALPHABET

#### Kevin Reid

##### CO-FOUNDER & PARTNER, BLUE LAGOON CAPITAL

## Advisors

#### Dan Warmenhoven

##### FORMER CEO & CHAIRMAN, NETAPP

#### Balan Nair

##### CEO, LIBERTY LATIN AMERICA

#### Aamir Hussain

##### VP & CHIEF PRODUCT OFFICER, VERIZON BUSINESS

#### Ross Ireland

##### FORMER SR. EVP & CTO, AT&T

# Are you ready to embrace groundbreaking radar performance?

GET IN TOUCH

###### NEWS

###### VIEW MORE

https://nps.ai/news/

https://nps.ai/news/

News

#### Giving Vehicles Better Vision – Lidar-like Performance at Radar-like Cost

Published On: 03/15/2025

https://nps.ai/news/

News

#### Software-Defined Radar Is First Leap On SDV Path

Published On: 03/06/2025

https://nps.ai/news/

Press Release

#### The Dawn of AI Radar: NPS Delivers 10x Higher Resolution for ADAS, Ushering in a New Era of Automotive Safety

READ

Published On: 12/18/2024

Neural Propulsion Systems, Inc 

## LinkedIn about

Founded in 2018 by Silicon Valley experts, NPS is a pioneer in next‑generation radar operating systems, delivering crystal‑clear visibility for mission‑critical defense applications. Its flagship product, Hyper‑Definition Radar Operating System (HROS), is an advanced radar solution that uses AI‑powered, patented mathematical frameworks to deliver unmatched clarity and responsiveness at a practical cost for large‑scale deployment. HROS currently supports multiple defense scenarios that rely on early threat detection and minimal false positives, giving military teams the awareness, confidence, a

## News (Exa, top 3)
- Neural Propulsion Systems Secures Series B Strategic Investment from Raytheon Technologies - NPS (2023-03-02) https://nps.ai/neural-propulsion-systems-secures-series-b-strategic-investment-from-raytheon-technologies/
  Neural Propulsion Systems Secures Series B Strategic Investment from Raytheon Technologies - NPS

# Neural Propulsion Systems Secures Series B Strategic Investment from Raytheon Technologies

###### SHARE

Funding Will Enable NPS to Advance New Hyper-Resolution Digital Imaging Radar Technology

PLEASANTON, Calif. – March 2, 2023 – Neural Propulsion Systems (NPS), a pioneer in hyper-resolution digital imaging radar technology, today announced a new Series B strategic investment by RTX Ventures, t
- NPS Completes $17.5M Series B Funding for Software-Defined ... (2024-03-07) https://www.prnewswire.com/news-releases/nps-completes-17-5m-series-b-funding-for-software-defined-radar-led-by-cota-capital-with-key-strategic-investment-from-gm-ventures-and-rtx-ventures-302082398.html
  NPS Completes $17.5M Series B Funding for Software-Defined Radar Led by Cota Capital with Key Strategic Investment from GM Ventures and RTX Ventures Accessibility Statement Skip Navigation

PLEASANTON, Calif., March 7, 2024 /PRNewswire/ -- Neural Propulsion Systems (NPS), a pioneer in software-defined radar, today announced it secured a $17.5 million Series B funding round. The round is led by Cota Capital with contributions from GM Ventures, the venture capital arm of General Motors Co. (NYSE: 
- Raytheon Technologies Invests In Hyper-Resolution Radar Company - Defense Daily (2023-03-02) https://www.defensedaily.com/raytheon-technologies-invests-in-hyper-resolution-radar-company/advanced-transformational-technology/
  Raytheon Technologies Invests In Hyper-Resolution Radar Company - Defense Daily

:

Search

Dr. Behrooz Rezvani, founder and CEO of NPS. Photo: NPS

Neural Propulsion Systems (NPS), a small company developing hyper-resolution radar technology for autonomous vehicles and other applications, said on Thursday it has received a strategic investment from Raytheon Technologies [RTX] to help further develop the technology for greater resolution and range for mobility and defense uses. The value of the 

## Open roles
(no jobs data found)

---
# Context for the scoring agent (do not re-search)

## Barry Hess niche
Defense and intelligence technology companies in the $5M-$10M range whose proven prototype is stuck between demo and fielded deployment, guided by a CTO who fielded a patented tactical Android app platform with operators in theater and now engineers deployment architecture and acquisition advocacy from day one.

**EDP:** When months-to-field exceeds the funded months remaining on the contract, the program dies in the DoD valley of death - and for a $5M-$10M defense tech company, one dead transition is often the company.

## Deployed-reality checklist (score absence against this)
- **DDIL**: denied/disconnected/intermittent/low-bandwidth comms. Canonical example: an AI
  crowd-detection startup streamed 30fps video; the contested environment had bandwidth for
  1 frame every 5 seconds. One architecture change (frame-rate decoupling) saved the project.
- **SWaP-C**: size, weight, power, cost on the platform actually carried/mounted.
- **ATO/RMF**: Authority to Operate on government networks; STIGs, POA&Ms, IL4/IL5.
- **CMMC / NIST 800-171**: company-level compliance before the contract dies.
- **ITAR/export**: controlled data handling.
- **Operator training & sustainment**: who trains the E-5, who fixes it in the field, spares,
  battery logistics, GFE integration.
- **Advocacy chain**: a champion above the operator level; operators loving it is not adoption.
- **Prime/flow-down mechanics**: subcontract structure, government back-end integration.
- **TAK ecosystem**: if it touches situational awareness and never mentions ATAK/TAK, ask why.


## Doctrine
# Targeting doctrine brief (inject into every dossier & company agent — do NOT re-read the corpus)

Distilled from Jordan Crawford's Cannonball/Blueprint corpus, 2026-07-23.

1. **Pain-Qualified Segment beats ICP.** Never target firmographics ("50-200 person SaaS").
   Target companies in an observable painful SITUATION right now. The situation must be
   provable from public data.
2. **Existential Data Point (EDP).** For each niche, name the single number that kills the
   company if the problem stays unfixed (e.g. equipment utilization 60% vs 80% margins;
   fleet utilization <70% = crisis). Every dossier must state the EDP of its CTO's target
   situation: "what does it cost a company, in what metric, to leave this unfixed?"
3. **Jobs are confessions.** A job posting is a company publicly describing its own pain.
   We match jobs whose EXISTENCE signals a problem the CTO has already fixed — not jobs the
   CTO would apply to. "First Head of Engineering" = no technical leadership. "Integration
   Engineer (Epic, HL7)" = drowning in healthcare plumbing.
4. **The message is a redescription of the targeting.** If targeting is precise, the message
   writes itself by describing what we found. The reader should think "how do they know that?"
5. **Message-as-gift (GTM Shift letter standard).** Conversational, unhurried, 4–6 short
   sentences. Opens with a verified surprising fact. Every claim traces to a named public
   source. Valuable even if never answered. One light question, under 15 words. Bans: "I
   noticed", flattery, "hope you're well", meeting asks, personal details about the recipient
   as the opener. Lead with the CTO's expertise matched to the company's evident problem.
6. **Niche = Industry × Situation × Unique Angle** — all three specific (Ben Horowitz
   principle: not best botanist; best Japanese botanist specializing in Zen gardens).
7. **Insight tests**: counterintuitive (would other CTOs argue?), specific (exact tools/
   patterns), quantifiable (before/after numbers), demonstrable (showable in 5 minutes).
8. **Evidence honesty.** Actions > words > database records. Aggregator/API data (Blitz,
   FullEnrich) never corroborates itself — load-bearing claims need the person's or
   company's OWN public artifacts. Date every claim; label estimates as estimates.
9. **Dropping a bad fit is success.** Never pad a list to hit a count.
