# Evidence pack: Orbital Operations
slug: orbital-operations | domain: orbitalops.tech | HQ: ,   | employees(LI): 22 | founded: 2024 | stage_band: unknown
sources: vc:harpoon
industry(LI): Defense and Space Manufacturing | specialties: 
channel notes: high-thrust space vehicles
own-language word count: ~1015

## Their own words (website via Exa, livecrawl-preferred)

URL: https://orbitalops.tech
Home - Orbital Ops

Astraeus maneuvers at mission speed, delivering intercept, defense, and repositioning capabilities without delay. Defense ready. Unmatched delta-V.

Contact Us

## Engineered for orbital dominance.

OUR MISSION

Thrust (lbf)

0 K

Payload Hosting (kg)

0

LEO-GEO-LEO Delivery (kg)

0.1 K

Delta-v (km/s)

0 +

Our vehicles provide the power to maneuver decisively in LEO to cislunar orbit, within hours—not months. Using tested and proven technologies for active cooling and storage—we are unlocking true defense capabilities in space.

THE TECHNOLOGY

##### Partners

### Latest

VIEW ALL

- Defense

### Orbital Operations Raises $8.8M to Build High-Thrust Vehicle for Satellite Defense

https://www.orbitalops.tech/orbital-operations-raises-8-8m-to-build-high-thrust-vehicle-for-satellite-defense/

- Milestones

### The Road to Astraeus

https://www.orbitalops.tech/the-road-to-astraeus/

- Tech

### Why Liquid Hydrogen

https://www.orbitalops.tech/why-liquid-hydrogen/

### Learn how to partner and get started with Orbital Operations

Contact Us

Made to move. Designed to defend.

© Orbital Operations. All Rights Reserved

URL: https://www.orbitalops.tech/technology/
Technology - Orbital Ops

Orbital Operations builds high-thrust, cryogenic space vehicles for rapid response and dynamic in-orbit missions.

Contact Us

### Protection. Precision. Power.

Defense

Space is now a contested domain. Astraeus is designed to patrol and protect U.S. assets in orbit, with the ability to intercept threats, deploy countermeasures, and reposition rapidly. Stationed in LEO with high-thrust cryogenic propulsion, it enables real-time response to adversarial maneuvers—turning months of lead time into hours.

Commercial

Satellite operators face growing challenges in positioning, servicing, and deorbiting their assets. Astraeus offers on-demand mobility for repositioning, life extension, and end-of-life disposal, all without waiting for the next launch window. It’s a new paradigm for commercial operators seeking precision and autonomy in orbit.

Infrastructure

Building the foundation for long-term space presence requires in-orbit fuel, power, and thermal systems. Astraeus supports future infrastructure with the ability to transport cargo, enable refueling, and demonstrate scalable cryogenic management. It’s the utility vehicle for the next generation of orbital construction and logistics.

### Outpace. Outmaneuver. Outlast.

Cryogenic Engines

Astraeus leverages proven cryogenic engine technology derived from launch vehicles, adapted for continuous use in space. This heritage thrust system delivers high performance and reliability, enabling rapid maneuvers and sustained operations far beyond Earth’s atmosphere.

Zero Boil-Off Storage

Using an advanced cooling system, Astraeus maintains cryogenic propellant at stable temperatures for long-duration missions. This zero boil-off capability allows the vehicle to loiter in orbit, always ready to engage without losing fuel to evaporation.

Integrated Radiative Cooling

Rather than relying on traditional radiators, Astraeus integrates its thermal management system directly its engine. This design reduces dry mass and complexity while efficiently dissipating heat—an elegant solution made possible by advanced materials and manufacturing.

Refuelable = Reusable

Astraeus is built for long-term sustainability in orbit. It can be refueled using water-based electrolysis systems or through cryogenic propellant transfers from launch vehicles, unlocking true orbital persistence.

## Engineered for orbital dominance.

| CASE | PAYLOAD | DELTA—V | TIME |
| --- | --- | --- | --- |
| HOSTED PAYLOAD | 300 K

URL: https://orbitalops.tech/
Home - Orbital Ops

Astraeus maneuvers at mission speed, delivering intercept, defense, and repositioning capabilities without delay. Defense ready. Unmatched delta-V.

Contact Us

## Engineered for orbital dominance.

OUR MISSION

Thrust (lbf)

0 K

Payload Hosting (kg)

0

LEO-GEO-LEO Delivery (kg)

0.1 K

Delta-v (km/s)

0 +

Our vehicles provide the power to maneuver decisively in LEO to cislunar orbit, within hours—not months. Using tested and proven technologies for active cooling and storage—we are unlocking true defense capabilities in space.

THE TECHNOLOGY

##### Partners

### Latest

VIEW ALL

- Defense

### Orbital Operations Raises $8.8M to Build High-Thrust Vehicle for Satellite Defense

https://www.orbitalops.tech/orbital-operations-raises-8-8m-to-build-high-thrust-vehicle-for-satellite-defense/

- Milestones

### The Road to Astraeus

https://www.orbitalops.tech/the-road-to-astraeus/

- Tech

### Why Liquid Hydrogen

https://www.orbitalops.tech/why-liquid-hydrogen/

### Learn how to partner and get started with Orbital Operations

Contact Us

Made to move. Designed to defend.

© Orbital Operations. All Rights Reserved

URL: https://www.orbitalops.tech/company/
Company - Orbital Ops

Our mission demands bold thinking, deep engineering, and a team ready to move fast. If you’re driven to shape the next era of space security, we want you with us.

## Orbital Operations

THE TECHNOLOGY

Our vision is to set a new standard for space mobility. One where Orbital Operations’ fleet maintains unbroken orbital dominance, empowers sustainable in-space infrastructure, and safeguards humanity’s interests across LEO, GEO, and beyond.

The current mission is engineering and deploying high-thrust, cryogenic-powered space vehicles delivering rapid-response interception and in-orbit defense capabilities. These vehicles deliver decisive protection for critical assets, ensuring security at the speed of need.

## Milestones

NEWSROOM

Commercial LETTERS OF INTENT

$ 0 B

Funding Secured

$ 0 M

sqft facility

0 K

## The Founding Team

## Ben Schleuniger

### CEO | CO-FOUNDER

Ben has over 7+ years of experience in development aerospace, from NASA, SpaceX, and Relativity. An early employee at Relativity Space, Ben owned the initial fluid systems design for Terran 1, built the components team from the ground up, and qualified the flight termination valves. From there he transitioned to turbomachinery, designing the heat exchangers for the Aeon R engine and becoming the Lead Engineer for the Integrated Design and Analysis team on propulsion.

While working at Relativity, Ben also supported the Dark Fission team by designing, analyzing, and creating test plans for their first nuclear propulsion demonstrator.

## Ross Doherty

### COO | CO-FOUNDER

Ross has been in the aerospace industry for nearly a decade, with experience from Aerojet Rocketdyne, Blue Origin, and Relativity Space. Ross joined Relativity to become the Lead Turbomachinery Manufacturing Engineer where he was instrumental in the build, test, and issue resolution of the Aeon 1 turbopumps and the flight termination valves. Ross was repeatedly flown out to Cape Canaveral by Relativity to support testing and re-works of the engine on the launch pad.

Ross transitioned to becoming the Lead Turbomachinery Manufacturing Engineer for Aeon R supporting development, qualification, and testing of the new engine.

## Nico Montoya

### HEAD OF TEST & OPS

Nico has over a decade of experience in propulsion and manufacturing across Blue Origin, Relativity Space, and Stoke Space. At Blue Origin, he was a Propulsion Development Engineer on the BE-4 engine program, leading assembly and testi

## LinkedIn about

Orbital Operations is developing a high thrust space vehicle that will be stationed in orbit for satellite defense.

## News (Exa, top 3)
- Orbital Operations Raises $8.8M For High Thrust Orbital Vehicle (2025-08-07) https://payloadspace.com/orbital-operations-raises-8-8m-for-high-thrust-orbital-vehicle/
  Orbital Operations Raises $8.8M For High Thrust Orbital Vehicle

The latest concept vehicle for the Space Force has arrived: A high-thrust, cryogenic platform that can loiter on-orbit for years at a time, as a tool for interception and space domain awareness.

Orbital Operations, founded by former Relativity engineers Ben Schleuniger and Ross Doherty, is working to solve a problem that has plagued high-performance space vehicles for years: Limiting the boil-off of the most powerful propellants, 
- Orbital Operations Raises $8.8M to Build High-Thrust Vehicle for ... (2025-08-19) https://www.orbitalops.tech/orbital-operations-raises-8-8m-to-build-high-thrust-vehicle-for-satellite-defense/
  Orbital Operations Raises $8.8M to Build High-Thrust Vehicle for Satellite Defense - Orbital Ops

###### NEWSROOM

https://www.orbitalops.tech/newsroom/

###### /

###### MILESTONE

https://www.orbitalops.tech/newsroom/?e-filter-9be65ac-category=defence

# Orbital Operations Raises $8.8M to Build High-Thrust Vehicle for Satellite Defense

### Articles

Payload: Orbital Operations Raises $8.8M For High Thrust Orbital Vehicle

Space News: Orbital Operations raises seed round for orbital vehicle us
- orbital operations (2026-06-24) https://orbitalops.tech/
  # Orbital Operations (Orbital Operations Incorporated)

Orbital Operations is a Defense and Space Manufacturing company. Orbital Operations is a pioneering in-space cryo management tech company that unlocks high thrust, high performance engines beyond earth’s atmosphere. Their vehicle is designed to operate permanently in space to transport customers from leo to meo, geo, and the moon. Their capabilities also put them ahead of the competition for rapid threat assessment in orbit. Orbital Operati

## Open roles (Blitz, live)
- Head of Business Development ({'city': 'Long Beach', 'country_code': 'US'})
- Fluids Design Engineer - Heat Exchangers ({'city': 'Long Beach', 'country_code': 'US'})

---
# Context for the scoring agent (do not re-search)

## Barry Hess niche
Defense and intelligence technology companies in the $5M-$10M range whose proven prototype is stuck between demo and fielded deployment, guided by a CTO who fielded a patented tactical Android app platform with operators in theater and now engineers deployment architecture and acquisition advocacy from day one.

**EDP:** When months-to-field exceeds the funded months remaining on the contract, the program dies in the DoD valley of death - and for a $5M-$10M defense tech company, one dead transition is often the company.

## Deployed-reality checklist (score absence against this)
- **DDIL**: denied/disconnected/intermittent/low-bandwidth comms. Canonical example: an AI
  crowd-detection startup streamed 30fps video; the contested environment had bandwidth for
  1 frame every 5 seconds. One architecture change (frame-rate decoupling) saved the project.
- **SWaP-C**: size, weight, power, cost on the platform actually carried/mounted.
- **ATO/RMF**: Authority to Operate on government networks; STIGs, POA&Ms, IL4/IL5.
- **CMMC / NIST 800-171**: company-level compliance before the contract dies.
- **ITAR/export**: controlled data handling.
- **Operator training & sustainment**: who trains the E-5, who fixes it in the field, spares,
  battery logistics, GFE integration.
- **Advocacy chain**: a champion above the operator level; operators loving it is not adoption.
- **Prime/flow-down mechanics**: subcontract structure, government back-end integration.
- **TAK ecosystem**: if it touches situational awareness and never mentions ATAK/TAK, ask why.


## Doctrine
# Targeting doctrine brief (inject into every dossier & company agent — do NOT re-read the corpus)

Distilled from Jordan Crawford's Cannonball/Blueprint corpus, 2026-07-23.

1. **Pain-Qualified Segment beats ICP.** Never target firmographics ("50-200 person SaaS").
   Target companies in an observable painful SITUATION right now. The situation must be
   provable from public data.
2. **Existential Data Point (EDP).** For each niche, name the single number that kills the
   company if the problem stays unfixed (e.g. equipment utilization 60% vs 80% margins;
   fleet utilization <70% = crisis). Every dossier must state the EDP of its CTO's target
   situation: "what does it cost a company, in what metric, to leave this unfixed?"
3. **Jobs are confessions.** A job posting is a company publicly describing its own pain.
   We match jobs whose EXISTENCE signals a problem the CTO has already fixed — not jobs the
   CTO would apply to. "First Head of Engineering" = no technical leadership. "Integration
   Engineer (Epic, HL7)" = drowning in healthcare plumbing.
4. **The message is a redescription of the targeting.** If targeting is precise, the message
   writes itself by describing what we found. The reader should think "how do they know that?"
5. **Message-as-gift (GTM Shift letter standard).** Conversational, unhurried, 4–6 short
   sentences. Opens with a verified surprising fact. Every claim traces to a named public
   source. Valuable even if never answered. One light question, under 15 words. Bans: "I
   noticed", flattery, "hope you're well", meeting asks, personal details about the recipient
   as the opener. Lead with the CTO's expertise matched to the company's evident problem.
6. **Niche = Industry × Situation × Unique Angle** — all three specific (Ben Horowitz
   principle: not best botanist; best Japanese botanist specializing in Zen gardens).
7. **Insight tests**: counterintuitive (would other CTOs argue?), specific (exact tools/
   patterns), quantifiable (before/after numbers), demonstrable (showable in 5 minutes).
8. **Evidence honesty.** Actions > words > database records. Aggregator/API data (Blitz,
   FullEnrich) never corroborates itself — load-bearing claims need the person's or
   company's OWN public artifacts. Date every claim; label estimates as estimates.
9. **Dropping a bad fit is success.** Never pad a list to hit a count.
