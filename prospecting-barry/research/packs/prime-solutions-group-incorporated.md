# Evidence pack: PRIME SOLUTIONS GROUP, INCORPORATED
slug: prime-solutions-group-incorporated | domain: psg-inc.net | HQ: Goodyear, Arizona US | employees(LI): 52 | founded: 2007 | stage_band: revenue_stage
sources: sbir
industry(LI): IT Services and IT Consulting | specialties: systems engineering, hardware and software integration, complex system modeling and simulation, cloud architectures, synthetic aperture radar, intelligence, surveillance and reconnaissance, cloud development
channel notes: SBIR PhII x1 [2023] National Geospatial-Intelligence Agency latest: DIALR Phase II hq: Goodyear,AZ emp: 40
own-language word count: ~1625

## Their own words (website via Exa, livecrawl-preferred)

URL: https://psg-inc.net
Prime Solutions Group solves SAR and big data challenges

# Prime Solutions Group, Inc.

# Software & Systems Engineering for Radar & Big Data

We understand the challenges you have in making your synthetic aperture radar (SAR) systems and big data usable and profitable.

Prime Solutions Group has been solving SAR and big data challenges for the US Government and companies like Raytheon, Lockheed Martin, IBM, and others for more than 15 years.

### Consulting

Receive Consulting on How to Leverage Insights from Your Data

### Performance

Get Timely Data Processing for Better Decisions

### Intelligence

Derive Useful, Actionable Intelligence from Synthetic Aperture Radar and Big Data

## The PSG Assurance Plan

### How to get started

Request a Consultation

Experience a Discovery Call

Receive a Custom Proposal

Improve the Utility and Profitability of Your Data

1

Request a Consultation

2

Experience a Discovery Call

3

Receive a Custom Proposal

4

Improve the Utility and Profitability of Your Data

## The Prime Solutions Group Promise

### Better Insights. Trusted Solutions.

- Actionable insights that lead to tangible business opportunities
- Work with a company that understands synthetic aperture radar (SAR), multi-sensor fusion, and big data
- Take advantage of next-generation AI, Machine Learning, and cloud infrastructure to better solve problems and control costs
- Gain the flexibility of working with an agile group of expert systems software engineers
- Leverage the quality culture, systems and software expertise trusted by defense industry leaders

## About Prime Solutions Group Experts

## Experts in Geo-Spatial Mission Solutions

Scalable Architectures Processing Workflows Cloud Integration

## Experts in Radar & Complex ISR Systems

Scalable Architectures Processing Workflows Cloud Integration

## Experts in Multi-Source Sensor Processing &Information Analytics

Scene Understanding & Change Detection Sensor Phenomenology & Analytics Collaborative R&D

## Experts in Model Based Systems Engineering

Development of PSG SAR Processing Product Flow Space System, Satellite, and Sensor DigitalEngineering Design

## Experts in Artificial Intelligence & MachineLearning

AI and ML Workflow Systems Management, and SAR Modeling and Simulation

Prime Solutions Group, Inc. (PSG) is a professional engineering services company with a advancing Intelligence, Surveillance and Reconnaissance (ISR) technologies. Leveraging deep experience and expertise in Synthetic Aperture Radar (SAR) processing, core skills in complex system-of-systems engineering, and cutting-edge applied research and development in image-based Machine Learning, PSG helps solve the 21st-century challenges faced by both private industry and government organizations.

Our roots trace back to Carl Wiley and the Goodyear Aircraft Company that first patented synthetic aperture radar (SAR) in 1951. The Goodyear Aircraft Company eventually developed into Loral Defense Systems and Lockheed Martin IS&GS. Many of the experts at PSG have Lockheed Martin pedigrees.

PSG has the experience and agility to rapidly and affordably tackle customers’ needs in today’s challenging and increasingly complex environments.

## Staff

- Highly Skilled Personnel with Security Clearances
- Most of the Workforce Hold Advanced Degrees and Technical Staff Members Have an Average of 25 Years of Experience
- A well-balanced team of Subject Matter Experts and Implementers

## Facilities

- 30,000 Sq. Ft. of Office Space with 8000+ Sq. Ft. SCIF Space
- Classified IT Infrastructure and Connectivity
- Unclassified CMMC Compliant IT Infrastructure

- Customers & Industry Leaders
- Partners & Affiliations

## What Our Clients Say

> The team did a great job and came together to produce technical products that was critical in moving the effort forward. PSG came through for us by committing their most senior people.

E.A., Raytheon

> Very much appreciate the continued hard work to bring the system up to compliance… Thank you for your on-going assessment of the target completion date.

W.B., Lockheed Martin

> Great scientific approach … Very thorough and pertinent work

T.M., NGA

### Your Guide

## Top 9 Challenges of Working with SAR Data and How to Solve Them

https://psg-inc.net/top-9-challenges-of-working-with-sar-data-and-how-to-solve-them#form

Download the Guide

## Connect with Us

We are eager to hear about what you are working on and how we might help.

Contact Us

URL: https://psg-inc.net/about-psg/
About PSG - Prime Solutions Group, Inc. - Systems and Software Solutions

# About PSG

## PSG: Transforming Data into Knowledge

Prime Solutions Group, Inc. (PSG) is a professional engineering services company with a legacy in Intelligence, Surveillance & Reconnaissance (ISR) technology. Leveraging deep experience and expertise in synthetic aperture radar (SAR) processing, core skills in complex system-of-systems engineering, and cutting-edge applied research and development in image-based machine learning, PSG helps solve the 21st century challenges faced by both private industry and government organizations.

Our roots trace back to Carl Wiley and the Goodyear Aircraft Company that first patented synthetic aperture radar (SAR) in 1951. The Goodyear Aircraft Company eventually developed into Loral Defense Systems and Lockheed Martin IS&GS. Many of the experts at PSG have Lockheed Martin pedigrees.

As a small business with minimal overhead, PSG has the experience and agility to rapidly and affordably tackle customers’ needs in today’s challenging and increasingly complex environments.

1941

### Goodyear Aircraft Corporation (GAC)

Was established as the Litchfield Naval Air Facility that employed as many as 7,500 people at one time. Goodyear Aircraft Corporation was where some of the famous blimps were manufactured that hovered over sporting events like the Rose Bowl football game and the Indianapolis 500.

1951

### SAR Was Invented

Carl Wiley invented Synthetic Aperture Radar (SAR). His invention resulted in the creation of the first SAR patent, first digital SAR processor, and development of several different high performance SAR systems.

1963

### Goodyear Aerospace Corporation

Goodyear Aircraft Corporation was renamed Goodyear Aerospace Corporation.

1987

### Loral Corp Buys GAC

Goodyear Tire and Rubber Company sold Goodyear Aerospace Corporation (GAC) to Loral for $640 million following a massive restructuring of Goodyear. The Goodyear name disappeared and became the defense systems unit of Loral.

1996

### Lockheed Martin Acquires Loral

On April 22, 1996, Lockheed Martin completed the acquisition of Loral Corporation’s defense electronics and system integration businesses for $9.1 billion.

2007

### Prime Solutions Group, Inc

Joseph Marvin established PSG to continue the mission with decades of SAR expertise.

Today

### Continuing the Mission

PSG continues to grow, innovate, and evolve across SAR, AI/ML, digital engineering, cloud techn

URL: https://psg-inc.net/products-services/
Products & Services - Prime Solutions Group, Inc. - Systems and Software Solutions

# Products & Services

## Systems

Rely on years of experience in systems engineering, complex system-of-systems design, implementation, and leadership on large scale, high-volume systems. We can help engineer the right systems solution for your needs.

Learn More

## Software

Our staff has years of expertise in high-speed, high-volume sensor data management, all built upon a solid foundation of complex system-of-systems engineering.

Learn More

## Information Security

Information system security has been an integral part of the PSG workplace environment from the beginning. We provide the secure facilities, staff, infrastructure, and processes required for securing success in today’s information-based business environment.

## Tomorrow’s Transformations

Our technologists collaboratively conduct state-of-the-art research and development (R&D) on transformative technology. We extend our core work with informative learning to explore and facilitate the digital paradigm shift. Discover how we pursue exciting emerging technologies.

## Data Science

It all starts with data. PSG experience in ISR analytics & data science has created new opportunities for characterizing sensor-based systems used in defense and commercial energy grid management. A mix of technology and legacy expertise helps us advance proprietary algorithms for extracting knowledge from data sources. We draw on statistical methods and machine learning experience during feature engineering and model construction to help us develop applications that deliver insights to stakeholders.

## Research & Development Projects

PSG actively participates in Small Business Innovation Research (SBIR) projects dealing with complex system-of-systems engineering and data science. Our focus is on modeling and simulation of complex system performance, artificial intelligence and machine learning applied to sensor data, and innovative analytics for creating or exploiting sensor products. PSG also engages with academia at universities through collaboration with the SenSIP research consortium, as well as research projects with engineering students at Arizona State University.

Learn More

URL: https://psg-inc.net/
Prime Solutions Group solves SAR and big data challenges

# Prime Solutions Group, Inc.

# Software & Systems Engineering for Radar & Big Data

We understand the challenges you have in making your synthetic aperture radar (SAR) systems and big data usable and profitable.

Prime Solutions Group has been solving SAR and big data challenges for the US Government and companies like Raytheon, Lockheed Martin, IBM, and others for more than 15 years.

### Consulting

Receive Consulting on How to Leverage Insights from Your Data

### Performance

Get Timely Data Processing for Better Decisions

### Intelligence

Derive Useful,

## LinkedIn about

Prime Solutions Group, Inc (PSG) provides engineering services and software data processing products for remote sensing systems. Our expert RF (SAR, Infrared, Electro-Optical) systems engineers and data scientists translate physics-based and nonlinear (Artificial Intelligence/Machine Learning) methods using advanced modeling and simulation tools. These outputs are used by software developers who develop, integrate, test and deploy higher order, scripting or tool language into software code executable in mission environments using cloud, on-premise or edge computing resources. Our unique brand 

## SBIR/STTR record (their own abstracts)

### DIALR Phase II — National Geospatial-Intelligence Agency 2023 ($999,991)
The pace of remotely sensed data collection is increasing exponentially while the skilled workforce that is available to analyze and interpret the data is not. Artificial Intelligence (AI) offers a technology pathway to automation which promises to rapidly reduce the gap between data and information products and services. However, AI techniques require massive quantities of labeled data to train models creating even more demand of skilled analyst time. The cost of labeling by professional analysts is exacerbated further for Synthetic Aperture Radar (SAR) imagery because it is non-literal compared to electro-optical modalities, requiring more interrogation time.   Prime Solutions Group, Inc. (PSG) proposes the Detect and Identify Automated Labeler for Radar (DIALR) SBIR Phase II program to develop and demonstrate a prototype algorithm that automatically detects, classifies, and labels targets in SAR imagery, including complex target collection scenarios. PSG’s innovative approach performs hierarchical classification within a taxonomy schema that aligns with NGA-Research’s Research Development Environment (RDE) taxonomy structure for seamless interaction between labeler and NGA-Research's pipeline management system. This combination of AI innovation and systems design will radically reduce data curation timelines and costs for the modeling mission of rapidly delivering derived information to decision makers.

## News (Exa, top 3)
- Goodyear defense contractor receives prestigious research grant from United States Air Force | News | Goodyear, AZ Economic Development () https://www.developgoodyearaz.com/Home/Components/News/News/1569/769
  Goodyear defense contractor receives prestigious research grant from United States Air Force | News | Goodyear, AZ Economic Development

Skip to Main Content

## Goodyear defense contractor receives prestigious research grant from United States Air Force

### Locally-owned Prime Solutions Group to fine-tune weapons systems performance

Post Date:07/10/2015

FOR IMMEDIATE RELEASE

July 10, 2015

Contact: Mike Sakal, Public Information Officer

Office: [phone]; [email]

#
- Goodyear defense contractor receives prestigious research grant ... (2015-07-10) https://www.goodyearaz.gov/Home/Components/News/News/1569/1127?arch=1&npage=4
  Goodyear defense contractor receives prestigious research grant from United States Air Force | City News | City of Goodyear

Skip to Main Content

## Goodyear defense contractor receives prestigious research grant from United States Air Force

### Locally-owned Prime Solutions Group to fine-tune weapons systems performance

Post Date:07/10/2015

FOR IMMEDIATE RELEASE

July 10, 2015

Contact: Mike Sakal, Public Information Officer

Office: [phone]; [email]

# Goodyear de
- defense solutions group, incorporated (2026-06-15) https://linkedin.com/company/defense-solutions-group-incorporated
  # Defense Solutions Group, Incorporated

Defense Solutions Group, Incorporated is a Manufacturing company. Defense Solutions Group, Incorporated employs 13 people. Headquartered in Fort Worth, Texas, United States.

## Company Details
- Industry: Manufacturing
- Headquarters: Fort Worth, United States
- Aliases: defense solutions group, incorporated
- LinkedIn: linkedin.com/company/defense-solutions-group-incorporated
- Linkedin Followers: 22

## Workforce
- Employees: 13
- Talent Sources: Longh

## Open roles (Blitz, live)
- Program Controls Analyst ({'city': 'Goodyear', 'country_code': 'US'})
- AI/ML Engineer - Computer Vision ({'city': 'Goodyear', 'country_code': 'US'})
- Senior Machine Learning Engineer ({'city': 'Goodyear', 'country_code': 'US'})
- Senior MLOps Engineer ({'city': 'Goodyear', 'country_code': 'US'})
- Senior Machine Learning Research Scientist ({'city': 'Goodyear', 'country_code': 'US'})
- Software Engineer (AI/ML Focus) ({'city': 'Goodyear', 'country_code': 'US'})
- Associate Software Engineer ({'city': 'Goodyear', 'country_code': 'US'})
- Lead DevOps Engineer (AI/ML Ops) ({'city': 'Goodyear', 'country_code': 'US'})
- Senior DevOps Engineer (AI/ML Ops) ({'city': 'Goodyear', 'country_code': 'US'})
- Senior DevOps Engineer (AI/ML Ops) ({'city': 'Goodyear', 'country_code': 'US'})

---
# Context for the scoring agent (do not re-search)

## Barry Hess niche
Defense and intelligence technology companies in the $5M-$10M range whose proven prototype is stuck between demo and fielded deployment, guided by a CTO who fielded a patented tactical Android app platform with operators in theater and now engineers deployment architecture and acquisition advocacy from day one.

**EDP:** When months-to-field exceeds the funded months remaining on the contract, the program dies in the DoD valley of death - and for a $5M-$10M defense tech company, one dead transition is often the company.

## Deployed-reality checklist (score absence against this)
- **DDIL**: denied/disconnected/intermittent/low-bandwidth comms. Canonical example: an AI
  crowd-detection startup streamed 30fps video; the contested environment had bandwidth for
  1 frame every 5 seconds. One architecture change (frame-rate decoupling) saved the project.
- **SWaP-C**: size, weight, power, cost on the platform actually carried/mounted.
- **ATO/RMF**: Authority to Operate on government networks; STIGs, POA&Ms, IL4/IL5.
- **CMMC / NIST 800-171**: company-level compliance before the contract dies.
- **ITAR/export**: controlled data handling.
- **Operator training & sustainment**: who trains the E-5, who fixes it in the field, spares,
  battery logistics, GFE integration.
- **Advocacy chain**: a champion above the operator level; operators loving it is not adoption.
- **Prime/flow-down mechanics**: subcontract structure, government back-end integration.
- **TAK ecosystem**: if it touches situational awareness and never mentions ATAK/TAK, ask why.


## Doctrine
# Targeting doctrine brief (inject into every dossier & company agent — do NOT re-read the corpus)

Distilled from Jordan Crawford's Cannonball/Blueprint corpus, 2026-07-23.

1. **Pain-Qualified Segment beats ICP.** Never target firmographics ("50-200 person SaaS").
   Target companies in an observable painful SITUATION right now. The situation must be
   provable from public data.
2. **Existential Data Point (EDP).** For each niche, name the single number that kills the
   company if the problem stays unfixed (e.g. equipment utilization 60% vs 80% margins;
   fleet utilization <70% = crisis). Every dossier must state the EDP of its CTO's target
   situation: "what does it cost a company, in what metric, to leave this unfixed?"
3. **Jobs are confessions.** A job posting is a company publicly describing its own pain.
   We match jobs whose EXISTENCE signals a problem the CTO has already fixed — not jobs the
   CTO would apply to. "First Head of Engineering" = no technical leadership. "Integration
   Engineer (Epic, HL7)" = drowning in healthcare plumbing.
4. **The message is a redescription of the targeting.** If targeting is precise, the message
   writes itself by describing what we found. The reader should think "how do they know that?"
5. **Message-as-gift (GTM Shift letter standard).** Conversational, unhurried, 4–6 short
   sentences. Opens with a verified surprising fact. Every claim traces to a named public
   source. Valuable even if never answered. One light question, under 15 words. Bans: "I
   noticed", flattery, "hope you're well", meeting asks, personal details about the recipient
   as the opener. Lead with the CTO's expertise matched to the company's evident problem.
6. **Niche = Industry × Situation × Unique Angle** — all three specific (Ben Horowitz
   principle: not best botanist; best Japanese botanist specializing in Zen gardens).
7. **Insight tests**: counterintuitive (would other CTOs argue?), specific (exact tools/
   patterns), quantifiable (before/after numbers), demonstrable (showable in 5 minutes).
8. **Evidence honesty.** Actions > words > database records. Aggregator/API data (Blitz,
   FullEnrich) never corroborates itself — load-bearing claims need the person's or
   company's OWN public artifacts. Date every claim; label estimates as estimates.
9. **Dropping a bad fit is success.** Never pad a list to hit a count.
