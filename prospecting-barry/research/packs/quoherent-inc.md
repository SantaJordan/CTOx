# Evidence pack: Quoherent, Inc.
slug: quoherent-inc | domain: quoherent.com | HQ: Winston Salem, North Carolina US | employees(LI): 14 | founded: 2021 | stage_band: preseed_seed
sources: exa-agent:sbir-graduates-raising-more
industry(LI): Computer Hardware | specialties: 
channel notes: Seed $4.7M seed led by Morpheus Ventures, with Draper Associates, Khosla Ventures, and Alpha Edison participating. dom: ai_ml status: prototype dod: Quoherent received a 2025 Phase II award for edge quantum processing supporting AI-enabled electronic warfare and GPS-denied navigation. hq: Huntsville, AL emp: 11-50 src: https://www.sbir.gov/awards/216876
own-language word count: ~760

## Their own words (website via Exa, livecrawl-preferred)

URL: https://quoherent.com
Quoherent

Quoherent is a cutting-edge startup pioneering the next frontier in quantum computing. Founded with a visionary spirit and a commitment to transforming the landscape of information processing, Quoherent focuses on developing qubits utilizing topological materials that operate at room temperature. Our technology marks a significant departure from conventional quantum computing approaches, enabling the realization of mobile quantum computing solutions.

At Quoherent, we are driven by a passion for innovation and a belief in the transformative power of quantum computing. Our team of dedicated scientists, engineers, and visionaries collaborate to harness the unique properties of topological materials, unlocking the potential for stable and scalable qubits that can be manipulated at room temperature. This breakthrough not only eliminates the need for extreme cooling measures but also propels quantum computing into a realm of unprecedented accessibility and portability.

Our mission is to usher in a new era of quantum computing, where the limitations of traditional methods are replaced by the limitless possibilities afforded by topological materials. We envision a world where quantum computing is not confined to specialized laboratories but empowers individuals and industries to harness the immense computational power of quantum systems on the go.

At the heart of Quoherent is a commitment to pushing the boundaries of what is possible. Our quantum computing solutions aim to redefine the way we process information, solve complex problems, and advance scientific discovery. Through continuous research, development, and collaboration with industry partners, we strive to make quantum computing an integral part of the technological landscape, empowering users to unleash the full potential of quantum information processing.

Join us on this exhilarating journey into the quantum realm. Quoherent is not just a company; it's a catalyst for change, a beacon guiding us towards a future where quantum computing is not only powerful but also accessible, practical, and portable. Welcome to a quantum future with Quoherent – Where You Need It, When You Need It.

URL: https://quoherent.com/
Quoherent

Quoherent is a cutting-edge startup pioneering the next frontier in quantum computing. Founded with a visionary spirit and a commitment to transforming the landscape of information processing, Quoherent focuses on developing qubits utilizing topological materials that operate at room temperature. Our technology marks a significant departure from conventional quantum computing approaches, enabling the realization of mobile quantum computing solutions.

At Quoherent, we are driven by a passion for innovation and a belief in the transformative power of quantum computing. Our team of dedicated scientists, engineers, and visionaries collaborate to harness the unique properties of topological materials, unlocking the potential for stable and scalable qubits that can be manipulated at room temperature. This breakthrough not only eliminates the need for extreme cooling measures but also propels quantum computing into a realm of unprecedented accessibility and portability.

Our mission is to usher in a new era of quantum computing, where the limitations of traditional methods are replaced by the limitless possibilities afforded by topological materials. We envision a world where quantum computing is not confined to specialized laboratories but empowers individuals and industries to harness the immense computational power of quantum systems on the go.

At the heart of Quoherent is a commitment to pushing the boundaries of what is possible. Our quantum computing solutions aim to redefine the way we process information, solve complex problems, and advance scientific discovery. Through continuous research, development, and collaboration with industry partners, we strive to make quantum computing an integral part of the technological landscape, empowering users to unleash the full potential of quantum information processing.

Join us on this exhilarating journey into the quantum realm. Quoherent is not just a company; it's a catalyst for change, a beacon guiding us towards a future where quantum computing is not only powerful but also accessible, practical, and portable. Welcome to a quantum future with Quoherent – Where You Need It, When You Need It.

URL: https://quoherent.com/cart
Quoherent

Quoherent
[
0
](/cart)
[
Skip to Content
](#page)
[
![Quoherent](//images.squarespace-cdn.com/content/v1/62138d4bdbeb655af3a4dd4b/0a292341-3626-4145-9541-1fbeed7e80b8/Picture9.png?format=1500w)
](/)
Open Menu
Close Menu
[
![Quoherent](//images.squarespace-cdn.com/content/v1/62138d4bdbeb655af3a4dd4b/0a292341-3626-4145-9541-1fbeed7e80b8/Picture9.png?format=1500w)
](/)
Open Menu
Close Menu

URL: https://quoherent.com/team
Team — Quoherent

### Meet Our Team

#### Leadership & Technical Team

Sinan Bugu

Senior Quantum Scientist

Daniel Multer

Scientist II

Gabe Marcus

Scientist II

Scott McGuire

Scientist II

Tenyu Aikawa

Scientist I

LinkedIn

Danny Lazega

RF Hardware Engineer

LinkedIn

Brandon Caveney

Quantum Engineer Intern

LinkedIn

Roberto Di Salvo

CEO & Co-Founder

SEE INFO →

Alton Reich

COO & Co-Founder

SEE INFO →

Nicholas Hofgren

CFO

SEE INFO →

David Carroll

CTO & Co-Founder

SEE INFO →

Christopher Jackson

CIO

SEE INFO →

Nate DeFlavio

Corporate Development

SEE INFO →

## LinkedIn about

Quoherent\'s mission is to deploy the world’s first high-fidelity, scalable quantum processors on mobile platforms.

## News (Exa, top 3)
- Award | SBIR () https://www.sbir.gov/awards/217947
  Award | SBIR Skip to main content

# Award

Portfolio Data

Back to Award Search

## Miniaturized Low Frequency Electric Field Sensing for Underwater Applications

### Awardee

#### QUOHERENT, INC.

3100 FRESH WAY SWHUNTSVILLE, AL, 35805-6720USA

Award Year: 2025

UEI: KE8SZ5Q8RNN1

HUBZone Owned: No

Woman Owned: No

Socially and Economically Disadvantaged: No

Congressional District: N/A

Tagged as:

STTR

Phase I

### Awarding Agency

DOD

Branch: NAVY

Total Award Amount: $139,732

Contract 
- Award | SBIR () https://www.sbir.gov/awards/207134
  Award | SBIR Skip to main content

# Award

Portfolio Data

Back to Award Search

## Quantum Material Design

### Awardee

#### QUOHERENT, INC.

3100 FRESH WAY SWHUNTSVILLE, AL, 35805-6720USA

Award Year: 2023

UEI: KE8SZ5Q8RNN1

HUBZone Owned: No

Woman Owned: No

Socially and Economically Disadvantaged: No

Congressional District: N/A

Tagged as:

STTR

Phase I

### Awarding Agency

DOD

Branch: MDA

Total Award Amount: $149,533

Contract Number: HQ0860-23-C-7533

Agency Tracking Number: B22B-
- quoherent (2026-06-29) https://quoherent.com/
  # Quoherent (Quoherent, Inc.)

Quoherent is a Computer Hardware company. Quoherent is a company that provides quantum computing services. Quoherent employs 10 people (+16.7% YoY, +2 people) and has an annual revenue of $1.1M, founded in 2021. Headquartered in Winston Salem, North Carolina, United States. Its workforce is distributed across United States and United Kingdom. Has $4.7M in total funding.

## About

Quoherent's mission is to deploy the world’s first high-fidelity, scalable quantum pr

## Open roles
(no jobs data found)

---
# Context for the scoring agent (do not re-search)

## Barry Hess niche
Defense and intelligence technology companies in the $5M-$10M range whose proven prototype is stuck between demo and fielded deployment, guided by a CTO who fielded a patented tactical Android app platform with operators in theater and now engineers deployment architecture and acquisition advocacy from day one.

**EDP:** When months-to-field exceeds the funded months remaining on the contract, the program dies in the DoD valley of death - and for a $5M-$10M defense tech company, one dead transition is often the company.

## Deployed-reality checklist (score absence against this)
- **DDIL**: denied/disconnected/intermittent/low-bandwidth comms. Canonical example: an AI
  crowd-detection startup streamed 30fps video; the contested environment had bandwidth for
  1 frame every 5 seconds. One architecture change (frame-rate decoupling) saved the project.
- **SWaP-C**: size, weight, power, cost on the platform actually carried/mounted.
- **ATO/RMF**: Authority to Operate on government networks; STIGs, POA&Ms, IL4/IL5.
- **CMMC / NIST 800-171**: company-level compliance before the contract dies.
- **ITAR/export**: controlled data handling.
- **Operator training & sustainment**: who trains the E-5, who fixes it in the field, spares,
  battery logistics, GFE integration.
- **Advocacy chain**: a champion above the operator level; operators loving it is not adoption.
- **Prime/flow-down mechanics**: subcontract structure, government back-end integration.
- **TAK ecosystem**: if it touches situational awareness and never mentions ATAK/TAK, ask why.


## Doctrine
# Targeting doctrine brief (inject into every dossier & company agent — do NOT re-read the corpus)

Distilled from Jordan Crawford's Cannonball/Blueprint corpus, 2026-07-23.

1. **Pain-Qualified Segment beats ICP.** Never target firmographics ("50-200 person SaaS").
   Target companies in an observable painful SITUATION right now. The situation must be
   provable from public data.
2. **Existential Data Point (EDP).** For each niche, name the single number that kills the
   company if the problem stays unfixed (e.g. equipment utilization 60% vs 80% margins;
   fleet utilization <70% = crisis). Every dossier must state the EDP of its CTO's target
   situation: "what does it cost a company, in what metric, to leave this unfixed?"
3. **Jobs are confessions.** A job posting is a company publicly describing its own pain.
   We match jobs whose EXISTENCE signals a problem the CTO has already fixed — not jobs the
   CTO would apply to. "First Head of Engineering" = no technical leadership. "Integration
   Engineer (Epic, HL7)" = drowning in healthcare plumbing.
4. **The message is a redescription of the targeting.** If targeting is precise, the message
   writes itself by describing what we found. The reader should think "how do they know that?"
5. **Message-as-gift (GTM Shift letter standard).** Conversational, unhurried, 4–6 short
   sentences. Opens with a verified surprising fact. Every claim traces to a named public
   source. Valuable even if never answered. One light question, under 15 words. Bans: "I
   noticed", flattery, "hope you're well", meeting asks, personal details about the recipient
   as the opener. Lead with the CTO's expertise matched to the company's evident problem.
6. **Niche = Industry × Situation × Unique Angle** — all three specific (Ben Horowitz
   principle: not best botanist; best Japanese botanist specializing in Zen gardens).
7. **Insight tests**: counterintuitive (would other CTOs argue?), specific (exact tools/
   patterns), quantifiable (before/after numbers), demonstrable (showable in 5 minutes).
8. **Evidence honesty.** Actions > words > database records. Aggregator/API data (Blitz,
   FullEnrich) never corroborates itself — load-bearing claims need the person's or
   company's OWN public artifacts. Date every claim; label estimates as estimates.
9. **Dropping a bad fit is success.** Never pad a list to hit a count.
