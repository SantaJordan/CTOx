# Evidence pack: Seasats
slug: seasats | domain: seasats.com | HQ: San Diego, California US | employees(LI): 66 | founded: 2020 | stage_band: unknown
sources: vc:shield
industry(LI): Maritime Transportation | specialties: 
channel notes: autonomous surface vessels
own-language word count: ~1330

## Their own words (website via Exa, livecrawl-preferred)

URL: https://seasats.com
Seasats | Ocean Autonomy That Works

# Ocean Autonomy That Works

- The coordinates of the ghost fleet.
- The path of hurricanes.
- The location of smuggling and illegal fishing.
- The spread of toxic algal blooms.
- The signals of submarines.
- The status of pipelines.

Ghost fleet

Path of hurricanes

Location - illegal fishing

The spread of toxic algal blooms

Submarine signals

Pipeline status

- Deploy in minutes.
- Detect from miles.
- React at once.

### Deploy in minutes.

#### Deploys by hand

Vessels fit in the back of a pickup truck or trailer. Lightfish can be deployed by hand with just one or two people.

#### Ships anywhere fast

Small enough to be shipped by commercial or military aircraft, anywhere in the world.

#### Mastered with ease

Built simple and intuitive. No specialists or multi-week training required. Make waves without delays.

#### Priced for scale

Affordable enough to buy without extra procurement red tape. Produced at speed, with immediate stock and data-as-a-service available.

### Detect from miles.

#### Captures complete intel

High-resolution data for maritime domain awareness, intelligence and surveillance missions, and subsea mapping.

#### Choose from our library of 50+ payloads, or add yours in hours

A modular payload system allows a library of payloads to plug and play in minutes. Other payloads added in hours or days.

#### Detects up to 6 months

Each vessel is hybrid-fueled for missions up to 6 months. Operational at sea state 6 conditions and during GPS denial.

#### Forms ocean-wide networks

The vessels' low cost permits their use at scale, covering vast geographic areas.

### React at once.

#### Sends real-time alerts

Data alerts delivered straight to the cell or email of key decision makers. Immediate action can be taken.

#### Inspects or intercepts

Cue your sensors or move in for a closer look. Scramble a manned response or Quickfish for rapid interception or inspection.

#### Deploys drones

Built for UAS integrations, each vessel can launch drones to gain a top-down view or for a show of force. Compatible with Group 1 and Group 2 UAS.

#### Creates escort buffers

Quickfish can protect the perimeter of vessels and infrastructure, intercepting and escorting anything that enters their space.

#### Deploys by hand

Vessels fit in the back of a pickup truck or trailer. Lightfish can be deployed by hand with just one or two people.

#### Ships anywhere fast

Small enough to be shipped by commercial or military aircraft, anywhere in the world.

#### Mastered with ease

Built simple and intuitive. No specialists or multi-week training required. Make waves without delays.

#### Priced for scale

Affordable enough to buy without extra procurement red tape. Produced at speed, with immediate stock and data-as-a-service available.

#### Captures complete intel

High-resolution data for maritime domain awareness, intelligence and surveillance missions, and subsea mapping.

#### Choose from our library of 50+ payloads, or add yours in hours

A modular payload system allows a library of payloads to plug and play in minutes. Other payloads added in hours or days.

#### Detects up to 6 months

Each vessel is hybrid-fueled for missions up to 6 months. Operational at sea state 6 conditions and during GPS denial.

#### Forms ocean-wide networks

The vessels' low cost permits their use at scale, covering vast geographic areas.

#### Sends real-time alerts

Data alerts delivered straight to the cell or email of key decision makers. Immediate action can be taken.

#### Inspects or intercepts

Cue your sensors or move in for a closer look. Scramble a manned response or Quickfish for rapid interception or inspection.

#### Deploys drones

Built for UAS integrations, each vessel can launch drones to gain a top-down view or for a show of force. Compatible with Group 1 and Group 2 UAS.

#### Creates escort buffers

Quickfish can protect the perimeter of vessels and infrastructure, intercepting and escorting anything that enters their space.

## Mission after mission

AllCommercialDefenseScience

## Satellites at sea

## Engineered for every need

LightfishQuickfishHeavyfish

### Lightfish

Persistent, Rugged, Portable, Modular

RoleEndurance & general purpose missions

Top Speed5 kts

Weight305 lbs (138 kg)

EnduranceUp to 6 months

Payload Capacity66 lbs (29.9 kg)

Deployment1–2 person manual deployment

Ideal Use CasesSurvey, research, security patrol

Role

Top Speed

Weight

Endurance

Payload Capacity

Deployment

Ideal Use Cases

### Lightfish

Persistent, Rugged, Portable, Modular

Endurance & general purpose missions

5 kts

305 lbs (138 kg)

Up to 6 months

66 lbs (29.9 kg)

1–2 person manual deployment

Survey, research, security patrol

### Quickfish

Fast, Tactical, Reactive, Modular

Tactical & fast-response operations

35+ kts

1,450 lbs (657.7 kg)

Up to 1 month

450 lbs (204 kg)

Trailer or crane (compact trailerable)

Tactical & fast response operations

### Heavyfish

Powerful, Capable, Resilient, ModularExpected Splash Date: 2026

Heavy payload & long-range missions

12 kts

9,000 lbs (4,082 kg)

Up to 6 months

1,000 lbs (453.5 kg)

Vehicle or crane-assisted launch

Security, patrols, survey

## Lightfish

### The vessel, in brief

### Persistent

### Portable

### Modular

### Rugged

### Affordable

### Simple

AIS, GPS, and satellite communications

Marinized solar panels

Electric motor pod

Ultra-efficient composite hull

Single-point lifting eye

Weighted keel for self-righting

Navigation light + additional antennas

x-ray view

Base modelSecurityHydrographicEnvironmental

Mission-ready from the start

The base model is outfitted with standard sensing, navigation and comms: 5 HD cameras, Iridium and Starlink communications, onboard AI for edge processing and target classification. Built modular for easy maintenance and customization. All vessels are controlled with an intuitive UI allowing users to plan, monitor, and update comple

URL: https://www.seasats.com/#products
Seasats | Ocean Autonomy That Works

# Ocean Autonomy That Works

- The coordinates of the ghost fleet.
- The path of hurricanes.
- The location of smuggling and illegal fishing.
- The spread of toxic algal blooms.
- The signals of submarines.
- The status of pipelines.

Ghost fleet

Path of hurricanes

Location - illegal fishing

The spread of toxic algal blooms

Submarine signals

Pipeline status

No matter the unknown, we will find it.

- Deploy in minutes.
- Detect from miles.
- React at once.

### Deploy in minutes.

#### Deploys by hand

Vessels fit in the back of a pickup truck or trailer. Lightfish can be deployed by hand with just one or two people.

#### Ships anywhere fast

Small enough to be shipped by commercial or military aircraft, anywhere in the world.

#### Mastered with ease

Built simple and intuitive. No specialists or multi-week training required. Make waves without delays.

#### Priced for scale

Affordable enough to buy without extra procurement red tape. Produced at speed, with immediate stock and data-as-a-service available.

### Detect from miles.

#### Captures complete intel

High-resolution data for maritime domain awareness, intelligence and surveillance missions, and subsea mapping.

#### Choose from our library of 50+ payloads, or add yours in hours

A modular payload system allows a library of payloads to plug and play in minutes. Other payloads added in hours or days.

#### Detects up to 6 months

Each vessel is hybrid-fueled for missions up to 6 months. Operational at sea state 6 conditions and during GPS denial.

#### Forms ocean-wide networks

The vessels' low cost permits their use at scale, covering vast geographic areas.

### React at once.

#### Sends real-time alerts

Data alerts delivered straight to the cell or email of key decision makers. Immediate action can be taken.

#### Inspects or intercepts

Cue your sensors or move in for a closer look. Scramble a manned response or Quickfish for rapid interception or inspection.

#### Deploys drones

Built for UAS integrations, each vessel can launch drones to gain a top-down view or for a show of force. Compatible with Group 1 and Group 2 UAS.

#### Creates escort buffers

Quickfish can protect the perimeter of vessels and infrastructure, intercepting and escorting anything that enters their space.

#### Deploys by hand

Vessels fit in the back of a pickup truck or trailer. Lightfish can be deployed by hand with just one or two people.

#### Ships anywhere fast

Smal

## LinkedIn about

Seasats builds long-endurance autonomous surface vessels for defense, commercial, and scientific missions. By combining commercial manufacturing agility with defense-grade reliability, Seasats delivers small, cost-effective unmanned systems that extend maritime reach, enhance situational awareness, and reduce risk to personnel.



## News (Exa, top 3)
- Seasats Awarded $24M in Department of War APFIT Funding to Accelerate Fielding of its Autonomous Surface Vessels (2026-01-15) https://www.prnewswire.com/news-releases/seasats-awarded-24m-in-department-of-war-apfit-funding-to-accelerate-fielding-of-its-autonomous-surface-vessels-302662524.html
  Seasats Awarded $24M in Department of War APFIT Funding to Accelerate Fielding of its Autonomous Surface Vessels Accessibility Statement Skip Navigation

SAN DIEGO, Jan. 15, 2026 /PRNewswire/ -- Seasats, a leading developer of long-endurance autonomous surface vessels (ASVs), today announced that it was selected by the Department of War (DoW) to receive an award under the Accelerate the Procurement and Fielding of Innovative Technologies (APFIT) program. The $24M award, made by recommendation of
- Seasats awarded $24M in FY-26 APFIT funding | InsideDefense.com () https://insidedefense.com/insider/seasats-awarded-24m-fy-26-apfit-funding
  Seasats awarded $24M in FY-26 APFIT funding | InsideDefense.com

Skip to main content

# Seasats awarded $24M in FY-26 APFIT funding

Uncrewed surface vessel company Seasats today revealed it is the previously unnamed contractor behind a $24 million project awarded under the Accelerate the Procurement and Fielding of Innovative Technologies program -- part of the first series of project awards under the initiative for fiscal year 2026.

The award -- for which the Navy and Marine Corps recommende
- Seasats Discloses $24 Million APFIT Award For Lightfish ASVs - Defense Daily (2026-01-15) https://www.defensedaily.com/seasats-discloses-24-million-apfit-award-for-lightfish-asvs/navy-usmc/
  Seasats Discloses $24 Million APFIT Award For Lightfish ASVs - Defense Daily

:

Search

- Space
- Army
- Navy/USMC
- Air Force
- Business/Financial
- Commentary

Seasats' Lightfish autonomous surface vessel. (Photo: Seasats)

Seasats on Thursday said it has received a $24 million award from the Defense Department to accelerate and scale production of the Lightfish autonomous surveillance vessel. The award was part of more than $400 million in Accelerate the Procurement and Fielding of Innovativ

## Open roles (Blitz, live)
- Mechanical Engineering Intern ({'city': 'San Diego', 'country_code': 'US'})
- Production Test Technician ({'city': 'San Diego', 'country_code': 'US'})
- Fabricator ({'city': 'San Diego', 'country_code': 'US'})
- Marketing and Communications Intern ({'city': 'San Diego', 'country_code': 'US'})
- Senior Front End Software Engineer ({'city': 'San Diego', 'country_code': 'US'})
- ASV Watch Floor Manager ({'city': 'San Diego', 'country_code': 'US'})
- Head of Electronics Engineering ({'city': 'San Diego', 'country_code': 'US'})
- Head of Software ({'city': 'San Diego', 'country_code': 'US'})
- Payload Integration Engineer ({'city': 'San Diego', 'country_code': 'US'})
- Autonomous Surface Vessel (ASV) Pilot ({'city': 'San Diego', 'country_code': 'US'})

---
# Context for the scoring agent (do not re-search)

## Barry Hess niche
Defense and intelligence technology companies in the $5M-$10M range whose proven prototype is stuck between demo and fielded deployment, guided by a CTO who fielded a patented tactical Android app platform with operators in theater and now engineers deployment architecture and acquisition advocacy from day one.

**EDP:** When months-to-field exceeds the funded months remaining on the contract, the program dies in the DoD valley of death - and for a $5M-$10M defense tech company, one dead transition is often the company.

## Deployed-reality checklist (score absence against this)
- **DDIL**: denied/disconnected/intermittent/low-bandwidth comms. Canonical example: an AI
  crowd-detection startup streamed 30fps video; the contested environment had bandwidth for
  1 frame every 5 seconds. One architecture change (frame-rate decoupling) saved the project.
- **SWaP-C**: size, weight, power, cost on the platform actually carried/mounted.
- **ATO/RMF**: Authority to Operate on government networks; STIGs, POA&Ms, IL4/IL5.
- **CMMC / NIST 800-171**: company-level compliance before the contract dies.
- **ITAR/export**: controlled data handling.
- **Operator training & sustainment**: who trains the E-5, who fixes it in the field, spares,
  battery logistics, GFE integration.
- **Advocacy chain**: a champion above the operator level; operators loving it is not adoption.
- **Prime/flow-down mechanics**: subcontract structure, government back-end integration.
- **TAK ecosystem**: if it touches situational awareness and never mentions ATAK/TAK, ask why.


## Doctrine
# Targeting doctrine brief (inject into every dossier & company agent — do NOT re-read the corpus)

Distilled from Jordan Crawford's Cannonball/Blueprint corpus, 2026-07-23.

1. **Pain-Qualified Segment beats ICP.** Never target firmographics ("50-200 person SaaS").
   Target companies in an observable painful SITUATION right now. The situation must be
   provable from public data.
2. **Existential Data Point (EDP).** For each niche, name the single number that kills the
   company if the problem stays unfixed (e.g. equipment utilization 60% vs 80% margins;
   fleet utilization <70% = crisis). Every dossier must state the EDP of its CTO's target
   situation: "what does it cost a company, in what metric, to leave this unfixed?"
3. **Jobs are confessions.** A job posting is a company publicly describing its own pain.
   We match jobs whose EXISTENCE signals a problem the CTO has already fixed — not jobs the
   CTO would apply to. "First Head of Engineering" = no technical leadership. "Integration
   Engineer (Epic, HL7)" = drowning in healthcare plumbing.
4. **The message is a redescription of the targeting.** If targeting is precise, the message
   writes itself by describing what we found. The reader should think "how do they know that?"
5. **Message-as-gift (GTM Shift letter standard).** Conversational, unhurried, 4–6 short
   sentences. Opens with a verified surprising fact. Every claim traces to a named public
   source. Valuable even if never answered. One light question, under 15 words. Bans: "I
   noticed", flattery, "hope you're well", meeting asks, personal details about the recipient
   as the opener. Lead with the CTO's expertise matched to the company's evident problem.
6. **Niche = Industry × Situation × Unique Angle** — all three specific (Ben Horowitz
   principle: not best botanist; best Japanese botanist specializing in Zen gardens).
7. **Insight tests**: counterintuitive (would other CTOs argue?), specific (exact tools/
   patterns), quantifiable (before/after numbers), demonstrable (showable in 5 minutes).
8. **Evidence honesty.** Actions > words > database records. Aggregator/API data (Blitz,
   FullEnrich) never corroborates itself — load-bearing claims need the person's or
   company's OWN public artifacts. Date every claim; label estimates as estimates.
9. **Dropping a bad fit is success.** Never pad a list to hit a count.
