# Evidence pack: Starfish Space
slug: starfish-space | domain: starfishspace.com | HQ: Tukwila, Washington US | employees(LI): 99 | founded: 2019 | stage_band: unknown
sources: vc:shield|vc:booz-ventures
industry(LI): Space Research and Technology | specialties: 
channel notes: satellite servicing spacecraft || satellite servicing vehicles
own-language word count: ~1279

## Their own words (website via Exa, livecrawl-preferred)

URL: https://starfishspace.com
Starfish Space

Rotate device

✕

# Say hello to Otter, your satellite’s new best friend

We’re unlocking affordable and available satellite servicing. It starts with Otter.

Learn more

## The age of satellite servicing is starting now.

As the number of satellites continues to grow on orbit, satellite servicing is critical for maximizing the value of space activities.

# Otter is an autonomous satellite servicing vehicle.

10x smaller, and significantly less expensive than alternatives, our Otter uses exclusively electric propulsion to dock with satellites. Otter’s first assignments will be satellite life extension and disposal.

More about Otter

# We are actively validating our technology on orbit.

Our Otter Pup 2 mission, launching summer 2025, aims to conduct rendezvous, proximity operations, and docking (RPOD) with another spacecraft in low Earth orbit—marking the first-ever commercial satellite docking in LEO.

See our latest news

window.dataLayer = window.dataLayer || []; function gtag(){dataLayer.push(arguments);} gtag('js', new Date()); gtag('config', 'G-M05YWDG65G'); >

URL: https://www.starfishspace.com/the-otter/
The otter – Starfish Space

Rotate device

# Otter is there, on call and ready to support.

It’s the first of its kind: a small and affordable servicing vehicle that can perform satellite life extension and disposal.

#### Satellite Life Extension

Running out of fuel? Our Otter will dock with your satellite and, like a jetpack, use its own fuel to keep your satellite operational in Geostationary orbit.

#### Satellite Disposal

Otter reliably and safely disposes defunct satellites that pose a collision risk to other satellites, and take up valuable orbital slots.

# Our technology makes Otter one-of-a-kind.

Groundbreaking guidance and control software. Binocular computer vision for navigation. A docking device that adheres to virtually any flat surface. Our innovative technologies enable autonomous Rendezvous, Proximity Operations, and Docking (RPOD) missions, and can work with any flight software stack.

#### Cetacean

##### OUR NAVIGATION SOFTWARE

1/3

Otter’s eyes. Our computer vision navigation software integrates with off-the-shelf sensors to provide accurate relative position, velocity, and attitude information, allowing Otter to safely navigate to its destination: your satellite.

#### Cephalopod

##### OUR GUIDANCE AND CONTROL SOFTWARE

2/3

Our game changer. An autonomous guidance and control software, with critical built-in safety features, that enables complex RPOD missions. Great things do come in small packages.

#### Nautilus

##### OUR DOCKING DEVICE

3/3

How Otter attaches to your satellite. A reusable system that works on virtually any flat surface and in any orbit. No need to add a docking interface to your satellite – Nautilus will dock with it even if it was never designed for docking.

#### Our vision for Otter and beyond

Otter specializes in satellite life extension and disposal, but this is just the beginning. We envision our technology having far-reaching implications and massive impact: performing in-space inspection, orbital relocation, and logistics services, as well as conducting repairs, component upgrades, and in-space assembly and manufacturing.

#### Our vision for Otter and beyond

Otter specializes in satellite life extension and disposal, but this is just the beginning. We envision our technology having far-reaching implications and massive impact: performing in-space inspection, orbital relocation, and logistics services, as well as conducting repairs, component upgrades, and in-space assembly and manufacturing.

wi

URL: https://www.starfishspace.com/company/
Company – Starfish Space

Rotate device

✕

# We are building the infrastructure for humanity’s expansion into space.

Since our founding in 2019, our mission has been clear: to enable affordable, available satellite servicing. We believe humans are meant to expand throughout the universe; our novel technology will help us autonomously build the future off-world economy.

## We are an execution-first team.

At Starfish Space, our story is rewritten every day by a team of some of the most talented humans on the planet, though our passion lies decidedly off-planet. We test early and often and are committed to building a presence in space that is affordable and available. This vision started with founders Austin Link and Trevor Bennett, but is carried forward by every person that makes up the Starfish Space team.

## Meet our team members

##### Austin Link

Co-founder Aerospace Engineering, Purdue Physics, Stanford

##### Trevor Bennett

Co-founder PhD Aerospace Engineering University of Colorado Boulder

##### Adam Herrmann

Flight Algorithms PhD Aerospace Engineering Sciences University of Colorado, Boulder

##### Aldo Siananta

Technician Engineering Arizona State University

##### Alex Coultrup

Business Development Human Factors and Space Policy Florida Institute of Technology

##### Alex Gonring

Flight Algorithms Aerospace Engineering Purdue

##### Alex Herbets

Business Development Electrical Engineering, MBA Georgia Tech, Chicago Booth

##### Amar Bhatt

Project Execution MS Engineering Management Arizona State University

##### Alyssa Jones

Operations Manager Deaf Education Texas Women’s University

##### Amelia Dumovic

Flight Algorithms Electrical Engineering University of Washington

##### Andrew Gerth

Satellite Systems MS Astronautical Engineering University of Southern California

##### Andy Wilcox

Flight Algorithms Aerospace Engineering Purdue

##### Anik Joshi

Project Execution Economics & Political Science Rutgers University

##### Anna Miller

Space Robotics Civil Engineering, UAA Mechanical Engineering, Dartmouth

##### Ari Juster

Business Operations Economics Northwestern

##### Arthur Klinkefus

Technician U.S. Navy

##### Asher Smith

Flight Algorithms MS Aerospace Engineering Embry-Riddle

##### Augie Collins

Satellite Systems Mechanical Engineering University of Washington

##### Azalea Andrews

Satellite Systems Multidisciplinary Engineering Technology Texas A&M

##### Ben Ponto

Mission Manager PhD Chemical Engineering Univer

URL: https://www.starfishspace.com/missions/
Missions – Starfish Space

Rotate device

✕

### The latest at Starfish

# The Otter Pup 2 Mission

Building on the heritage of the Otter Pup 1 mission, Otter Pup 2 will conduct rendezvous, proximity operations, and ultimately attempt docking with another spacecraft, an unprepared commercial satellite in low Earth orbit (LEO).

Come along and join us in the exciting journey of Otter Pup 2! Follow us on X

## Starfish in the News

#### SDA taps Starfish for first-of-its-kind satellite disposal service

Breaking Defense January 22, 2026

#### Two space startups prove you don’t need to break the bank to rendezvous in space

Ars Technica December 19, 2025

#### Starfish Space ready to launch docking demonstration mission

SpaceNews May 20, 2025

#### Starfish Space raises $29M to launch satellite-servicing spacecraft missions

TechCrunch November 13, 2024

#### NRO announces space tech agreements with three commercial companies

SpaceNews October 30, 2024

#### NASA funds Starfish Space debris inspection mission

SpaceNews September 27, 2024

#### Starfish spacecraft will extend the life of an expensive GEO satellite in 2026 mission

TechCrunch June 26, 2024

#### New heroes of spaceflight: Not the astronauts but the software nerds

Washington Post June 11, 2024

#### Starfish Space lands $37.5 million Space Force contract for on-orbit servicing vehicle

SpaceNews May 20, 2024

#### Starfish Space and D-Orbit complete orbital rendezvous, bringing Otter Pup mission to a close

TechCrunch May 7, 2024

#### Starfish Space scrubs plan for a satellite rendezvous after thruster suffers anomaly

GeekWire November 10, 2023

#### Starfish Space wins NASA contract to plan demonstration of orbital debris inspection

GeekWire October 3, 2023

#### Starfish Space pulls Otter Pup servicing vehicle back from ‘the brink of death’

TechCrunch August 29, 2023

#### Spin control: Starfish Space makes use of magnetism to stop satellite’s death spiral

GeekWire August 29, 2023

#### Starfish Space wins $1.8M to keep working on satellite guidance system for Air Force

GeekWire August 8, 2023

#### SpaceX launch gives a big sendoff to Starfish Space’s satellite docking craft

GeekWire June 12, 2023

#### Starfish Space captures new funding ahead of orbital servicing demo mission

TechCrunch March 8, 2023

#### Starfish Space raises $14M to advance development of satellite servicing vehicles

GeekWire March 8, 2023

#### Starfish books launch for in-orbit satellite docking mission ne

## LinkedIn about

Starfish Space is building the infrastructure for humanity\'s expansion into Space. Our Otter servicing vehicle maximizes satellite capabilities through life extension and end of life disposal missions.

## SBIR/STTR record (their own abstracts)

### Developing CETACEAN computer vision-based relative navigation system for RPOD using synthetic data pipeline — Air Force 2023 ($1,700,000)
For this proposal effort, Starfish Space, developer of the Otter satellite servicing vehicle including the CETACEAN relative navigation system, is partnering with Lexset, a leading innovator in synthetic imagery generation for computer vision applications

## News (Exa, top 3)
- Starfish Space Awarded $54.5 Million Space Force ... (2026-02-07) https://www.starfishspace.com/press-release/starfish-space-awarded-54-5-million-space-force-contract-for-dedicated-otter-satellite-servicing-vehicle/
  Starfish Space Awarded $54.5 Million Space Force Contract for Dedicated Otter Satellite Servicing Vehicle – Starfish Space

Rotate device

## Starfish Space Awarded $54.5 Million Space Force Contract for Dedicated Otter Satellite Servicing Vehicle

February 7, 2026

The contract, issued under the APFIT program, acquires an Otter for the U.S. Space Force, expanding the Service’s augmented maneuver capabilities

SEATTLE, Washington — [February 7, 2026] — Starfish Space today announced it has been 
- Space Force awards $54.5 million contract to Starfish ... (2026-02-07) https://spacenews.com/space-force-awards-54-5-million-contract-to-starfish-space-for-geo-servicing-vehicle/
  Space Force awards $54.5 million contract to Starfish Space for GEO servicing vehicle - SpaceNews

Home/ Space Force awards $54.5 million contract to Starfish Space for GEO servicing vehicle

Illustration of the Otter vehicle in geosynchronous Earth orbit. Credit: Starfish Space

WASHINGTON — The U.S. Space Force has awarded a $54.5 million contract to Starfish Space to build and operate a spacecraft designed to support military satellites in geostationary Earth orbit.

The contract, awarded on 
- Starfish Space Awarded First Ever End-of-Life Disposal ... (2026-01-21) https://www.starfishspace.com/press-release/starfish-space-awarded-first-ever-end-of-life-disposal-contract-for-a-leo-constellation/
  Starfish Space Awarded First Ever End-of-Life Disposal Contract for a LEO Constellation – Starfish Space

Rotate device

## Starfish Space Awarded First Ever End-of-Life Disposal Contract for a LEO Constellation

January 21, 2026

U.S. Space Force contracted Starfish to provide Deorbit-as-a-Service for the Proliferated Warfighter Space Architecture, marking a historical first for satellite servicing

Seattle, Washington — [January 21, 2026] — Starfish Space today announced it has been awarded a 

## Open roles (Blitz, live)
- GNC Engineer ({'city': 'Seattle', 'country_code': 'US'})

---
# Context for the scoring agent (do not re-search)

## Barry Hess niche
Defense and intelligence technology companies in the $5M-$10M range whose proven prototype is stuck between demo and fielded deployment, guided by a CTO who fielded a patented tactical Android app platform with operators in theater and now engineers deployment architecture and acquisition advocacy from day one.

**EDP:** When months-to-field exceeds the funded months remaining on the contract, the program dies in the DoD valley of death - and for a $5M-$10M defense tech company, one dead transition is often the company.

## Deployed-reality checklist (score absence against this)
- **DDIL**: denied/disconnected/intermittent/low-bandwidth comms. Canonical example: an AI
  crowd-detection startup streamed 30fps video; the contested environment had bandwidth for
  1 frame every 5 seconds. One architecture change (frame-rate decoupling) saved the project.
- **SWaP-C**: size, weight, power, cost on the platform actually carried/mounted.
- **ATO/RMF**: Authority to Operate on government networks; STIGs, POA&Ms, IL4/IL5.
- **CMMC / NIST 800-171**: company-level compliance before the contract dies.
- **ITAR/export**: controlled data handling.
- **Operator training & sustainment**: who trains the E-5, who fixes it in the field, spares,
  battery logistics, GFE integration.
- **Advocacy chain**: a champion above the operator level; operators loving it is not adoption.
- **Prime/flow-down mechanics**: subcontract structure, government back-end integration.
- **TAK ecosystem**: if it touches situational awareness and never mentions ATAK/TAK, ask why.


## Doctrine
# Targeting doctrine brief (inject into every dossier & company agent — do NOT re-read the corpus)

Distilled from Jordan Crawford's Cannonball/Blueprint corpus, 2026-07-23.

1. **Pain-Qualified Segment beats ICP.** Never target firmographics ("50-200 person SaaS").
   Target companies in an observable painful SITUATION right now. The situation must be
   provable from public data.
2. **Existential Data Point (EDP).** For each niche, name the single number that kills the
   company if the problem stays unfixed (e.g. equipment utilization 60% vs 80% margins;
   fleet utilization <70% = crisis). Every dossier must state the EDP of its CTO's target
   situation: "what does it cost a company, in what metric, to leave this unfixed?"
3. **Jobs are confessions.** A job posting is a company publicly describing its own pain.
   We match jobs whose EXISTENCE signals a problem the CTO has already fixed — not jobs the
   CTO would apply to. "First Head of Engineering" = no technical leadership. "Integration
   Engineer (Epic, HL7)" = drowning in healthcare plumbing.
4. **The message is a redescription of the targeting.** If targeting is precise, the message
   writes itself by describing what we found. The reader should think "how do they know that?"
5. **Message-as-gift (GTM Shift letter standard).** Conversational, unhurried, 4–6 short
   sentences. Opens with a verified surprising fact. Every claim traces to a named public
   source. Valuable even if never answered. One light question, under 15 words. Bans: "I
   noticed", flattery, "hope you're well", meeting asks, personal details about the recipient
   as the opener. Lead with the CTO's expertise matched to the company's evident problem.
6. **Niche = Industry × Situation × Unique Angle** — all three specific (Ben Horowitz
   principle: not best botanist; best Japanese botanist specializing in Zen gardens).
7. **Insight tests**: counterintuitive (would other CTOs argue?), specific (exact tools/
   patterns), quantifiable (before/after numbers), demonstrable (showable in 5 minutes).
8. **Evidence honesty.** Actions > words > database records. Aggregator/API data (Blitz,
   FullEnrich) never corroborates itself — load-bearing claims need the person's or
   company's OWN public artifacts. Date every claim; label estimates as estimates.
9. **Dropping a bad fit is success.** Never pad a list to hit a count.
