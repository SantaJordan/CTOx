# Evidence pack: Super Powers Mobility
slug: super-powers-mobility | domain: spm.fun | HQ: Pittsburgh,  US | employees(LI): 5 | founded:  | stage_band: unknown
sources: vc:harpoon
industry(LI): Motor Vehicle Manufacturing | specialties: embedded controls, high voltage batteries, high voltage motors, system integration, powertrain development
channel notes: high-performance electric UTVs
own-language word count: ~1332

## Their own words (website via Exa, livecrawl-preferred)

URL: https://spm.fun
SPM – Super Powers Mobility Inc

## HIGH PERFORMANCE ELECTRIC UTVs

Scalable power options. Built for any platform.

## designed to outperform traditional drivetrains.

#### 100% ELECTRIC

#### HYBRID OPTIONAL

#### CHARGE ANYWHERE

#### MADE IN AMERICA

## The EPEX

Turn key, high performance electric UTV

## VEHICLE FEATURES

### regenerative braking

Adjustable "engine braking" feel when off the throttle pedal.

### instant torque

Like you've never felt before.

### high power density lithium battery

Utilizing the latest battery technology.

### off-road performance

Turn key and ready to rip when you order an EPEX.

### fast-charge compatible

At home or on the go using existing EV charging stations.

## Configure Your ride

Select a do it yourself conversion kit, or a complete turn key vehicle.

pick your adventure

#### 1. PRE-ORDER

#### 2. BUILD WITH US

#### 3. RIDE OF A LIFETIME

- Order a turn key vehicle or DIY kit with a refundable deposit.
- We'll keep you updated along the way as we create your dream vehicle.
- Enjoy the future as you take a ride in your new electric UTV.

© 2026 Super Powers Mobility Inc. Powered by Shopify.

URL: https://spm.fun/products/evk
EVK - Electric UTV Conversion Kit – Drop-in for RZR – Super Powers Mobility Inc

#### drop-in

No fabrication required.

#### RIDE ALL DAY

Quick charge times.

#### built for you

# SPM EVK - Pre-order

Deposit FEE - SAVE YOUR SPOT

$100.00

$100.00 Save $-100.00 (%)

Shipping calculated at checkout.

Delivers Q2 2027

10 kWh kit sale price - $30,000

20 kWh kit sale price - $35,000

Alpha Deliveries Q3 2026

Inquire to become an Alpha at [email]

Select Battery Capacity

10 kWh - 25 mile range

20 kWh - 50 mile range

Quantity must be 1 or more

Add to cart

Go to cart

Quantity must be 1 or more

Add to cart

Go to cart

Non-refundable pre-order deposit.

Experience the newest innovation in UTV performance - the EVK. SPM's EV-powertrain kit (EVK) can convert any UTV with a CVT to a high performance electric monster from the future.

Currently compatible with the Polaris Rzr family, the EVK is a drop-in, zero fabrication, powertrain kit. Future models will be compatible with Can-Am, Yamaha and Kawasaki vehicles.

What's Included in the kit:

- Motor
- Battery Pack
- Inverter (Motor Controller)
- Vehicle Control Unit
- Combined Charger and DCDC Converter
- Mounting Brackets (motor, battery, high voltage devices)
- Low Voltage Wiring Harness
- High Voltage Distribution Cables
- Thermal System
- Digital Dash Display
- Lightweight Lithium Ion 12V Battery - NOCO
- Drive Selector (Park, Reverse, Neutral)

## American made

Built in Pittsburgh, Pennsylvania

## 100% Electric

The ultimate in innovation and performance.

## high performance

Instant torque and control.

### pre-order information

We'll contact you once your order has been placed to discuss your build. Fully refundable deposit minus card processing fee.

Our tentative goal is to deliver EVK in Q2 of 2026. We are developing the kit and partnering with suppliers to ensure future orders can be fulfilled in the shortest time possible.

$100.00per Deposit

- Share on Facebook
- Tweet on Twitter
- Pin on Pinterest

## Technical Specifications

The SPM EV Kit is a drop-in electric powertrain conversion kit for UTVs. It replaces the internal combustion engine with a high-performance electric motor and battery system, providing instant torque and exceptional performance.

### powertrain

| Motor | Brushles axial-flux, liquid cooled |
| --- | --- |
| Power | 165 horsepower / 120 kW |
| Motor Torque | 162 lb-ft / 220 Nm |
| Motor Weight | 35 lbs (yes that's right) |
| Drivetrain | CVT replaced by Poly

URL: https://spm.fun/products/epex
EPEX: High Performance Electric UTV (165 HP) – Super Powers Mobility Inc

#### TURN KEY, HIGH PERFORMANCE

#### RIDE ALL DAY

#### custom built

# SPM EPEX - Pre-order

Deposit FEE - SAVE YOUR SPOT

$100.00

$100.00 Save $-100.00 (%)

Shipping calculated at checkout.

Delivers Q2 2027

Prices depend on level of customization from donor vehicle starting point

10 kWh (25 mile range) EPEX starting at $40,000

20 kWh (50 mile range) EPEX starting at $45,000

Alpha Deliveries Q3 2026

Inquire to become an Alpha at [email]

Select Battery Capacity

10 kWh - 25 mile range

20 kWh - 50 mile range

Quantity must be 1 or more

Add to cart

Go to cart

Quantity must be 1 or more

Add to cart

Go to cart

Non-refundable pre-order deposit.

Experience the EPIC APEX of UTV performance - the EPEX. Equipped with a powertrain from the future and industry-leading protection and suspension components, the SPM EPEX fulfills both function and form.

Based on the versatile and capable Polaris Rzr chassis, the SPM EPEX delivers performance that you didn't know you needed, because until now it was not possible.

## American made

Built in Pittsburgh, Pennsylvania

## 100% Electric

The ultimate in innovation and performance.

## high performance

Instant torque and control.

### pre-order information

Final price dependent on level of customization, battery size and base vehicle choice.

We'll contact you once your order has been placed to discuss your build. Fully refundable deposit minus card processing fee.

Our tentative goal is to deliver EPEX in Q2 of 2026. We are developing the kit and partnering with suppliers to ensure future orders can be fulfilled in the shortest time possible.

$100.00per Deposit

- Share on Facebook
- Tweet on Twitter
- Pin on Pinterest

## Technical Specifications

The SPM EPEX is a high-performance electric UTV designed for both recreational and competitive use. Below are the key specifications and features of the vehicle.

### Performance

| Powertrain | SPM EVK |
| --- | --- |
| Power | 165 horsepower / 120 kW |
| Motor Torque | 162 lb-ft / 220 Nm |
| Powertrain Voltage | 470V max, 400V nominal |
| Battery Capacity | 10 kWh, 20 kWh, or more, Lithium Ion |
| Battery Weight | 183 lbs |
| Est Range (10 kWh) | 24-33 miles |
| Est Range (20 kWh) | 50-60 miles |
| Vehicle Weight | 1,500 lbs |
| Charging | J1772 110V or 220V |

### Dimensions & Components

| Base Vehicle | RZR XP1000 |
| --- | --- |
| Track Width | 65 inches |
| Wheelbase | 90.5 

URL: https://spm.fun/collections/vehicle-products
Vehicle Products – Super Powers Mobility Inc

# EXPLORE OUR ELECTRIC VEHICLES

EXPLORE OUR ELECTRIC VEHICLES

## EPEX

TURN KEY ELECTRIC VEHICLE

Our flagship model and what brought SPM to life.

- Demo Ride!
- High performance electric UTV
- Ride all day, charge in your garage overnight
- Custom built to your specifications

## EVK

INSTALL IT YOURSELF KIT

For those who are mechanically inclined and enjoy the process of building. The EVK is the same drivetrain, battery, and electronics used in our EPEX turn key vehicle.

- Drop-in universal EV-KIT
- No fabrication required
- 165 HP, 162 LB-FT Torque
- Based on the Polaris RZR XP1000

## VEHICLES AND KITS

Currently offered turn key vehicles and install yourself kits for the platforms we love most.

$00 Product Title 3 colors

$00 Product Title 3 colors

$00 Product Title 3 colors

### SPM EVK - Pre-order

$100.00 $100.00 Save $-100.00 (%) in your cart

Quickshop

$100.00

$100.00 Save $-100.00 (%)

SPM EVK - Pre-order

in your cart

Quickshop Quantity must be 1 or more

### SPM EPEX - Pre-order

$100.00 $100.00 Save $-100.00 (%) in your cart

Quickshop

$100.00

$100.00 Save $-100.00 (%)

SPM EPEX - Pre-order

in your cart

Quickshop Quantity must be 1 or more

$00 Product Title 3 colors

$00 Product Title 3 colors

$00 Product Title 3 colors

## don't see what you need?

We’re currently engineering electric systems for select vehicle platforms. Have a platform in mind? Let’s talk about bringing it to life — get in touch through our contact page.

Electrify My Vehicle!

© 2026 Super Powers Mobility Inc. Powered by Shopify.

## LinkedIn about

SPM is building the highest performance electric UTVs in Pittsburgh Pennsylvania. We are a full-stack high voltage off-road powertrain development company.

## News (Exa, top 3)
- Pittsburgh startup pitches EV kit to elite troops (2026-07-18) https://defence-blog.com/pittsburgh-startup-pitches-ev-kit-to-elite-troops/
  # Pittsburgh startup pitches EV kit to elite troops

Published: 2026-07-18T03:36:20-05:00
Source: defence-blog.com (defence-blog.com)
Language: en

## Story

Pittsburgh startup pitches EV kit to elite troops

[Facebook](https://www.facebook.com/defencenewsblog/) [Patreon](https://www.patreon.com/defenceblog) [X](https://twitter.com/defence_blog) [Youtube](https://www.youtube.com/channel/UCh9ODc2DO4VYZIQHHvCg4Aw) [Bluesky](https://bsky.app/profile/defence-blog.com)

- [Home](https://defence-blog.
- Why Special Operations Forces Want This New Pittsburgh Elect — Fightcancer (2026-07-18) https://m.fightcancer.org/special-operations-forces-want-new-pittsburgh-electric-vehicle-kit
  Why Special Operations Forces Want This New Pittsburgh Elect — Fightcancer

Military operations depend heavily on stealth, but standard utility terrain vehicles sound like chainsaws tearing through the woods. When elite troops need to slip behind enemy lines unnoticed, a loud internal combustion engine is a massive liability. That is exactly the problem a Pittsburgh startup called Super Powers Mobility is tackling with its new hardware. Known as SPM, the company recently demonstrated its new sys
- Partnering with Super Powers Mobility to Accelerate Dual-Use Military/Civilian Electric Off-Road Powertrains | Uma Ventures (2025-07-21) https://www.umaventures.com/2025/07/21/uma-ventures-invests-in-super-powers-mobility-electric-off-road-powertrains/
  Partnering with Super Powers Mobility to Accelerate Dual-Use Military/Civilian Electric Off-Road Powertrains | Uma Ventures

Uma Ventures is thrilled to announce our investment in Super Powers Mobility (SPM), a pioneering startup developing the most reliable electric off-road vehicles for both military and civilian applications.

SPM is redefining the off-road vehicle industry with its bespoke electric powertrain solutions, purpose-built for utility terrain vehicles (UTVs) and scalable for use o

## Open roles
(no jobs data found)

---
# Context for the scoring agent (do not re-search)

## Barry Hess niche
Defense and intelligence technology companies in the $5M-$10M range whose proven prototype is stuck between demo and fielded deployment, guided by a CTO who fielded a patented tactical Android app platform with operators in theater and now engineers deployment architecture and acquisition advocacy from day one.

**EDP:** When months-to-field exceeds the funded months remaining on the contract, the program dies in the DoD valley of death - and for a $5M-$10M defense tech company, one dead transition is often the company.

## Deployed-reality checklist (score absence against this)
- **DDIL**: denied/disconnected/intermittent/low-bandwidth comms. Canonical example: an AI
  crowd-detection startup streamed 30fps video; the contested environment had bandwidth for
  1 frame every 5 seconds. One architecture change (frame-rate decoupling) saved the project.
- **SWaP-C**: size, weight, power, cost on the platform actually carried/mounted.
- **ATO/RMF**: Authority to Operate on government networks; STIGs, POA&Ms, IL4/IL5.
- **CMMC / NIST 800-171**: company-level compliance before the contract dies.
- **ITAR/export**: controlled data handling.
- **Operator training & sustainment**: who trains the E-5, who fixes it in the field, spares,
  battery logistics, GFE integration.
- **Advocacy chain**: a champion above the operator level; operators loving it is not adoption.
- **Prime/flow-down mechanics**: subcontract structure, government back-end integration.
- **TAK ecosystem**: if it touches situational awareness and never mentions ATAK/TAK, ask why.


## Doctrine
# Targeting doctrine brief (inject into every dossier & company agent — do NOT re-read the corpus)

Distilled from Jordan Crawford's Cannonball/Blueprint corpus, 2026-07-23.

1. **Pain-Qualified Segment beats ICP.** Never target firmographics ("50-200 person SaaS").
   Target companies in an observable painful SITUATION right now. The situation must be
   provable from public data.
2. **Existential Data Point (EDP).** For each niche, name the single number that kills the
   company if the problem stays unfixed (e.g. equipment utilization 60% vs 80% margins;
   fleet utilization <70% = crisis). Every dossier must state the EDP of its CTO's target
   situation: "what does it cost a company, in what metric, to leave this unfixed?"
3. **Jobs are confessions.** A job posting is a company publicly describing its own pain.
   We match jobs whose EXISTENCE signals a problem the CTO has already fixed — not jobs the
   CTO would apply to. "First Head of Engineering" = no technical leadership. "Integration
   Engineer (Epic, HL7)" = drowning in healthcare plumbing.
4. **The message is a redescription of the targeting.** If targeting is precise, the message
   writes itself by describing what we found. The reader should think "how do they know that?"
5. **Message-as-gift (GTM Shift letter standard).** Conversational, unhurried, 4–6 short
   sentences. Opens with a verified surprising fact. Every claim traces to a named public
   source. Valuable even if never answered. One light question, under 15 words. Bans: "I
   noticed", flattery, "hope you're well", meeting asks, personal details about the recipient
   as the opener. Lead with the CTO's expertise matched to the company's evident problem.
6. **Niche = Industry × Situation × Unique Angle** — all three specific (Ben Horowitz
   principle: not best botanist; best Japanese botanist specializing in Zen gardens).
7. **Insight tests**: counterintuitive (would other CTOs argue?), specific (exact tools/
   patterns), quantifiable (before/after numbers), demonstrable (showable in 5 minutes).
8. **Evidence honesty.** Actions > words > database records. Aggregator/API data (Blitz,
   FullEnrich) never corroborates itself — load-bearing claims need the person's or
   company's OWN public artifacts. Date every claim; label estimates as estimates.
9. **Dropping a bad fit is success.** Never pad a list to hit a count.
