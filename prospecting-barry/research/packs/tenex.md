# Evidence pack: Tenex
slug: tenex | domain: tenex.ai | HQ: Sarasota, Florida US | employees(LI): 145 | founded: 2024 | stage_band: unknown
sources: vc:shield
industry(LI): Technology, Information and Internet | specialties: 
channel notes: AI-powered security operations
own-language word count: ~1365

## Their own words (website via Exa, livecrawl-preferred)

URL: https://tenex.ai
AI-Native SOC & Managed Detection Response | TENEX.AI

Better, Faster, More Cost-Effective – TENEX Leverages AI and Automation to Transform Security Operations

contact us

8586 Potter Park Dr.Sarasota, FL 34238

‪[phone]‬

[email protected]

Linkedin

Facebook

X-twitter

10X Faster Detection. 100% Alert Coverage.

# AI-Native. Human-Led. United Against Every Threat.

The first leading AI-Native, Human-Led Agentic SOC

Triage every alert. Investigate every threat. Free your team for strategic work.

Explore TENEX

920

%

### Human-AI Alignment

<

10

minute

### Triage Time

300

%+

### Lower Total Cost

980

%

### False Positives Eliminated

Its Time To Protect

## Humans + AI. Because One Without the Other Isn't Enough.

01

DETECT FASTER

### Cut Through the Noise Instantly

Domain-specific AI triages every alert in under a minute. It then scores, ranks and surfaces only what your team needs.

02

PRIORITIZE SMARTER

### 10x Faster. Purpose-Built Intelligence.

Cybersecurity-trained models understand attacker behavior, not just patterns, and deliver the highest priority threats to humans to manage. That’s the 10x difference.

03

CONTAIN SOONER

### Stop Threats Before They Escalate

Real-time guidance, clear steps and expert intelligence contain threats faster. Our team guides every critical decision.

How It Works

## AI-Powered Security Operations

### Triage

Instantly summarize and prioritize your security alerts using AI.

01

Read More

### Investigate

Autonomously gathers and correlates data across your entire security stack.

02

Read More

### Respond

Provides automated remediation and clear, actionable steps for your team.

03

Read More

### Adapt

Continuously learns from your environment and feedback to become smarter over time.

04

Read More

How It Works

## The AI-SOC is faster, more precise, and more cost-effective

### AI-Native, Human-Led

01

### Unified Agentic-SOC

02

### Radical Transparency

05

### Speed That Matches The Threat

03

### An Ecosystem That Covers Everything

04

### Radical Transparency

05

### Choose Your Security Plan

Light

Basic

Intermediate

Please enable JavaScript in your browser to complete this form.

TB Needed

TB Needed

#### 25TB

###### $99,000

Essential AI-Native MDR

#### 50TB

###### $149,000

Standard 24/7 MDR

#### 100TB

###### $199,000

Enhanced + 24/7 MDR

Submit

- Deployment, Onboarding, & Implementation
- Google or Microsoft Security Operations Platform Management
- Human-led, AI-powered Managed Detection + Response
- Platform Training & Education
- Dashboards and Reports
- White Glove Customer Success

CHOOSE PLAN

#### ENTERPRISE

##### Custom

Contact Us

Read More

A fully customized solution designed for unique, complex, or mission-critical security requirements.

- Bespoke Scope and Service Level
- Dedicated Security Engineering Team
- Integration with Proprietary Tools
- Flexible Contract and Pricing
- Quarterly Executive Briefings

The AI SOC Company.

## The AI Inflection Point Is Here. And That’s Good News.

The only AI-native Managed Detection and Response (MDR) provider built from the ground up around AI. This isn’t a disruption, it’s an inflection point. The teams that lean into AI don’t lose their edge. They multiply it. The analysts who embrace it stop drowning in noise and start doing the work they trained for. That’s not a smaller role. That’s a larger one.

Embrace it and it multiplies you. Fear it and it passes you by.

Read More

Career Opportunities

## We’re Hiring. Because We Believe in People.

We’re AI-native, born AI, built AI, think AI, and actively searching for the best human minds in cybersecurity. If we believed AI was here to replace people, why would we do that? The bar didn’t go away. It went up.

If you want to do the most important work of your career — at the intersection of elite human expertise and the most advanced AI in the industry — we want to talk to you.

View Open Positions

READ MORE ABOUT TENEX

## News and Industry Insight

https://tenex.ai/blog/google-ai-threat-defense/

- 2026-05-27

- Blog

### TENEX Partners with Google Cloud on Google AI Threat Defense

https://tenex.ai/blog/google-ai-threat-defense/

Read More

https://tenex.ai/blog/defend/

- 2026-04-08

- Blog

### It’s Time To Protect.

https://tenex.ai/blog/defend/

Read More

https://tenex.ai/blog/emea-piers-morgan/

- 2026-04-07

- Blog

### Why Piers Morgan Joined TENEX

https://tenex.ai/blog/emea-piers-morgan/

Read More

https://tenex.ai/news/extrahop-launches-the-agentic-soc-alliance-to-validate-ashared-operating-model-for-machine-speed-defense/

- 2026-07-22

- Press Releases

### ExtraHop® Launches the Agentic SOC Alliance to Validate aShared Operating Model for Machine-Speed Defense

https://tenex.ai/news/extrahop-launches-the-agentic-soc-alliance-to-validate-ashared-operating-model-for-machine-speed-defense/

Read More

https://tenex.ai/news/microsoft/

- 2026-07-08

- Press Releases

### TENEX.AI Expands Microsoft Security Collaboration to Help Enterprises Realize the Promise of the Agentic SOC

https://tenex.ai/news/microsoft/

Read More

https://tenex.ai/news/official-cybersecurity-partner-of-the-kansas-city-chiefs/

- 2026-06-17

- Press Releases

### TENEX.ai Named Official Cybersecurity Partner of the Kansas City Chiefs

https://tenex.ai/news/official-cybersecurity-partner-of-the-kansas-city-chiefs/

Read More

### Official Cybersecurity Partner of the Kansas City Chiefs

Learn More

#### Operationalizing AI

#### Webinar Series

View Webinar

#### Threat Intel Newsletter

### Ready to Transform Your Security Operations?

See for yourself how the TENEX.AI platform can supercharge your security team.

10X Faster Detection. 100% Alert Coverage.

## Prioritize Faster. Detect Smarter. Contain Sooner.

The first AI-native, human-led SOC that triages every alert, investigates every threat, and frees your team for strategic work.

Explore TENEX

Diverse Sector Coverage

### Tailored C

URL: https://tenex.ai/about-us/
About Tenex | Innovators in Cybersecurity Solutions

Better, Faster, More Cost-Effective – TENEX Leverages AI and Automation to Transform Security Operations

contact us

8586 Potter Park Dr.Sarasota, FL 34238

‪[phone]‬

[email protected]

Linkedin

Facebook

X-twitter

Who We Are

## About TENEX – Security Reimagined for the AI Era

TENEX is redefining cybersecurity by combining AI-driven detection, automation, and hands-on expertise to protect organizations at scale. We were founded by leaders who previously built Google’s largest cybersecurity partner, managing over 75% of Google’s security business at peak. Now, we’re leveraging that experience to transform how companies secure their digital environments in the age of artificial intelligence with our human-led, machine-driven services.

Our mission is simple – to deliver 10x better security outcomes, faster and more cost-effectively than legacy providers.

Contact Us

How We Help

### Our Approach

We believe the future of security lies in automation, AI, and expert orchestration. TENEX blends machine-driven processes with human expertise to create services that are smarter, faster, and continuously evolving.

### Automation-First

Eliminate blind spots and reduce operational overhead with automations that make your job easier.

### AI-Native

Detect and respond to threats with unmatched speed and precision thanks to AI built into the core.

### Built for Scale

From SMBs to global enterprises, TENEX adapts to your needs, delivering world-class security outcomes regardless of size or complexity.

### Why TENEX?

- Proven Leadership: Our founding team has decades of experience building and scaling security operations for some of the most complex environments on the planet.
- Google Cloud Expertise: We are the go-to partner for implementing, operating, and optimizing Google Cloud Security solutions.
- AI-Driven Advantage: Unlike others bolting AI onto existing solutions, TENEX was designed from the ground up in the age of AI. Our platform continuously learns and adapts, ensuring your security operations stay ahead of emerging threats. Our founding team has spent the past several years at the forefront of the practical application of AI in cybersecurity.
- Tailored for Growth: Whether you're a Fortune 500 or a mid-sized business, our flexible offerings provide the right level of protection without unnecessary complexity.

who we work with

### Our Investors and Partners

TENEX is backed by Andrees

URL: https://tenex.ai/services/managed-detection-and-response/
Managed Detection & Response (MDR) | 24/7 Cyber Threat Protection

Better, Faster, More Cost-Effective – TENEX Leverages AI and Automation to Transform Security Operations

contact us

8586 Potter Park Dr.Sarasota, FL 34238

‪[phone]‬

[email protected]

Linkedin

Facebook

X-twitter

Diverse Sector Coverage

## MDR: Your First Line of Defense

Get Pricing

https://tenex.ai/about-us/

### AI-Driven Threat Detection

Harness advanced AI to identify and analyze threats with precision, uncovering risks traditional methods might miss.

https://tenex.ai/about-us/

### 24/7 Monitoring and Protection

With MDR enjoy around-the-clock surveillance of your systems, ensuring continuous security and peace of mind.

https://tenex.ai/about-us/

### Real-Time Incident Containment

Immediate responses to neutralize threats and prevent them from escalating into full-blown security breaches.

https://tenex.ai/about-us/

### Comprehensive Threat Analysis

TENEX empowers enterprises with scalable solutions designed to protect complex networks, sensitive intellectual property, and critical infrastructure.

PLAN TYPES

### Choose the Plan That Works for You

#### Make Your Selection

###### Core Security Platform

#### Google Cloud Security Platform as a Service

Choose Plan

10x Experts Build, Operate, and Manage Google’s Security Stack with Expert Precision

Implementat

## LinkedIn about

TENEX is the first AI-native, human-led MDR powered by AI SOC.  Backed by 24/7 U.S.based expert analysts with 8+ years avg. experience. Our human-led, AI-driven platform delivers 10x faster detection, <1-minute MTTX, and 95% fewer false positives, transforming security operations economics while dramatically improving outcomes.  AI handles 100% of alerts at machine speed, allowing our experts to focus on complex threats demanding human judgment. We scale through AI, not headcount, delivering premium outcomes without enterprise-level costs.  

## News (Exa, top 3)
- CONTRACT to TENAX AEROSPACE LLC () https://www.usaspending.gov/award/CONT_AWD_1202SA23K9410_12C2_1202SA23T9450_12C2
  USAspending.gov
- Ammunition - Ammunition Tender in USA, ID 11508885 | Tender Impulse () https://tenderimpulse.com/government-tenders/usa/ammunition-11508885
  Ammunition - Ammunition Tender in USA, ID 11508885 | Tender Impulse

- Phone:+91 9768157682/+91 9987300609
- Email: [email]

# Ammunition - 11508885

This tender is from the country of USA in Americas region. The tender was published by DEPT OF DEFENSE on 20 Feb 2026 for Reason for amendment is to extend solicitation Cartridge 22 Cal Eley Tenex. The last date to submit your bid for this tender was 20 Feb 2026. This tender is for the companies specializing in Ammunition and similar
- TENAX AEROSPACE LLC | Federal Award Recipient Profile () https://www.usaspending.gov/recipient/1ee6edf8-2db3-d98f-9f0a-a86b2b9f0821-C/latest
  USAspending.gov

## Open roles (Blitz, live)
- Sales Operations Associate-EMEA ({'city': None, 'country_code': 'GB'})
- Senior Security Researcher ({'city': None, 'country_code': 'US'})
- Senior Security Researcher ({'city': 'Sarasota', 'country_code': 'US'})
- Senior Security Researcher ({'city': 'Overland Park', 'country_code': 'US'})
- Office Coordinator ({'city': 'Overland Park', 'country_code': 'US'})
- Senior Security Engineer - IAM ({'city': None, 'country_code': 'US'})
- Purple Team - Lead Cloud Security Engineer ({'city': None, 'country_code': 'US'})
- Lead Cloud Security Engineer ({'city': None, 'country_code': 'US'})
- Lead Cloud Security Engineer ({'city': 'Sarasota', 'country_code': 'US'})
- Lead Cloud Security Engineer ({'city': 'Overland Park', 'country_code': 'US'})

---
# Context for the scoring agent (do not re-search)

## Barry Hess niche
Defense and intelligence technology companies in the $5M-$10M range whose proven prototype is stuck between demo and fielded deployment, guided by a CTO who fielded a patented tactical Android app platform with operators in theater and now engineers deployment architecture and acquisition advocacy from day one.

**EDP:** When months-to-field exceeds the funded months remaining on the contract, the program dies in the DoD valley of death - and for a $5M-$10M defense tech company, one dead transition is often the company.

## Deployed-reality checklist (score absence against this)
- **DDIL**: denied/disconnected/intermittent/low-bandwidth comms. Canonical example: an AI
  crowd-detection startup streamed 30fps video; the contested environment had bandwidth for
  1 frame every 5 seconds. One architecture change (frame-rate decoupling) saved the project.
- **SWaP-C**: size, weight, power, cost on the platform actually carried/mounted.
- **ATO/RMF**: Authority to Operate on government networks; STIGs, POA&Ms, IL4/IL5.
- **CMMC / NIST 800-171**: company-level compliance before the contract dies.
- **ITAR/export**: controlled data handling.
- **Operator training & sustainment**: who trains the E-5, who fixes it in the field, spares,
  battery logistics, GFE integration.
- **Advocacy chain**: a champion above the operator level; operators loving it is not adoption.
- **Prime/flow-down mechanics**: subcontract structure, government back-end integration.
- **TAK ecosystem**: if it touches situational awareness and never mentions ATAK/TAK, ask why.


## Doctrine
# Targeting doctrine brief (inject into every dossier & company agent — do NOT re-read the corpus)

Distilled from Jordan Crawford's Cannonball/Blueprint corpus, 2026-07-23.

1. **Pain-Qualified Segment beats ICP.** Never target firmographics ("50-200 person SaaS").
   Target companies in an observable painful SITUATION right now. The situation must be
   provable from public data.
2. **Existential Data Point (EDP).** For each niche, name the single number that kills the
   company if the problem stays unfixed (e.g. equipment utilization 60% vs 80% margins;
   fleet utilization <70% = crisis). Every dossier must state the EDP of its CTO's target
   situation: "what does it cost a company, in what metric, to leave this unfixed?"
3. **Jobs are confessions.** A job posting is a company publicly describing its own pain.
   We match jobs whose EXISTENCE signals a problem the CTO has already fixed — not jobs the
   CTO would apply to. "First Head of Engineering" = no technical leadership. "Integration
   Engineer (Epic, HL7)" = drowning in healthcare plumbing.
4. **The message is a redescription of the targeting.** If targeting is precise, the message
   writes itself by describing what we found. The reader should think "how do they know that?"
5. **Message-as-gift (GTM Shift letter standard).** Conversational, unhurried, 4–6 short
   sentences. Opens with a verified surprising fact. Every claim traces to a named public
   source. Valuable even if never answered. One light question, under 15 words. Bans: "I
   noticed", flattery, "hope you're well", meeting asks, personal details about the recipient
   as the opener. Lead with the CTO's expertise matched to the company's evident problem.
6. **Niche = Industry × Situation × Unique Angle** — all three specific (Ben Horowitz
   principle: not best botanist; best Japanese botanist specializing in Zen gardens).
7. **Insight tests**: counterintuitive (would other CTOs argue?), specific (exact tools/
   patterns), quantifiable (before/after numbers), demonstrable (showable in 5 minutes).
8. **Evidence honesty.** Actions > words > database records. Aggregator/API data (Blitz,
   FullEnrich) never corroborates itself — load-bearing claims need the person's or
   company's OWN public artifacts. Date every claim; label estimates as estimates.
9. **Dropping a bad fit is success.** Never pad a list to hit a count.
