# Evidence pack: TrustPoint
slug: trustpoint | domain: trustpointgps.com | HQ: Mountain View, California US | employees(LI): 87 | founded:  | stage_band: series_a
sources: exa-agent:sbir-graduates-raising
industry(LI): Aviation and Aerospace Component Manufacturing | specialties: gps, gnss, satellite communications, digital signal processing
channel notes: Series A More than $20M venture equity, including investment led by DCVC. dom: data_collection status: prototype dod: TrustPoint received Phase II STTR/SBIR contracts to adapt commercial PNT payloads for DoD architectures. hq: Dulles, VA emp: 11-50 employees src: https://www.businesswire.com/news/home/20240409173368/en/TrustPoint-Announces-AFWERX-Phase-II-Contract-Award
own-language word count: ~1339

## Their own words (website via Exa, livecrawl-preferred)

URL: https://trustpointgps.com
Precision GNSS | TrustPoint, Inc.

top of page

#### Resilient GPS for the Automated and Augmented World

Contact Us

About & Subscribe

#### The Challenge We Face Today

GPS is a ubiquitous global utility in modern society, where knowing one’s location and accurate time is critical for government, commercial, and consumer applications. With new threats on the rise globally, it is even more important to complement the heritage GPS and other government-owned systems to enhance resiliency, accuracy, and security against jamming and spoofing, especially for safety-critical and high-precision applications. TrustPoint aims to provide this diversity and resilience.

"We estimate that the loss of GPS service would have a $1 billion per-day [economic] impact."

National Institute of Standards and Technology

June 2019 Report "Economic Benefits of the Global Positioning System"

#### Pursuing the Future We All Imagine

##### Our Vision

Humanity, physically connected by autonomous transportation, virtually connected through proliferated networks, secured and enabled by the integrity of its PNT services.

##### Our Mission

To build the world's most trusted and capable

global positioning and timing service.

"...we’re looking beyond GPS…we need to find alternatives for military use that are more resilient and less vulnerable."

Former Secretary of Defense Ash Carter

#### What We are Building

TrustPoint is developing a fully commercial Global Navigation Satellite System (GNSS) to provide significant performance, security and reliability improvements to GNSS users by leveraging our advanced C-Band LEO satellite constellation. These improvements, including better accuracy, quicker time to first fix, anti-spoof and anti-jam capabilities, are supporting US Government position and timing service resiliency as well as enabling next-generation commercial applications. The TrustPoint system has been developed from the ground up to achieve the high performance, security, and availability required for autonomous navigation, critical infrastructure, and national security.

The GNSS market is ripe for disruption and TrustPoint intends to lead that revolution with our microsatellite based commercial infrastructure and innovative positioning and timing services.

#### Key Sectors We Support

Our technology and services go beyond GPS Augmentation and have been developed to address the Position, Navigation and Timing pain points of both commercial and government sectors with an eye towards facilitating the proliferation of nascent applications in autonomous navigation.

If you are a GNSS stakeholder of any kind and are interested in learning more about our technology and systems, please reach out here.

## Automotive

Autonomous Cars, Trucks and Other Forms of Land Based Robotic Delivery

## Aviation

Commercial Aviation, Drone Delivery, and Urban Air Mobility

## Critical Infrastructure

Electrical Power Grids, Telecommunication and Financial Networks

## National Security

Air, Marine, Mounted and Dismounted Applications for the DoD and IC

“14 of the 16 critical infrastructures designated by the Department of Homeland Security rely on 24/7 GPS to operate for the country.”

US Cyber & Infrastructure Security Agency (CISA)

#### Who We Are

We are a team of engineers and entrepreneurs, who seek to solve multidisciplinary problems at the complex nexus of business, aerospace technology and government regulatory constraints. Combined we have over a 150 years of related defense and commercial experience and have led and held key technical roles in the development of quick turn cubesat missions, where speed and affordability are key, as well as in critical government systems, where security and reliability are king.

We are bi-coastal, with operations just outside Washington DC in Northern Virginia, and at the heart of Silicon Valley in Mountain View, CA. We are constantly on the look out for new team members and partners, so please do not hesitate reach out if you are interested in joining us on this endeavor.

Meet the Team

### Follow us on social media or subscribe below to be the first to hear news and product updates.

Subscribe Now

Thanks for submitting!

bottom of page

URL: https://www.trustpointgps.com/
Precision GNSS | TrustPoint, Inc.

top of page

#### Resilient GPS for the Automated and Augmented World

Contact Us

About & Subscribe

#### The Challenge We Face Today

GPS is a ubiquitous global utility in modern society, where knowing one’s location and accurate time is critical for government, commercial, and consumer applications. With new threats on the rise globally, it is even more important to complement the heritage GPS and other government-owned systems to enhance resiliency, accuracy, and security against jamming and spoofing, especially for safety-critical and high-precision applications. TrustPoint aims to provide this diversity and resilience.

"We estimate that the loss of GPS service would have a $1 billion per-day [economic] impact."

National Institute of Standards and Technology

June 2019 Report "Economic Benefits of the Global Positioning System"

#### Pursuing the Future We All Imagine

##### Our Vision

Humanity, physically connected by autonomous transportation, virtually connected through proliferated networks, secured and enabled by the integrity of its PNT services.

##### Our Mission

To build the world's most trusted and capable

global positioning and timing service.

"...we’re looking beyond GPS…we need to find alternatives for military use that are more resilient and less vulnerable."

Former Secretary of Defense Ash Carter

#### What We are Building

TrustPoint is developing a fully commercial Global Navigation Satellite System (GNSS) to provide significant performance, security and reliability improvements to GNSS users by leveraging our advanced C-Band LEO satellite constellation. These improvements, including better accuracy, quicker time to first fix, anti-spoof and anti-jam capabilities, are supporting US Government position and timing service resiliency as well as enabling next-generation commercial applications. The TrustPoint system has been developed from the ground up to achieve the high performance, security, and availability required for autonomous navigation, critical infrastructure, and national security.

The GNSS market is ripe for disruption and TrustPoint intends to lead that revolution with our microsatellite based commercial infrastructure and innovative positioning and timing services.

#### Key Sectors We Support

Our technology and services go beyond GPS Augmentation and have been developed to address the Position, Navigation and Timing pain points of both commercial and government sectors with an e

URL: https://www.trustpointgps.com/team-1
Team | TrustPoint, Inc.

top of page

### Meet The Team

Founder & CEO

Patrick Shannon

Product & Partnerships

Anil Goparaju

Embedded Software

Bibek Guragain

Marketing & Brand

Burcu "B" Erkmen

Founder & COO

Chris DeMay

Government Programs

Nicole Hilliard

RF Design

Patrick Ryan

Human Resources

Danielle Anderson

Signals and Processing

George Schmitt

GNSS Science

Dr. Furqan Ahmed

Embedded Systems

Austin Martin

# Directors and Advisors

Charles Beames

Executive Chairman

Former Director for Space & Intelligence, Office of Under Secretary of Defense AT&L, Director Vulcan Capital

Jeff Gick

Founder Advisor Founder and CEO of Optimum Technologies

Dr. Lisa Costa

Advisor Former US Space Force Chief Technology and Innovation Officer

John Hildebrand

Founder Advisor

Founder and COO of Optimum Technologies

Kevin O'Connell

Advisor

Former Director of the Office of Space Commerce

Jennifer Alvarez

Advisor

CTO, EnduroSat US, Founder and CEO/CTO Aurora Insights

Logan Scott

Advisor US National PNT Adviosry Board Member

Josephine Millward

Advisor

Investor, Venture Partner, OpAmp Capital

## Apply Today to Join Us

[email]

bottom of page

URL: https://www.trustpointgps.com/news
News (All) | TrustPoint, Inc.

top of page

### News

For media inquiries please contact [email]

Industry NewsPress ReleaseTrustPoint News

TrustPoint News, Industry News

May 12, 2026

## TrustPoint Receives $4M SpaceWERX Tactical Funding for Independent PNT System

Press Release, TrustPoint News

Feb 5, 2026

## TrustPoint Accelerates Defense-Grade, GPS-Independent PNT with Fifth Phase II SBIR Award in 18 Months

Read More

Press Release, TrustPoint News

Jan 15, 2026

## TrustPoint Achieves Key Milestone in Building GPS-Independent Commercial PNT

Read More

TrustPoint News, Industry News

May 12, 2026

## Space Force awards TrustPoint $4 million for LEO navigation demonstration

TrustPoint News

Jan 21, 2026

## Debrief: Making PNT Possible From LEO

TrustPoint News

Dec 9, 2025

## TrustPoint Sets 2027 Target for Initial Rollout of LEO-Based Navigation Services

Press Release

May 12, 2026

## TrustPoint Secures U.S. Space Force TACFI Contract to Demonstrate End-to-End GPS-Independent PNT

Read More

TrustPoint News

Jan 15, 2026

## TrustPoint Demonstrates Non-GPS Navigation for LEO Satellites

TrustPoint News

Nov 13, 2025

## TrustPoint is a Finalist for The 2025 Global Space Awards: SuperScaler of the Year

Read More

bottom of page

## LinkedIn about

TrustPoint is developing a set of cutting edge position, navigation and timing solutions to help augment the existing US GPS system, increasing its reliability, security, availability and accuracy.

Now Hiring: www.trustpointgps.com/careers

## News (Exa, top 3)
- TrustPoint Secures U.S. Space Force TACFI Contract to Demonstrate End-to-End GPS-Independent PNT | TrustPoint, Inc. () https://www.trustpointgps.com/news/trustpoint-secures-u.s.-space-force-tacfi-contract-to-demonstrate-end-to-end-gps-independent-pnt
  TrustPoint Secures U.S. Space Force TACFI Contract to Demonstrate End-to-End GPS-Independent PNT | TrustPoint, Inc.

top of page

< Back

# TrustPoint Secures U.S. Space Force TACFI Contract to Demonstrate End-to-End GPS-Independent PNT

May 12, 2026

TrustPoint

WASHINGTON--(BUSINESS WIRE)--TrustPoint, a pioneer in next-generation space-based positioning and navigation solutions, today announced it has been awarded a $4 million Tactical Funding Increase (TACFI) contract to demonstrate a first-o
- Space Force awards TrustPoint $4 million for LEO navigation ... (2026-05-12) https://spacenews.com/space-force-awards-trustpoint-4-million-for-leo-navigation-demonstration/
  Space Force awards TrustPoint $4 million for LEO navigation demonstration - SpaceNews

Home/ Space Force awards TrustPoint $4 million for LEO navigation demonstration

TrustPoint's low Earth orbit navigation system ground station. Credit: TrustPoint

WASHINGTON — TrustPoint, a Virginia startup developing a low-Earth-orbit navigation system intended to complement or back up GPS, said May 12 it received a $4 million contract from the U.S. Space Force to demonstrate a GPS-independent positioning, n
- TrustPoint Secures U.S. Space Force TACFI Contract to Demonstrate End- ... (2026-05-12) https://www.businesswire.com/news/home/20260512853671/en/TrustPoint-Secures-U.S.-Space-Force-TACFI-Contract-to-Demonstrate-End-to-End-GPS-Independent-PNT
  TrustPoint Secures U.S. Space Force TACFI Contract to Demonstrate End-to-End GPS-Independent PNT

May 12, 2026 9:00 AM Eastern Daylight Time

# TrustPoint Secures U.S. Space Force TACFI Contract to Demonstrate End-to-End GPS-Independent PNT

Share

---

WASHINGTON--(BUSINESS WIRE)--TrustPoint, a pioneer in next-generation space-based positioning and navigation solutions, today announced it has been awarded a $4 million Tactical Funding Increase (TACFI) contract to demonstrate a first-of-its-kind

## Open roles (Blitz, live)
- Satellite Operations Engineer ({'city': 'Dulles', 'country_code': 'US'})
- Ground Station Lead ({'city': 'Dulles', 'country_code': 'US'})
- Ground Station Lead ({'city': 'Dulles', 'country_code': 'US'})
- Senior VHDL Digital Communications Engineer ({'city': 'Dulles', 'country_code': 'US'})
- Embedded Software Engineer ({'city': 'Dulles', 'country_code': 'US'})
- Embedded Software Engineer ({'city': 'Dulles', 'country_code': 'US'})
- Electrical and RF Engineer ({'city': 'Dulles', 'country_code': 'US'})
- Electrical and RF Engineer ({'city': 'Dulles', 'country_code': 'US'})
- Commercial Sales & Business Development Lead ({'city': 'Dulles', 'country_code': 'US'})
- Ground Station Lead ({'city': 'Dulles', 'country_code': 'US'})

---
# Context for the scoring agent (do not re-search)

## Barry Hess niche
Defense and intelligence technology companies in the $5M-$10M range whose proven prototype is stuck between demo and fielded deployment, guided by a CTO who fielded a patented tactical Android app platform with operators in theater and now engineers deployment architecture and acquisition advocacy from day one.

**EDP:** When months-to-field exceeds the funded months remaining on the contract, the program dies in the DoD valley of death - and for a $5M-$10M defense tech company, one dead transition is often the company.

## Deployed-reality checklist (score absence against this)
- **DDIL**: denied/disconnected/intermittent/low-bandwidth comms. Canonical example: an AI
  crowd-detection startup streamed 30fps video; the contested environment had bandwidth for
  1 frame every 5 seconds. One architecture change (frame-rate decoupling) saved the project.
- **SWaP-C**: size, weight, power, cost on the platform actually carried/mounted.
- **ATO/RMF**: Authority to Operate on government networks; STIGs, POA&Ms, IL4/IL5.
- **CMMC / NIST 800-171**: company-level compliance before the contract dies.
- **ITAR/export**: controlled data handling.
- **Operator training & sustainment**: who trains the E-5, who fixes it in the field, spares,
  battery logistics, GFE integration.
- **Advocacy chain**: a champion above the operator level; operators loving it is not adoption.
- **Prime/flow-down mechanics**: subcontract structure, government back-end integration.
- **TAK ecosystem**: if it touches situational awareness and never mentions ATAK/TAK, ask why.


## Doctrine
# Targeting doctrine brief (inject into every dossier & company agent — do NOT re-read the corpus)

Distilled from Jordan Crawford's Cannonball/Blueprint corpus, 2026-07-23.

1. **Pain-Qualified Segment beats ICP.** Never target firmographics ("50-200 person SaaS").
   Target companies in an observable painful SITUATION right now. The situation must be
   provable from public data.
2. **Existential Data Point (EDP).** For each niche, name the single number that kills the
   company if the problem stays unfixed (e.g. equipment utilization 60% vs 80% margins;
   fleet utilization <70% = crisis). Every dossier must state the EDP of its CTO's target
   situation: "what does it cost a company, in what metric, to leave this unfixed?"
3. **Jobs are confessions.** A job posting is a company publicly describing its own pain.
   We match jobs whose EXISTENCE signals a problem the CTO has already fixed — not jobs the
   CTO would apply to. "First Head of Engineering" = no technical leadership. "Integration
   Engineer (Epic, HL7)" = drowning in healthcare plumbing.
4. **The message is a redescription of the targeting.** If targeting is precise, the message
   writes itself by describing what we found. The reader should think "how do they know that?"
5. **Message-as-gift (GTM Shift letter standard).** Conversational, unhurried, 4–6 short
   sentences. Opens with a verified surprising fact. Every claim traces to a named public
   source. Valuable even if never answered. One light question, under 15 words. Bans: "I
   noticed", flattery, "hope you're well", meeting asks, personal details about the recipient
   as the opener. Lead with the CTO's expertise matched to the company's evident problem.
6. **Niche = Industry × Situation × Unique Angle** — all three specific (Ben Horowitz
   principle: not best botanist; best Japanese botanist specializing in Zen gardens).
7. **Insight tests**: counterintuitive (would other CTOs argue?), specific (exact tools/
   patterns), quantifiable (before/after numbers), demonstrable (showable in 5 minutes).
8. **Evidence honesty.** Actions > words > database records. Aggregator/API data (Blitz,
   FullEnrich) never corroborates itself — load-bearing claims need the person's or
   company's OWN public artifacts. Date every claim; label estimates as estimates.
9. **Dropping a bad fit is success.** Never pad a list to hit a count.
