# Evidence pack: Unmanned Experts Inc.
slug: unmanned-experts-inc | domain: unmannedexperts.com | HQ: Denver, Colorado US | employees(LI): 15 | founded: 2010 | stage_band: revenue_stage
sources: sbir
industry(LI): Strategic Management Services | specialties: training (instructors, courseware  simulators), operations, utm, regulations, datalinks), uam, avs, rdt  e, safety management, industry liaison - business development - market analysis, maintenance, manpower services, hardware (gcss, engineering, swarms
channel notes: SBIR PhII x4 [2022, 2023] Air Force latest: AXIS: Autonomous Intelligent Swarmer hq: DENVER,CO emp: 5
own-language word count: ~2620

## Their own words (website via Exa, livecrawl-preferred)

URL: https://unmannedexperts.com
UMEX

# UnmannedExperts

WATCH DEMO

#### Our smart traffic software helps flying drones avoid crashes and find the best routes, guiding them safely through skies that don’t currently have traditional air traffic controllers.

Learn About Our Solution

### THE BIG PROBLEM

## Skies & Roads Are Becoming More Congested & Complex to Navigate While Lacking Autonomy Infrastructure

Imagine trains without tracks, Cell phones without cell towers

#### No standardized systems

No standardized systems to manage the flow of autonomous traffic, especially for air vehicles (drones) operating in uncontrolled airspace.

#### Safety Risks

Without proper coordination, there is a high likelihood of collisions between drones, small autonomous aircraft, and even manned vehicles operating in these spaces.

#### Inefficiencies in Navigation

Autonomous vehicle operators struggle to optimize routes due to a lack of shared, real-time traffic data, resulting in delays, wasted energy, and reduced operational efficiency.

#### Limited Airspace Accessibility

Many autonomous air vehicles avoid uncontrolled airspace entirely, as the lack of regulation and coordination makes it challenging to ensure safe passage. This limits their potential for widespread use in delivery, surveillance, and transportation.

#### Fragmented Solutions

Current tools are siloed, with separate systems for ground vehicles and aerial drones, making it difficult to integrate traffic management across different modes of autonomous transportation.

>24,000 ft

18,000 ft

<1,200 ft

### Airspace Authority

Upper Aerospace - Class A

Middle Aerospace - Class E

UNREGULATED AIR SPACE

Lower Aerospace - Class G

### Our UNMANNED SOLUTION

## BlueSkies

## BlueSkies Core Competencies

Unmanned Experts is leading the charge in redefining drone technology with our industry changing solution, BlueSkies. Designed to tackle the challenges of vast, uncontrolled airspace, BlueSkies represents a leap forward in autonomy, efficiency, and safety. By integrating cutting-edge technology and next-level situational awareness, it’s not just keeping pace with industry demands—it’s setting the standard.

OUR TECH IN USE

#### End-to-End AAM Solutions

Blue Skies covers all aspects from initial planning to full implementation, offering an intuitive and user-friendly platform that supports early adoption and easy entry into AAM planning.

#### Air Corridor Editor Optimized for On-Demand Mobility

Blue Skies covers all aspects from initial planning to full implementation, offering an intuitive and user-friendly platform that supports early adoption and easy entry into AAM planning.

#### Integrated Traffic Management

Utilizing a mesh network of software-defined radios and a PSU* software backbone, Blue Skies ensures secure, efficient communication between vehicles and infrastructure.

#### Cybersecurity and Data Privacy:

Robust cybersecurity framework to protect communication channels, navigation systems, and user data fostering trust in AAM systems by ensuring safety and privacy for operators and passengers.

#### Advanced Simulation & Infrastructure

Our Live-Virtual-Constructive (LVC) simulation capabilities and scalable ‘Jigsaw’ approach enable clients to visualize and optimize their operations.

#### Digital Twin Technology

Use digital twins to simulate AAM operations, including flight paths and corridors, weather, and infrastructure usage to optimize planning, predict maintenance needs, and evaluate the impact of new policies or technologies.

### BlueSkies Interface: AirCommons

### Integrated Traffic Management

Air Commons’ Integrated Traffic Management solutions leverage multi-mission hardware and software gateways to streamline airspace operations. By incorporating sensor/data fusion and open architecture systems, Air Commons ensures seamless coordination and information dominance, enabling efficient and safe air traffic across diverse environments.

### Air Domain Awareness

Air Commons’ Air Domain Awareness capabilities provide real-time situational awareness through deployable and ruggedized platforms. Utilizing customizable systems and sensor/data fusion, Air Commons offers comprehensive monitoring solutions that empower operators with critical insights for superior information dominance in the air domain.

### Customizable & Deployable

Air Commons is designed to be both customizable and deployable, focusing on flexibility and durability. Built with an open architecture framework and ruggedized for challenging environments, Air Commons ensures rapid deployment and adaptability to various mission requirements, delivering unparalleled performance and reliability.

#### STRATEGIC PARTNERSHIPS

#### STRATEGIC PARTNERSHIPS

### INDUSTRIES WE SERVE

Unmanned Experts (UMEX) is building autonomous systems to make operations safer and more efficient across emergency response, oil and gas, agriculture, logistics, defense, and homeland security. Our tech helps first responders, optimizes supply chains, and secures critical infrastructure—leveraging robotics, swarming, and UTM to get the job done faster and smarter.

### Public Sector

VIEW CASE STUDY

### Department of Homeland Security

Border security, disaster response, law enforcement and public safety, critical infrastructure protection, counterterrorism, environment, research and development.

### Department of Defense

Intelligence, surveillance, and recon (ISR), combat strike operations, logistics and resupply, cyber operations, disaster response.

### Emergency Response

Search and rescue, disaster response, wildfire management, hazardous material incidents, flood and water rescue, firefighting, law enforcement emergencies, medical response, environmental monitoring.

### Private Sector

VIEW CASE STUDY

### Oil & Gas

Inspection and monitoring, exploration and surveying, logistics and deliver environmental impacts, security and surveillance, maintenance and repairs, logistics.

### Agriculture

Cro

URL: https://www.unmannedexperts.com/#solution
UMEX

# UnmannedExperts

WATCH DEMO

#### Our smart traffic software helps flying drones avoid crashes and find the best routes, guiding them safely through skies that don’t currently have traditional air traffic controllers.

Learn About Our Solution

### THE BIG PROBLEM

## Skies & Roads Are Becoming More Congested & Complex to Navigate While Lacking Autonomy Infrastructure

Imagine trains without tracks, Cell phones without cell towers

#### No standardized systems

No standardized systems to manage the flow of autonomous traffic, especially for air vehicles (drones) operating in uncontrolled airspace.

#### Safety Risks

Without proper coordination, there is a high likelihood of collisions between drones, small autonomous aircraft, and even manned vehicles operating in these spaces.

#### Inefficiencies in Navigation

Autonomous vehicle operators struggle to optimize routes due to a lack of shared, real-time traffic data, resulting in delays, wasted energy, and reduced operational efficiency.

#### Limited Airspace Accessibility

Many autonomous air vehicles avoid uncontrolled airspace entirely, as the lack of regulation and coordination makes it challenging to ensure safe passage. This limits their potential for widespread use in delivery, surveillance, and transportation.

#### Fragmented Solutions

Current tools are siloed, with separate systems for ground vehicles and aerial drones, making it difficult to integrate traffic management across different modes of autonomous transportation.

>24,000 ft

18,000 ft

<1,200 ft

### Airspace Authority

Upper Aerospace - Class A

Middle Aerospace - Class E

UNREGULATED AIR SPACE

Lower Aerospace - Class G

### Our UNMANNED SOLUTION

## BlueSkies

## BlueSkies Core Competencies

Unmanned Experts is leading the charge in redefining drone technology with our industry changing solution, BlueSkies. Designed to tackle the challenges of vast, uncontrolled airspace, BlueSkies represents a leap forward in autonomy, efficiency, and safety. By integrating cutting-edge technology and next-level situational awareness, it’s not just keeping pace with industry demands—it’s setting the standard.

OUR TECH IN USE

#### End-to-End AAM Solutions

Blue Skies covers all aspects from initial planning to full implementation, offering an intuitive and user-friendly platform that supports early adoption and easy entry into AAM planning.

#### Air Corridor Editor Optimized for On-Demand Mobility

Blue Skies covers all aspects from in

URL: https://unmannedexperts.com/about
About

# About Unmanned Experts

## Confidence at the Cutting Edge

UMEX/AX is a highly focused team of professionals leading the US’s pursuit of technological superiority in the fields of smart autonomy, swarming systems, collaborative robotics & networked communications

Learn About Our Solution

### LEADERSHIP

### Keven Gambold

Chief Executive Officer

### Kevin Gambold

CEO

Keven brings extensive military aviation expertise to the company from his distinguished 21-year career as a Royal Air Force Squadron Leader. His impressive flight experience includes 1,500 hours commanding Tornado GR4 fast jets and an additional 1,500 hours piloting MQ-1B Predator missions for the U.S. Air Force.A recognized subject matter expert in unmanned systems, swarming technologies, and human-machine interfacing, Keven holds a Master's in Aeronautical Science from Embry-Riddle Aeronautical University and is pursuing his doctorate in strategic leadership. As a published author on topics including swarm technologies, Advanced Air Mobility (AAM), and Vehicle-to-Vehicle (V2V) CONOPS, he brings unique insights to UMEX's development of next-generation autonomous aviation solutions.

READ BIO

### Bill Cossoff

COO/ Founder

### Bill Cossoff

COO - Co-Founder

Bill serves as Chief Operating Officer and Founder of Unmanned Experts (UMEX), bringing 30 years of C-level experience 

## LinkedIn about

Unmanned Experts Inc.(UMEX) is a world-leading provider of Subject Matter Expertise in the nascent fields of autonomous robotics, swarms, UTM and mass data. We also have class-beating experience in the traditional unmanned aircraft systems (UAS) domains. The UMEX team has peerless understanding of military, civilian, commercial and para-public unmanned system operations and continues to push the sharp end of the robotics industry forward. We provide the rare capability to originate and conduct research, development, testing, and evaluation (RDT & E) of unmanned technologies, develop working Co

## SBIR/STTR record (their own abstracts)

### AXIS: Autonomous Intelligent Swarmer — Air Force 2023 ($1,227,070)
Mass, autonomous, swarming assets need to be Robust, Reliable and Ready. Current systems are none of the above: The Autonomous Intelligent Swarmer (AXIS) will fix that. AXIS solves a myriad of problems that the Unmanned Experts (UMEX) team have consistent
### DROIDISH: Collaborative Autonomous Vehicle Language — Air Force 2022 ($749,733)
The deployment of collaborative autonomous munitions & unmanned systems is the next phase of the DoD’s entry into the Information Age with programs such as Golden Horde, a USAF Vanguard program, working to integrate software-defined datalink radios and collaborative capabilities into legacy weapons systems. DROIDISH™ is designed to integrate directly into this program and to demonstrate the utility and ubiquity of a standardized vehicular ad-hoc network (VANET) language. The DROIDISH™ team comprises of some of the lead V2V practitioners in the civilian standards world, notably IEEE and ASTM. Previous UMEX programs, including the AFWERX Phase I OXYGEN project, have aimed to set and test the range, data rates, RSSI and SNR standards for IEEE, and the need for a ‘common’ language has been the source of ongoing discussions. DROIDISH™ aims to build that standard common language and provide a framework for its adoption across the spectrum of civil and military roles. UMEX has been working with AFLCMC’s Emerging Technologies Branch (EBZ) to develop a swarming munitions planning, and mission management program called Air Commons™ - Swarm (Phase II Contract # FA8656-21-C-0109). DROIDISH™ has tremendous potential in allowing these assets to collaborate on complex mission types, involving heterogenous payloads and distinct sortie profiles. UMEX’s intent is to ‘fold’ the developing DROIDISH™ language capabilities into AC-Swarm and other associated programs as the maturity level allows. The OXYGEN program culminated with a three-ship UAS ‘Bumper Cars’ flight test called GOLD 6 (see below). From that work, the need to expand the V2V and V2I communications capability and language dictionary became apparent. A GOLD 7 flight was designed and introduced to allow for V2V and V2I communications to be run concurrently, with AES and DTLS encryption operating. Another EBZ program, the Weapons Open Communications Architecture (WOCA), has some strong connectivity to the DROIDISH™ efforts and an outreach has already commenced with IS4S, the Government partner, to tie the projects where appropriate. The UMEX team plans to schedule a DROIDISH™ and WOCA ‘gladiatorial’ contest within the AFRL Colosseum simulation test system when both programs are suitably matured. It is one goal of the DROIDISH™ project to obtain designation as an “e-Program” within the Digital Transformation efforts at the Armament Directorate. AC-Swarm is attempting this process at the current time and the desire is to include DROIDISH™ in these USAF efforts to modernize weapon system procurement.
### WEB WEASELS: Autonomous Swarming AI Munitions — Air Force 2022 ($1,833,687)
Web Weasels (WW) is a multi-pronged RDT&E program designed to meet one of the stated goals from the USAF’s 2030 Science & Technology Strategy document: “Overwhelm adversaries with complexity, unpredictability, and numbers through a collaborative and autonomous network of systems and effects” The near-peer conflict environment is far deadlier to 4th Gen air systems and even 5th Gen can be challenged through cyber and EW methods. The next DoD ‘Offset’ calls for smart, autonomous, and connected munitions to prepare, seize and maintain air superiority over the Battlespace. WW builds on a unique swarm planning and deployment capability (Air Commons® - Swarm or AC-S) by applying cutting edge AI/ML training techniques and edge computing. WW redefines a ‘smart bomb’ to be a highly trained, thinking, reactive, and team-playing asset in the fight. In addition, WW has been built to address the AFRL 2022 SBIR requirements: “The Armament Directorate is pursuing ideas that permit Blue Forces to command various collaborative weapons systems and coordinated tactics to ensure success. A dynamic battlespace requires automated, adaptive weapons systems and cooperative tactics… Artificial intelligence algorithms with “dialable” human influence…” Squadrons of autonomous collaborative munitions operating at the far edge of the C2 infrastructure, need the training, tactics, techniques, and procedures (TTPs) to handle the speed-of-datalink environment that occurs in modern combat. Teamwork, communication, shared mental models, and a robust set of tried and tested strategies are required to survive and to dominate. The parent program, AC-S, allowed commanders to plan, task and manage multiple swarming assets through a ’Swarm ATO’ and a Swarm Engine. WW is designed to overlay AI/ML training algorithms onto AC-S to provide pre-launch munitions with a series of TTPs in a ‘Playbook’ for a given mission set (i.e., SEAD). The munitions will launch with AI-on-the-Edge hardware, firmware and comms architecture (OXYGEN and DROIDISH™) which allows them to collaboratively assess the combat environment, vote on the most suitable Play, call an ‘Audible’ change, and execute the improved plan. WW uses three teams, one building and training AI/ML Playbook algorithms; one focused on AI-on-the-Edge, voting and Audibles; and the third one testing Human-Swarm Interfaces (HSwI) configurations. WW will use AirSim to run trial iterations and Mission Rehearsals, and then fly suitably equipped small UAS (emulating autonomous stand-off munitions) against Liteye System’s SHIELD Counter-UAS systems (emulating an Enemy Force integrated radar threat) in the time-honored traditional mission sets of the original USAF Wild Weasels: “First In, Last Out”.
### AIR COMMONS - DENIM: Deployed Environment Networked Information Matrix — Air Force 2022 ($749,220)
Airbases both at home and overseas are strategic assets. The surge in Commercial off The Shelf (COTS) unmanned systems has produced an airspace integration problem. Advanced Air Mobility (AAM), Unmanned Traffic Management (UTM), Swarms and Counter-small UAS (C-sUAS) systems all involve tracking large numbers of semi-autonomous aircraft, some friendly, some not, yet all operating in close proximity and within an airbase’s Area of Responsibility (AOR). Around Main and Forward Operating Bases (MOB/FOB), traditional Air Traffic Management (ATM) and Security personnel must maintain Air Domain Awareness (ADA) for safety of flight, airspace deconfliction, optimal flight path planning and successful defense of the perimeter. States-side MOBs and OCONUS FOBs need cyber-secure, standalone (zero backhaul) ATM networks that incorporate either air-gapped or Deployable UTM and any fielded C-sUAS. These integrations would provide total ADA of BLUE, Unknown and RED manned/unmanned assets in the airbase’s AOR. AIR COMMONS™ - DENIM (AC-DENIM) is designed to mitigate these concerns by providing complete ADA of the MOB/FOB low altitude airspace within the AOR by using all available intelligence feeds to detect, identify and factor air assets; and by providing that collated intelligence picture to all fielded systems over a secure mesh network. Unmanned Experts Inc. (UMEX), in conjunction with AFRL, the Civil Air Patrol (CAP) and 87th Security Forces Squadron, will design, build, test and demonstrate a hardware-software solution that integrates Situational Awareness platforms such as ATAK and AERONet, C-sUAS systems such as NINJA, and SHIELD AI, and fielded ATM solutions into UMEX’s TRL7 Air Commons Deployable-UTM and provide a complete air picture, Common Operating Picture (COP) and ADA picture to Air Traffic, Ops and Security personnel at home and abroad. Based on the Air Commons UTM, which was developed for wildland fire and disaster response operations, the AC-DENIM program can rapidly integrate with fielded systems and adopt customer / end-user specific requirements (hardware, spectrum management, security & cyber specs). AC-DENIM builds on the highly successful AFWERX Phase I AIR COMMONS™ - BLUE (AC-BLUE) SBIR program (Contract FA8649-21-P-1048) flown in 2021. To build on that program, UMEX proposes an extensive Phase II, called AC-DENIM which aims for a Deployed Environment Networked Information Matrix to MOBs & FOBs. In AC-DENIM, the air and ground info feeds from many disparate sources, to include ATM, ATAK, AERONet and C-sUAS, and even networked aircraft, are injected into the AIR COMMONS™ backbone. The ground and air ‘pictures’ are then broadcast over the secure mesh network to all nodes, to include the original systems, and other suitably equipped/secured units. AC-DENIM is designed to provide either a full hardware ‘box’ and mesh network for standalone operations, or the integration of a software product within an established Program of Record system.

## News (Exa, top 3)
- Award | SBIR () https://www.sbir.gov/awards/199095
  Award | SBIR Skip to main content

# Award

Portfolio Data

Back to Award Search

## WEB WEASELS: Autonomous Swarming AI Munitions

### Awardee

#### Unmanned Experts Inc.

720 S Colorado Blvd Penthouse N Denver, CO, 80246-1962USA

Award Year: 2022

UEI: NDL1C8TZZA97

HUBZone Owned: No

Woman Owned: No

Socially and Economically Disadvantaged: No

Congressional District: 1

Tagged as:

SBIR

Phase II

### Awarding Agency

DOD

Branch: USAF

Total Award Amount: $1,833,687

Contract Number: FA8658
- Unmanned Experts, Inc. - | SBIR () https://www.sbir.gov/portfolio/663078
  Firm | SBIR Skip to main content

# Company

Portfolio Data

Back to Company Search

## Unmanned Experts Inc.

### Address

720 S Colorado Blvd Penthouse N Denver, CO, 80246-1962USA

UEI: NDL1C8TZZA97

Number of Employees: 8

HUBZone Owned: No

Woman Owned: No

Socially and Economically Disadvantaged: No

#### SBIR/STTR Involvement

Year of first award: 2016

6

Phase I Awards

7

Phase II Awards

116.67%

Conversion Rate

$633,376

Phase I Dollars

$7,356,906

Phase II Dollars

$7,990,282

Tota
- US Air Force selects team led by Liteye and Unmanned Experts for swarm management solution – Unmanned airspace () https://www.unmannedairspace.info/uncategorized/us-air-force-selects-team-led-by-liteye-and-unmanned-experts-for-swarm-management-solution/
  US Air Force selects team led by Liteye and Unmanned Experts for swarm management solution – Unmanned airspace

The US Air Force’s AFLCMC Armament Directorate has awarded a one-year contract to the team led by Unmanned Experts and drone detection company Liteye Systems to build Web Weasels Autonomous Swarming Artificially Intelligent Munitions programme. Web Weasels is part of Unmanned Experts parent program called Air Commons -Swarm which allows commanders to plan, task, and manage multiple swa

## Open roles
(no jobs data found)

---
# Context for the scoring agent (do not re-search)

## Barry Hess niche
Defense and intelligence technology companies in the $5M-$10M range whose proven prototype is stuck between demo and fielded deployment, guided by a CTO who fielded a patented tactical Android app platform with operators in theater and now engineers deployment architecture and acquisition advocacy from day one.

**EDP:** When months-to-field exceeds the funded months remaining on the contract, the program dies in the DoD valley of death - and for a $5M-$10M defense tech company, one dead transition is often the company.

## Deployed-reality checklist (score absence against this)
- **DDIL**: denied/disconnected/intermittent/low-bandwidth comms. Canonical example: an AI
  crowd-detection startup streamed 30fps video; the contested environment had bandwidth for
  1 frame every 5 seconds. One architecture change (frame-rate decoupling) saved the project.
- **SWaP-C**: size, weight, power, cost on the platform actually carried/mounted.
- **ATO/RMF**: Authority to Operate on government networks; STIGs, POA&Ms, IL4/IL5.
- **CMMC / NIST 800-171**: company-level compliance before the contract dies.
- **ITAR/export**: controlled data handling.
- **Operator training & sustainment**: who trains the E-5, who fixes it in the field, spares,
  battery logistics, GFE integration.
- **Advocacy chain**: a champion above the operator level; operators loving it is not adoption.
- **Prime/flow-down mechanics**: subcontract structure, government back-end integration.
- **TAK ecosystem**: if it touches situational awareness and never mentions ATAK/TAK, ask why.


## Doctrine
# Targeting doctrine brief (inject into every dossier & company agent — do NOT re-read the corpus)

Distilled from Jordan Crawford's Cannonball/Blueprint corpus, 2026-07-23.

1. **Pain-Qualified Segment beats ICP.** Never target firmographics ("50-200 person SaaS").
   Target companies in an observable painful SITUATION right now. The situation must be
   provable from public data.
2. **Existential Data Point (EDP).** For each niche, name the single number that kills the
   company if the problem stays unfixed (e.g. equipment utilization 60% vs 80% margins;
   fleet utilization <70% = crisis). Every dossier must state the EDP of its CTO's target
   situation: "what does it cost a company, in what metric, to leave this unfixed?"
3. **Jobs are confessions.** A job posting is a company publicly describing its own pain.
   We match jobs whose EXISTENCE signals a problem the CTO has already fixed — not jobs the
   CTO would apply to. "First Head of Engineering" = no technical leadership. "Integration
   Engineer (Epic, HL7)" = drowning in healthcare plumbing.
4. **The message is a redescription of the targeting.** If targeting is precise, the message
   writes itself by describing what we found. The reader should think "how do they know that?"
5. **Message-as-gift (GTM Shift letter standard).** Conversational, unhurried, 4–6 short
   sentences. Opens with a verified surprising fact. Every claim traces to a named public
   source. Valuable even if never answered. One light question, under 15 words. Bans: "I
   noticed", flattery, "hope you're well", meeting asks, personal details about the recipient
   as the opener. Lead with the CTO's expertise matched to the company's evident problem.
6. **Niche = Industry × Situation × Unique Angle** — all three specific (Ben Horowitz
   principle: not best botanist; best Japanese botanist specializing in Zen gardens).
7. **Insight tests**: counterintuitive (would other CTOs argue?), specific (exact tools/
   patterns), quantifiable (before/after numbers), demonstrable (showable in 5 minutes).
8. **Evidence honesty.** Actions > words > database records. Aggregator/API data (Blitz,
   FullEnrich) never corroborates itself — load-bearing claims need the person's or
   company's OWN public artifacts. Date every claim; label estimates as estimates.
9. **Dropping a bad fit is success.** Never pad a list to hit a count.
