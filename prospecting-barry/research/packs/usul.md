# Evidence pack: Usul
slug: usul | domain: usul.com | HQ: San Francisco, California US | employees(LI): 16 | founded: 2024 | stage_band: unknown
sources: vc:scout
industry(LI): Software Development | specialties: govcon, defense, ai, procurement, govtech
channel notes: defense contracting intelligence platform
own-language word count: ~1041

## Their own words (website via Exa, livecrawl-preferred)

URL: https://usul.com
Book a demo

# Where government contracts are won

Today's top government contractors trust Usul's AI platform to compete smarter and win bigger across every market.

See Usul in action

Contract Inbox

Forecasts

Teaming

Market Maps

- Our AI matches every contract against your capabilities and relationships making sure no contract is ever missed again. Say goodbye to keyword search. Learn more Automatically identify opportunities
- Surface every expiring contract and upcoming opportunity 2-5 years before they hit the street. Empower your team to displace the incumbent before competitors even know the contract exists. Learn more Win contracts competitors miss
- Identify the best teaming partners and subcontractors for every pursuit based on past performance, certifications, and agency relationships. Build the right team before the RFP drops and walk in with a winning roster. Learn more Build winning teams
- Track program funding, agency priorities, and stakeholder shifts across every market you compete in. Shift strategy in real time on the highest value pursuits. Learn more Map funding and stakeholders

Usul gives government contractors a secure, trusted way to deploy AI across your organization. We embed directly within your teams to drive wins at scale.

### Usul is the trusted AI partner for government contractors

Capture Management Spend less time searching, more time winning. Pursue opportunities from your target customers daily.

Strategic Planning Analyze where budgets are flowing, which programs are funded, and who controls spending.

Business Development Build a smarter pipeline. Track expiring contracts and new programs before opportunities hit the street.

Supplier Management Find and connect with the best teaming partners and subcontractors for every pursuit.

### Our customers have won over $12B through Usul

contract portals and sources

0+, opportunities pursued, 0k, BUILT FOR ALL INDUSTRIES

# Usul is committed to helping any company across any allied nation win government contracts.

Protecting our allied nations Defense

Modernizing public health systems Healthcare

Securing the digital frontier IT & Cyber

Building tomorrow's infrastructure Construction

Powering the clean transition Energy

Sustaining the global food supply Agriculture

Expanding humanity's reach Space

Connecting the modern world Transportation

Top government contractors use Usul for

Competitive intelligence, Stakeholder mapping, Budget analysis, Contract identification, Recompete tracking, Teaming requests, Pipeline management

Book a demo

###### Our mission

$8.4 trillion in government spending flows across allied nations every year. Most of it is invisible to the companies best positioned to win it.

We're building the platform that connects what governments need with what companies can deliver, across every industry and every allied nation.

Together with the contractors, startups, and governments on our platform, we're creating a fundamentally new way for companies and governments to find each other, transact, and build the future.

###### Come build the future of B2G spend

We're a fast-moving team backed by Y Combinator, shipping products that reshape how governments buy technology across the world. Build the future of B2G with us.

Learn More

### The trusted AI partner for government contractors.

### Unleash your experience.

Book a demo

---

## Navigation

- Sources
- Services
- LinkedIn
- Book a demo
- Teaming
- Market Maps
- Forecasts
- Contract Inbox
- Contract CRM
- Competitive Intel
- Stakeholder Directory
- Industry Days
- Capture Management
- Business Development
- Government Consulting
- Strategic Planning
- Supplier Management
- Defense
- Healthcare
- IT & Cyber
- Construction
- Energy
- Agriculture
- Space
- Transportation
- Company
- News
- Careers

URL: https://usul.com/book-a-demo
Book a demo

# Book a demo

Request a demo to see how Usul fits into your daily workflows. Fill out the form and our team will get back to you shortly.

---

## Navigation

- Sources
- Services
- LinkedIn
- Book a demo
- Teaming
- Market Maps
- Forecasts
- Contract Inbox
- Contract CRM
- Competitive Intel
- Stakeholder Directory
- Industry Days
- Capture Management
- Business Development
- Government Consulting
- Strategic Planning
- Supplier Management
- Defense
- Healthcare
- IT & Cyber
- Construction
- Energy
- Agriculture
- Space
- Transportation
- Company
- News
- Careers

URL: https://usul.com/#contract-inbox-scroll
Book a demo

# Where government contracts are won

Today's top government contractors trust Usul's AI platform to compete smarter and win bigger across every market.

See Usul in action

Contract Inbox

Forecasts

Teaming

Market Maps

- Our AI matches every contract against your capabilities and relationships making sure no contract is ever missed again. Say goodbye to keyword search. Learn more Automatically identify opportunities
- Surface every expiring contract and upcoming opportunity 2-5 years before they hit the street. Empower your team to displace the incumbent before competitors even know the contract exists. Learn more Win contracts competitors miss
- Identify the best teaming partners and subcontractors for every pursuit based on past performance, certifications, and agency relationships. Build the right team before the RFP drops and walk in with a winning roster. Learn more Build winning teams
- Track program funding, agency priorities, and stakeholder shifts across every market you compete in. Shift strategy in real time on the highest value pursuits. Learn more Map funding and stakeholders

Usul gives government contractors a secure, trusted way to deploy AI across your organization. We embed directly within your teams to drive wins at scale.

### Usul is the trusted AI partner for government contractors

Capture Management Spend less time searching, more time winning. Pursue opportunities from your target customers daily.

Strategic Planning Analyze where budgets are flowing, which programs are funded, and who controls spending.

Business Development Build a smarter pipeline. Track expiring contracts and new programs before opportunities hit the street.

Supplier Management Find and connect with the best teaming partners and subcontractors for every pursuit.

### Our customers have won over $12B through Usul

contract portals and sources

0+, opportunities pursued, 0k, BUILT FOR ALL INDUSTRIES

# Usul is committed to helping any company across any allied nation win government contracts.

Protecting our allied nations Defense

Modernizing public health systems Healthcare

Securing the digital frontier IT & Cyber

Building tomorrow's infrastructure Construction

Powering the clean transition Energy

Sustaining the global food supply Agriculture

Expanding humanity's reach Space

Connecting the modern world Transportation

Top government contractors use Usul for

Competitive intelligence, Stakeholder mapping, Budget analysis, Contract identif

## LinkedIn about

Fortune 500 companies and the fastest growing startups use Usul\'s AI platform to win government contracts across 60+ nations. We connect everything government contracting teams need including opportunity identification, recompete tracking, strategic planning, stakeholder mapping, pipeline management, competitive intelligence, industry events and more.

## News (Exa, top 3)
- $3.3M Seed Fundraising Announcement () https://usul.com/news/3-3m-seed-fundraising-announcement
  Book a demo

May 12, 2025

# $3.3M Seed Fundraising Announcement

Usul raised $3.3M in seed funding led by Scout Ventures and backed by defense legends Steve Blank and Lt. Gen. Jack Shanahan to build the infrastructure reshaping government procurement worldwide.

Table of contents

# We're Redefining How Allied Nations Buy Technology

Defense primes and startups have won over $500M in contracts through Usul in less than a year. But we're building something bigger than a capture platform.

We're 
- Exclusive: Usul Raises $3.3M to Help Companies Win ... (2025-05-12) https://www.tectonicdefense.com/exclusive-usul-raises-3-3m-to-help-companies-win-defense-contracts/
  Exclusive: Usul Raises $3.3M to Help Companies Win Defense Contracts

Sit around any defense tech conference long enough, and the industry’s main gripe will become abundantly clear: the Pentagon is a byzantine labyrinth, and the government is, well, a pain in the ass to work with.

Turns out one company may have finally found a solution. This morning, software startup Usul—born out of Stanford’s Hacking for Defense course and Y-Combinator—announced a $3.3M seed funding round, led by Scout Ventur
- Usul - AI Partner For Winning Government Contracts () https://usul.com/
  Book a demo

# Where government contracts are won

Today's top government contractors trust Usul's AI platform to compete smarter and win bigger across every market.

See Usul in action

Contract Inbox

Forecasts

Teaming

Market Maps

- Our AI matches every contract against your capabilities and relationships making sure no contract is ever missed again. Say goodbye to keyword search. Learn more Automatically identify opportunities
- Surface every expiring contract and upcoming opportunity 2-5 

## Open roles (Blitz, live)
- Founding Growth Marketer / GTM Engineer ({'city': 'San Francisco', 'country_code': 'US'})
- Head of Growth ({'city': 'San Francisco', 'country_code': 'US'})
- Deployment Strategist ({'city': 'San Francisco', 'country_code': 'US'})
- Chief of Staff ({'city': 'San Francisco', 'country_code': 'US'})
- Founding Recruiter / Head of People ({'city': 'San Francisco', 'country_code': 'US'})
- Founding Account Executive ({'city': 'San Francisco', 'country_code': 'US'})
- Founding Full Stack Engineer ({'city': 'San Francisco', 'country_code': 'US'})

---
# Context for the scoring agent (do not re-search)

## Barry Hess niche
Defense and intelligence technology companies in the $5M-$10M range whose proven prototype is stuck between demo and fielded deployment, guided by a CTO who fielded a patented tactical Android app platform with operators in theater and now engineers deployment architecture and acquisition advocacy from day one.

**EDP:** When months-to-field exceeds the funded months remaining on the contract, the program dies in the DoD valley of death - and for a $5M-$10M defense tech company, one dead transition is often the company.

## Deployed-reality checklist (score absence against this)
- **DDIL**: denied/disconnected/intermittent/low-bandwidth comms. Canonical example: an AI
  crowd-detection startup streamed 30fps video; the contested environment had bandwidth for
  1 frame every 5 seconds. One architecture change (frame-rate decoupling) saved the project.
- **SWaP-C**: size, weight, power, cost on the platform actually carried/mounted.
- **ATO/RMF**: Authority to Operate on government networks; STIGs, POA&Ms, IL4/IL5.
- **CMMC / NIST 800-171**: company-level compliance before the contract dies.
- **ITAR/export**: controlled data handling.
- **Operator training & sustainment**: who trains the E-5, who fixes it in the field, spares,
  battery logistics, GFE integration.
- **Advocacy chain**: a champion above the operator level; operators loving it is not adoption.
- **Prime/flow-down mechanics**: subcontract structure, government back-end integration.
- **TAK ecosystem**: if it touches situational awareness and never mentions ATAK/TAK, ask why.


## Doctrine
# Targeting doctrine brief (inject into every dossier & company agent — do NOT re-read the corpus)

Distilled from Jordan Crawford's Cannonball/Blueprint corpus, 2026-07-23.

1. **Pain-Qualified Segment beats ICP.** Never target firmographics ("50-200 person SaaS").
   Target companies in an observable painful SITUATION right now. The situation must be
   provable from public data.
2. **Existential Data Point (EDP).** For each niche, name the single number that kills the
   company if the problem stays unfixed (e.g. equipment utilization 60% vs 80% margins;
   fleet utilization <70% = crisis). Every dossier must state the EDP of its CTO's target
   situation: "what does it cost a company, in what metric, to leave this unfixed?"
3. **Jobs are confessions.** A job posting is a company publicly describing its own pain.
   We match jobs whose EXISTENCE signals a problem the CTO has already fixed — not jobs the
   CTO would apply to. "First Head of Engineering" = no technical leadership. "Integration
   Engineer (Epic, HL7)" = drowning in healthcare plumbing.
4. **The message is a redescription of the targeting.** If targeting is precise, the message
   writes itself by describing what we found. The reader should think "how do they know that?"
5. **Message-as-gift (GTM Shift letter standard).** Conversational, unhurried, 4–6 short
   sentences. Opens with a verified surprising fact. Every claim traces to a named public
   source. Valuable even if never answered. One light question, under 15 words. Bans: "I
   noticed", flattery, "hope you're well", meeting asks, personal details about the recipient
   as the opener. Lead with the CTO's expertise matched to the company's evident problem.
6. **Niche = Industry × Situation × Unique Angle** — all three specific (Ben Horowitz
   principle: not best botanist; best Japanese botanist specializing in Zen gardens).
7. **Insight tests**: counterintuitive (would other CTOs argue?), specific (exact tools/
   patterns), quantifiable (before/after numbers), demonstrable (showable in 5 minutes).
8. **Evidence honesty.** Actions > words > database records. Aggregator/API data (Blitz,
   FullEnrich) never corroborates itself — load-bearing claims need the person's or
   company's OWN public artifacts. Date every claim; label estimates as estimates.
9. **Dropping a bad fit is success.** Never pad a list to hit a count.
