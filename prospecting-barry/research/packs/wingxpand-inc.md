# Evidence pack: WINGXPAND INC.
slug: wingxpand-inc | domain: wingxpand.com | HQ: St. Louis, Missouri US | employees(LI): 19 | founded:  | stage_band: revenue_stage
sources: sbir
industry(LI): Aviation and Aerospace Component Manufacturing | specialties: drones, manufacturing, uas, ai, machine learning, iot, uav, aircraft, integration, defense, agriculture, public safety, inspection, utility
channel notes: SBIR PhII x1 [2023] Air Force latest: Rucksack Portable Fixed-Wing sUAS Streamlines Logistics and CONOPS hq: St. Louis,MO emp: 10
own-language word count: ~881

## Their own words (website via Exa, livecrawl-preferred)

URL: https://wingxpand.com
WingXpand | Smart Planes Built in America for Defense & Disaster Response

Rapidly deployable smart robotics that protect lives, prevent risks, and improve response.

# Automatic AI Detection.Instant alerts.

Get Started

## Made for the missions that matter.

‍Defense. Security. Inspection.‍WingXpand’s U.S.-built smart planes expand from a backpack with automatic detection and instant alerts, delivering AI-powered awareness where it’s needed most. Our customizable payloads are designed to change as fast as the mission does.

Learn More

## Smaller pack. Smarter plane.

The only U.S.-built smart plane that expand from a backpack in seconds —For automatic AI detection, real-time alerts, and customizable payloads.

xRAISee Through the Unknown.‍Full autonomy, real-time AI detections, & customizable payloads — unlocking new missions for the future.Learn More

## More clarity. Less burden.

AI detection filters the noise and delivers only what matters — helping crews make faster, clearer decisions.Affordable by design, WingXpand makes advanced drone technology accessible and scalable across defense, agriculture, disaster response, and inspection.erage automation to move fast, while always giving customers a human, helpful experience.

Learn More

## Trusted where it counts.

WingXpand is working with U.S. defense, agriculture, security, and industry partners to expand what’s possible in the field today.

Latest News

Training

# We work where you do.

### Defense

Rapidly deployable, multi-mission utility.

Learn more

### Emergency

Quick situational awareness on demand.

Learn more

### Agriculture

Observe fields & livestock preventatively.

Learn more

### Inspection

Monitor & assess infrastructure routinely.

Learn more

## Contact Us

[email]

Your information is submitted succesfully!

Oops! Something went wrong while submitting the form.

URL: https://wingxpand.com/xrai
Smart Planes | xRAI

AI Detect to Alert Speed

1 sec

Max Flight Time

2 Hrs

Max Custom Payload

7 lbs

Time to Deploy

2 min

Pricing

## Meet xRAI

Your mission never stays the same. Neither should your smart plane.

Customize your smart plane for your mission.

## xRAI-D

Fixed Wing UAS - Base Model

Max Range. Max Value.

$14,990

Upgradeable as Your Mission Grows.

This includes...

NDAA Compliant System

Flight Autonomy

Situational Awareness Camera

Standard Range Radio

Carry Case & 1 Battery

Get started

## xRAI-SE

eVTOL UAS - Starter Bundle

Pro Performance + AI insights.

$49,990

Ready for your Mission.

All standard xRAI-D features, plus:

Advanced Edge AI Computer

Vertical Takeoff Kit

Rugged Handheld Controller

Spare Battery & Extra Enclosure

Fast Charger

Get started

Popular

## xRAI

eVTOL UAS - Starter Bundle

Most Advanced AI + Versatile.

$83,990

Ultimate Multi-Mission Smart Plane.

All standard xRAI-SE features, plus:

Max Payloads Available

Aerospace-Grade Composite Wings

Enhanced AI Detections Available

Dropping Capability

Infrared Light Kit

Get started

From agriculture and public safety to utilities and tactical response — choose the path that fits your mission and see how one platform can do more for you.

## Contact Us

[email]

Your information is submitted succesfully!

Oops! Something went wrong while submitting the form.

URL: https://wingxpand.com/customize
Customize | Payloads

# Camera Options

For Surveillance and Reconnaissance

Nighthawk 2-UZNDAA Compliant360 Degree Gimbal, Day/ Night Vision, Zoom: X40 (X20 + X2 digital); Thermal Resolution: 1280x720

For Inspection, Public Safety, and Mapping

Colibri2NDAA Compliant360 Degree Gimbal, Day/ Night Vision, Zoom: X40 (X20 + X2 digital); Thermal Resolution: 640x480

For Inspection, Public Safety, & Defense

Hadron 640R‍NDAA CompliantDay/ Night Vision (RGB + Thermal)

For Survey, Mapping, and Inspection

ILX-LR1‍(61MP Full-Frame, UHD 4K60)

For Precision Agriculture and Crop Insurance

Survey3W(Orange+Cyan+NIR (OCN, NDVI)) Additional Options Available

For Precision Agriculture, Research, and Crop Insurance

RedEdge(RGB+ red edge + near-IR)

Custom Integrations

Let us help you expand what’s possible with drones!

Services

## Your Partner in the Field

### Analysis Reports

Whether you want us to do the analysis or prefer the raw data — We deliver insights you need right to your door.

Learn More

### Training

Our experts are ready to train your team to operate our smart planes to maximize business value.

Learn More

### Consultation

Just getting started with drones? Let WingXpand help you get off the ground and into the air.

Learn More

URL: https://wingxpand.com/software
Smart Software

# Smart AI Software

WingXpand's software is your go-to solutionfor planning and executing autonomousmissions with ease. It empowers you toswiftly translate intelligence into actionableplans, ensuring seamless execution every time.

Learn More

## WHAT ARE OURSOFTWARECAPABILITIES?

Discover effortless mission planning and execution with WingXpand’s Edge AI software. Our platform features intuitive controls and user-friendly interfaces, ensuring a seamless experience.

## Feature Overview

### Autonomous Operation

Flies by itself from launch, to mission, to landing.

### Real-Time Detection

Real-time AI enabled detection software allows you to quickly and accurately detect items of interest without user input.

### Edge AI

On-board smart computers enable autonomy and real-time detections without reliance on internet connectivity or timely post-processing.

### Open Architecture

Open systems architecture design allows for seamless integration of advanced new capabilities from our ecosystem of partners.

### Secure Mesh Network

Enhanced reliability, scalability, and coverage, ensures seamless connectivity in the widest range of environments.

### Radio Silent Mode

Improved communication reliability in sensitive or hostile environments and failsafe if connectivity lapses for any reason.

## LinkedIn about

WingXpand is expanding what’s possible with autonomous systems with our 8ft smart plane that expands from a backpack to help farmers grow more yield, public safety officials respond to disasters faster, and keep soldiers safe.

## SBIR/STTR record (their own abstracts)

### Rucksack Portable Fixed-Wing sUAS Streamlines Logistics and CONOPS — Air Force 2023 ($1,249,496)
What if an airplane could fit within a rucksack, deploy in less than two minutes, fly for hours, and automatically detect threats in real time? Now it can with the highly capable and easy to use WingXpand™ small unmanned aerial system (sUAS), enabled by o

## News (Exa, top 3)
- Award | SBIR () https://www.sbir.gov/awards/209413
  Award | SBIR Skip to main content

# Award

Portfolio Data

Back to Award Search

## Expeditionary Edge AI Enabled Battle Network from a Pack Portable Fixed-Wing UAS

### Awardee

#### WINGXPAND INC.

911 WASHINGTON AVE STE 501SAINT LOUIS, MO, 63101-1272USA

Award Year: 2024

UEI: LB9YEL5LJKV8

HUBZone Owned: No

Woman Owned: Yes

Socially and Economically Disadvantaged: Yes

Congressional District: N/A

Tagged as:

SBIR

Phase I

### Awarding Agency

DOD

Branch: ARMY

Total Award Amount: $249,
- Award | SBIR () https://www.sbir.gov/awards/204407
  Award | SBIR Skip to main content

# Award

Portfolio Data

Back to Award Search

## Rucksack Portable Fixed-Wing sUAS Streamlines Logistics and CONOPS

### Awardee

#### WINGXPAND INC.

https://www.sbir.gov/portfolio/2270755 911 WASHINGTON AVE STE 501SAINT LOUIS, MO, 63101-1272USA

Award Year: 2023

UEI: LB9YEL5LJKV8

HUBZone Owned: No

Woman Owned: Yes

Socially and Economically Disadvantaged: Yes

Congressional District: N/A

Tagged as:

SBIR

Phase II

### Awarding Agency

DOW

Branch: USAF

- Award | SBIR () https://www.sbir.gov/awards/207163
  Award | SBIR Skip to main content

# Award

Portfolio Data

Back to Award Search

## Accelerated Target Engagement with a Droppable Payload Integration on a Rucksack Portable Fixed-Wing UAS

### Awardee

#### WINGXPAND INC.

https://www.sbir.gov/portfolio/2270755 911 WASHINGTON AVE STE 501SAINT LOUIS, MO, 63101-1272USA

Award Year: 2023

UEI: LB9YEL5LJKV8

HUBZone Owned: No

Woman Owned: Yes

Socially and Economically Disadvantaged: Yes

Congressional District: N/A

Tagged as:

SBIR

Phase I

##

## Open roles (Blitz, live)
- Marketing & Content Specialist - Technology ({'city': 'Chicago', 'country_code': 'US'})
- UAS / Drone Pilot & Training Specialist | Defense ({'city': 'Chicago', 'country_code': 'US'})
- Senior - Principal Electrical Engineer - Robotics ({'city': 'Chicago', 'country_code': 'US'})
- Project Manager – Robotics Hardware & Software ({'city': 'Greater Chicago Area', 'country_code': 'US'})
- UAS / Drone Pilot and Customer Success Specialist - Defense ({'city': 'Greater Chicago Area', 'country_code': 'US'})
- UAV / Drone Pilot and Defense Operations Specialist ({'city': 'Schaumburg', 'country_code': 'US'})
- UAS / Drone Pilot and Customer Success Specialist - Defense ({'city': 'Schaumburg', 'country_code': 'US'})
- Manufacturing Technician – Drone Assembly & Electronics ({'city': 'Chicago', 'country_code': 'US'})
- Manufacturing Technician – Drone Assembly & Electronics ({'city': 'Chicago', 'country_code': 'US'})
- UAV / Drone Pilot and Customer Success Specialist - Defense ({'city': 'Greater Chicago Area', 'country_code': 'US'})

---
# Context for the scoring agent (do not re-search)

## Barry Hess niche
Defense and intelligence technology companies in the $5M-$10M range whose proven prototype is stuck between demo and fielded deployment, guided by a CTO who fielded a patented tactical Android app platform with operators in theater and now engineers deployment architecture and acquisition advocacy from day one.

**EDP:** When months-to-field exceeds the funded months remaining on the contract, the program dies in the DoD valley of death - and for a $5M-$10M defense tech company, one dead transition is often the company.

## Deployed-reality checklist (score absence against this)
- **DDIL**: denied/disconnected/intermittent/low-bandwidth comms. Canonical example: an AI
  crowd-detection startup streamed 30fps video; the contested environment had bandwidth for
  1 frame every 5 seconds. One architecture change (frame-rate decoupling) saved the project.
- **SWaP-C**: size, weight, power, cost on the platform actually carried/mounted.
- **ATO/RMF**: Authority to Operate on government networks; STIGs, POA&Ms, IL4/IL5.
- **CMMC / NIST 800-171**: company-level compliance before the contract dies.
- **ITAR/export**: controlled data handling.
- **Operator training & sustainment**: who trains the E-5, who fixes it in the field, spares,
  battery logistics, GFE integration.
- **Advocacy chain**: a champion above the operator level; operators loving it is not adoption.
- **Prime/flow-down mechanics**: subcontract structure, government back-end integration.
- **TAK ecosystem**: if it touches situational awareness and never mentions ATAK/TAK, ask why.


## Doctrine
# Targeting doctrine brief (inject into every dossier & company agent — do NOT re-read the corpus)

Distilled from Jordan Crawford's Cannonball/Blueprint corpus, 2026-07-23.

1. **Pain-Qualified Segment beats ICP.** Never target firmographics ("50-200 person SaaS").
   Target companies in an observable painful SITUATION right now. The situation must be
   provable from public data.
2. **Existential Data Point (EDP).** For each niche, name the single number that kills the
   company if the problem stays unfixed (e.g. equipment utilization 60% vs 80% margins;
   fleet utilization <70% = crisis). Every dossier must state the EDP of its CTO's target
   situation: "what does it cost a company, in what metric, to leave this unfixed?"
3. **Jobs are confessions.** A job posting is a company publicly describing its own pain.
   We match jobs whose EXISTENCE signals a problem the CTO has already fixed — not jobs the
   CTO would apply to. "First Head of Engineering" = no technical leadership. "Integration
   Engineer (Epic, HL7)" = drowning in healthcare plumbing.
4. **The message is a redescription of the targeting.** If targeting is precise, the message
   writes itself by describing what we found. The reader should think "how do they know that?"
5. **Message-as-gift (GTM Shift letter standard).** Conversational, unhurried, 4–6 short
   sentences. Opens with a verified surprising fact. Every claim traces to a named public
   source. Valuable even if never answered. One light question, under 15 words. Bans: "I
   noticed", flattery, "hope you're well", meeting asks, personal details about the recipient
   as the opener. Lead with the CTO's expertise matched to the company's evident problem.
6. **Niche = Industry × Situation × Unique Angle** — all three specific (Ben Horowitz
   principle: not best botanist; best Japanese botanist specializing in Zen gardens).
7. **Insight tests**: counterintuitive (would other CTOs argue?), specific (exact tools/
   patterns), quantifiable (before/after numbers), demonstrable (showable in 5 minutes).
8. **Evidence honesty.** Actions > words > database records. Aggregator/API data (Blitz,
   FullEnrich) never corroborates itself — load-bearing claims need the person's or
   company's OWN public artifacts. Date every claim; label estimates as estimates.
9. **Dropping a bad fit is success.** Never pad a list to hit a count.
