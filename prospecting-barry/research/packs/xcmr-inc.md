# Evidence pack: XCMR INC
slug: xcmr-inc | domain: xcmr.co | HQ: ,   | employees(LI): 10 | founded: 2020 | stage_band: revenue_stage
sources: sbir
industry(LI): Biotechnology Research | specialties: 
channel notes: SBIR PhII x1 [2022] Air Force latest: Next Generation Personal Protective Equipment for Real-time Inactivation of Airborne Biological Threats hq: Penn Valley,PA emp: 8
own-language word count: ~654

## Their own words (website via Exa, livecrawl-preferred)

URL: https://xcmr.co
Skip to content

## XCMR leads in biosecurity innovation incorporating UVC into wearable, portable and stationary devices to prevent the transmission of infectious disease.

## XCMR’s IoT cloud enables AI-driven predictive analytics to improve visibility, response and preparedness for scaled infection transmission resiliency.

## Breakthrough achievements.

Based on science and research.
Powered by a passion to improve health security for all.

Advanced Next-Generation
Biological Threat Protection
from XCMR

 Learn more 

```json
{"@context":"https:\/\/schema.org","@graph":[{"@type":"WebPage","@id":"https:\/\/xcmr.co\/","url":"https:\/\/xcmr.co\/","name":"Landing page - XCMR","isPartOf":{"@id":"https:\/\/xcmr.co\/#website"},"about":{"@id":"https:\/\/xcmr.co\/#organization"},"datePublished":"2024-08-26T12:53:17+00:00","dateModified":"2025-05-21T17:32:04+00:00","breadcrumb":{"@id":"https:\/\/xcmr.co\/#breadcrumb"},"inLanguage":"en-US","potentialAction":[{"@type":"ReadAction","target":["https:\/\/xcmr.co\/"]}]},{"@type":"BreadcrumbList","@id":"https:\/\/xcmr.co\/#breadcrumb","itemListElement":[{"@type":"ListItem","position":1,"name":"Home"}]},{"@type":"WebSite","@id":"https:\/\/xcmr.co\/#website","url":"https:\/\/xcmr.co\/","name":"XCMR","description":"","publisher":{"@id":"https:\/\/xcmr.co\/#organization"},"potentialAction":[{"@type":"SearchAction","target":{"@type":"EntryPoint","urlTemplate":"https:\/\/xcmr.co\/?s={search_term_string}"},"query-input":{"@type":"PropertyValueSpecification","valueRequired":true,"valueName":"search_term_string"}}],"inLanguage":"en-US"},{"@type":"Organization","@id":"https:\/\/xcmr.co\/#organization","name":"XCMR","url":"https:\/\/xcmr.co\/","logo":{"@type":"ImageObject","inLanguage":"en-US","@id":"https:\/\/xcmr.co\/#\/schema\/logo\/image\/","url":"https:\/\/yeaxwgje.elementor.cloud\/wp-content\/uploads\/2024\/08\/xcmr-white-logo.png","contentUrl":"https:\/\/yeaxwgje.elementor.cloud\/wp-content\/uploads\/2024\/08\/xcmr-white-logo.png","width":1000,"height":323,"caption":"XCMR"},"image":{"@id":"https:\/\/xcmr.co\/#\/schema\/logo\/image\/"}}]}

```

URL: https://xcmr.co/about/
Skip to content

## About XCMR

## The COVID-19 pandemic exposed vulnerabilities in society's ability to respond swiftly to a global public health threat. Recognizing the urgent need for next-generation personal protective equipment (PPE), XCMR was founded to develop breakthrough technology for respiratory protection and infection control. Our goal: commercialize innovative devices that leverage safe, continuous UVC irradiation to neutralize pathogens.

## Today, XCMR's foundational research and development, alongside academic, commercial, and US Government partners, have sparked a growing ecosystem dedicated to Electromagnetic Energy for Infection Transmission Resiliency (EMITR™). This broad-based platform harnesses novel applications of UVC irradiation, combined with data and sensor fusion, to combat airborne biological threats.

## While some companies use UVC to disinfect built environments, XCMR focuses on close-range protection, targeting the primary zone of transmission and infection. Our Near-field Infection Protection (NIP)™ devices provide biosafety barriers against aerosol transmission in various stationary and mobile settings, wherever people or animals interact closely.

## XCMR's next generation of mobile biodefense and biosecurity solutions are poised to revolutionize protection against biological threats. By integrating the safest and most effective spectrum of UVC energy into wearable, portable, and stationary NIP devices, XCMR can significantly reduce exposure to airborne pathogens. We create a controlled, continuous, "invisible" bubble of irradiated air, maximizing infection prevention and mitigating the risks associated with antimicrobial resistance.

## Moreover, XCMR's NIP technology is expected to drastically reduce environmental waste from single-use masks, lower the incidence and prevalence of respiratory diseases (e.g., flu, RSV, hMPV), and potentially serve as a universal safety device akin to seat belts for public health. This innovative approach is also far more capital and time efficient than traditional medical countermeasures like antibiotics and vaccines.

## With a highly experienced, multidisciplinary team of scientific and business experts, XCMR's mission is clear: to transform biodefense and biosecurity efforts, providing robust protection for military and civilian populations alike. Our solutions ensure optimal efficacy in real-world conditions while prioritizing human safety, comfort, affordability, and accessibility 

URL: https://xcmr.co/#content
Skip to content

## XCMR leads in biosecurity innovation incorporating UVC into wearable, portable and stationary devices to prevent the transmission of infectious disease.

## XCMR’s IoT cloud enables AI-driven predictive analytics to improve visibility, response and preparedness for scaled infection transmission resiliency.

## Breakthrough achievements.

Based on science and research.
Powered by a passion to improve health security for all.

Advanced Next-Generation
Biological Threat Protection
from XCMR

 Learn more 

```json
{"@context":"https:\/\/schema.org","@graph":[{"@type":"WebPage","@id":"https:\/\/xcmr.co\/","url":"https:\/\/xcmr.co\/","name":"Landing page - XCMR","isPartOf":{"@id":"https:\/\/xcmr.co\/#website"},"about":{"@id":"https:\/\/xcmr.co\/#organization"},"datePublished":"2024-08-26T12:53:17+00:00","dateModified":"2025-05-21T17:32:04+00:00","breadcrumb":{"@id":"https:\/\/xcmr.co\/#breadcrumb"},"inLanguage":"en-US","potentialAction":[{"@type":"ReadAction","target":["https:\/\/xcmr.co\/"]}]},{"@type":"BreadcrumbList","@id":"https:\/\/xcmr.co\/#breadcrumb","itemListElement":[{"@type":"ListItem","position":1,"name":"Home"}]},{"@type":"WebSite","@id":"https:\/\/xcmr.co\/#website","url":"https:\/\/xcmr.co\/","name":"XCMR","description":"","publisher":{"@id":"https:\/\/xcmr.co\/#organization"},"potentialAction":[{"@type":"SearchAction","target":{"@type":"EntryPoint","urlTemplate":"https:\/\/xcmr.co\/?s={search_term_string}"},"query-input":{"@type":"PropertyValueSpecification","valueRequired":true,"valueName":"search_term_string"}}],"inLanguage":"en-US"},{"@type":"Organization","@id":"https:\/\/xcmr.co\/#organization","name":"XCMR","url":"https:\/\/xcmr.co\/","logo":{"@type":"ImageObject","inLanguage":"en-US","@id":"https:\/\/xcmr.co\/#\/schema\/logo\/image\/","url":"https:\/\/yeaxwgje.elementor.cloud\/wp-content\/uploads\/2024\/08\/xcmr-white-logo.png","contentUrl":"https:\/\/yeaxwgje.elementor.cloud\/wp-content\/uploads\/2024\/08\/xcmr-white-logo.png","width":1000,"height":323,"caption":"XCMR"},"image":{"@id":"https:\/\/xcmr.co\/#\/schema\/logo\/image\/"}}]}

```

## LinkedIn about

XCMR is a pioneering techbio company specializing in breakthrough biosecurity innovations in non-pharmaceutical medical countermeasures for biodefense. Together with its academic, industry and government partners, we integrate proprietary UV-C technology into wearable, portable, and stationary devices for personal protection and to prevent the transmission of infectious diseases. Our IoT cloud platform leverages AI-driven predictive analytics to enhance visibility, response, and preparedness for scalable infection transmission resiliency.

## SBIR/STTR record (their own abstracts)

### Next Generation Personal Protective Equipment for Real-time Inactivation of Airborne Biological Threats — Air Force 2022 ($748,503)
XCMR is developing a safe and wearable, Far UV-C biodefense device that is highly effective at inactivating viruses, bacteria, and other airborne pathogens, thus providing a continuous "invisible" bubble of clean air supporting an airman's respiration. XCMR's Far UV-C (range between 221-223 nm with a peak at 222 nm) electromagnetic technology provides significant advantages over current UV germicidal light sources (254-280 nm) that have been used historically to disinfect surfaces and volumes of air but are harmful to humans. XCMR's development of a safe Far UV-C system is transformative and can be deployed in a broad range of military applications to protect against human exposure to airborne pathogens. XCMR’s modular technology provides superior protection against biological threats and pathogens in a variety of form factors that easily integrate into existing Mission Oriented Protective Postures (MOPP), safety, and communications equipment. Individual Airmen in operational and expeditionary environments, from pilots to maintainers to aeromedical evacuation personnel, will benefit from unencumbered protection against biological weapons and natural infectious disease threats.

## News (Exa, top 3)
- Award | SBIR () https://www.sbir.gov/awards/209153
  Award | SBIR Skip to main content

# Award

Portfolio Data

Back to Award Search

## Next-Gen Far UV-C irradiation technology: Mitigating Surgical Site and Wound Infection Risk

### Awardee

#### XCMR INC

219 RIGHTERS MILL RDPENN VALLEY, PA, 19072-1314USA

Award Year: 2024

UEI: CSUGTGL9HMN9

HUBZone Owned: No

Woman Owned: No

Socially and Economically Disadvantaged: No

Congressional District: N/A

Tagged as:

SBIR

Phase II

### Awarding Agency

DOD

Branch: USAF

Total Award Amount: $1,249,
-  () https://xcmr.co/xcmr-secures-1-25m-afwerx-grant-to-combat-surgical-site-infections/
  ---
title: XCMR Secures $1.25M AFWERX Contract to Combat Surgical Site Infections - XCMR
description: XCMR has been awarded a $1.25 million, 21-month Direct to Phase II Small Business Innovation Research (SBIR) contract from the U.S. Air Force Research Laboratory (AFRL), through its AFWERX program.
image: https://xcmr.co/wp-content/uploads/2024/01/us-airforce.jpg
---

[Skip to content](#content) 

* [ News Feed ](https://xcmr.co/news-feed)
* [ January 18, 2024 ](https://xcmr.co/2024/01/18/)
* [A
- XCMR INC - Far UV-C Light Device May Help Prevent Surgical and Battlefield Wound Infections Award | SBIR.org () https://sbir.org/awards/dod-FA8649-24-P-0393-1
  XCMR INC - Far UV-C Light Device May Help Prevent Surgical and Battlefield Wound Infections Award | SBIR.org

Phase II $1249822 Department of Defense

# Far UV-C Light Device May Help Prevent Surgical and Battlefield Wound Infections

XCMR INC· January 2024

XCMR is developing a Far UV-C light device that appears capable of killing bacteria and viruses in surgical wounds and combat injuries, potentially reducing infection rates that currently reach 34% in battlefield casualties.

## What is this

## Open roles
(no jobs data found)

---
# Context for the scoring agent (do not re-search)

## Barry Hess niche
Defense and intelligence technology companies in the $5M-$10M range whose proven prototype is stuck between demo and fielded deployment, guided by a CTO who fielded a patented tactical Android app platform with operators in theater and now engineers deployment architecture and acquisition advocacy from day one.

**EDP:** When months-to-field exceeds the funded months remaining on the contract, the program dies in the DoD valley of death - and for a $5M-$10M defense tech company, one dead transition is often the company.

## Deployed-reality checklist (score absence against this)
- **DDIL**: denied/disconnected/intermittent/low-bandwidth comms. Canonical example: an AI
  crowd-detection startup streamed 30fps video; the contested environment had bandwidth for
  1 frame every 5 seconds. One architecture change (frame-rate decoupling) saved the project.
- **SWaP-C**: size, weight, power, cost on the platform actually carried/mounted.
- **ATO/RMF**: Authority to Operate on government networks; STIGs, POA&Ms, IL4/IL5.
- **CMMC / NIST 800-171**: company-level compliance before the contract dies.
- **ITAR/export**: controlled data handling.
- **Operator training & sustainment**: who trains the E-5, who fixes it in the field, spares,
  battery logistics, GFE integration.
- **Advocacy chain**: a champion above the operator level; operators loving it is not adoption.
- **Prime/flow-down mechanics**: subcontract structure, government back-end integration.
- **TAK ecosystem**: if it touches situational awareness and never mentions ATAK/TAK, ask why.


## Doctrine
# Targeting doctrine brief (inject into every dossier & company agent — do NOT re-read the corpus)

Distilled from Jordan Crawford's Cannonball/Blueprint corpus, 2026-07-23.

1. **Pain-Qualified Segment beats ICP.** Never target firmographics ("50-200 person SaaS").
   Target companies in an observable painful SITUATION right now. The situation must be
   provable from public data.
2. **Existential Data Point (EDP).** For each niche, name the single number that kills the
   company if the problem stays unfixed (e.g. equipment utilization 60% vs 80% margins;
   fleet utilization <70% = crisis). Every dossier must state the EDP of its CTO's target
   situation: "what does it cost a company, in what metric, to leave this unfixed?"
3. **Jobs are confessions.** A job posting is a company publicly describing its own pain.
   We match jobs whose EXISTENCE signals a problem the CTO has already fixed — not jobs the
   CTO would apply to. "First Head of Engineering" = no technical leadership. "Integration
   Engineer (Epic, HL7)" = drowning in healthcare plumbing.
4. **The message is a redescription of the targeting.** If targeting is precise, the message
   writes itself by describing what we found. The reader should think "how do they know that?"
5. **Message-as-gift (GTM Shift letter standard).** Conversational, unhurried, 4–6 short
   sentences. Opens with a verified surprising fact. Every claim traces to a named public
   source. Valuable even if never answered. One light question, under 15 words. Bans: "I
   noticed", flattery, "hope you're well", meeting asks, personal details about the recipient
   as the opener. Lead with the CTO's expertise matched to the company's evident problem.
6. **Niche = Industry × Situation × Unique Angle** — all three specific (Ben Horowitz
   principle: not best botanist; best Japanese botanist specializing in Zen gardens).
7. **Insight tests**: counterintuitive (would other CTOs argue?), specific (exact tools/
   patterns), quantifiable (before/after numbers), demonstrable (showable in 5 minutes).
8. **Evidence honesty.** Actions > words > database records. Aggregator/API data (Blitz,
   FullEnrich) never corroborates itself — load-bearing claims need the person's or
   company's OWN public artifacts. Date every claim; label estimates as estimates.
9. **Dropping a bad fit is success.** Never pad a list to hit a count.
