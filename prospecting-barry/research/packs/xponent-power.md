# Evidence pack: XPonent Power
slug: xponent-power | domain: xponentpower.com | HQ: Fremont, California US | employees(LI): 9 | founded:  | stage_band: unknown
sources: vc:scout
industry(LI): Renewable Energy Equipment Manufacturing | specialties: 
channel notes: retractable solar power systems
own-language word count: ~1537

## Their own words (website via Exa, livecrawl-preferred)

URL: https://xponentpower.com
High Power Retractable Solar for Vehicles, Trailers, and Homes

# Retrofit Retractable Solar Panels for Go-Anywhere Power

## Up to 56% More Power than Rooftop Solar!

### “Xpanse™ is without competition. It’s so much more innovative and better made than anything else in the RV industry.”

#### – Andy Ball, RV Customer

## As Featured In

### “Xpanse is an amazing and solid product”

#### – Brian D., Service Manager, Mike Thompson’s RV

## Robust Panel Design

### The Xpanse™ solar awning features rigid and robust solar panels that are designed for performance and durability. Built using traditional solar materials, these glass-based solar panels are very shade tolerant and designed to last for years. Learn More

https://xponentpower.com/xpanse-solar-system/

## Highly Wind Tolerant

### The awning has an innovative design that allows wind to pass through gaps between the panels when the awning is retracted slightly, which enables impressive stability in windy conditions. In extreme weather conditions, the awning automatically retracts completely. Learn More

https://xponentpower.com/xpanse-solar-system/

### “The build quality of the system is top notch”

#### – AJ W., Lead Technician, Troy’s Custom Solutions

## Quiet And Clean

### Enjoy the serene outdoors free of noise, smelly fumes, and other restrictions and limitations that come with a generator. Xpanse™ produces power every day, no matter how long you are off-grid. Learn More

https://xponentpower.com/xpanse-solar-system/

### “Its plug and play. Very easy to use.”

#### – Ben W., Co-owner, RV Solar Guys

## Super Easy to Operate

### Push a button to open it and start generating energy in no time. Push a button to close it, and you’re ready to hit the road. Learn More

https://xponentpower.com/xpanse-solar-system/

### “It’s like bringing science fiction to life.”

#### – Aaron R., Technician, Sun City RV

## 1200W Of Power

### Xpanse™ is built with a high efficiency solar cell technology and an innovative panel design to deliver a power rating of 1200Watts. That’s ample power to run most of the appliances in your RV without plug-ins. Learn More

https://xponentpower.com/xpanse-solar-system/

### “I couldn’t find anything else that does what this does.”

#### – Andy B, RV Customer

## Compact

### Xpanse™ integrates beautifully and seamlessly with your RV. When you are ready to go, the awning retracts neatly and securely into an elegant enclosure. Learn More

https://xponentpower.com/xpanse-solar-system/

## Why choose Xpanse™ over other options?

## More power. More time off-grid. Superior value

## SHORE POWER

- Restricted travel
- Crowded
- High campground fees

## GENERATOR

- Loud
- Polluting
- Requires constant re-fueling
- Use restricted in some parks

## ROOFTOP SOLAR

- Limited power due to limited of roof space
- Exposed to potential damage all the time
- Shading on a part of panel often prevents rest of panel from generating power
- Will not generate power if covered, eg by snow
- Can overheat quickly, limiting production, due to lack of airflow

## XPANSE™

- Takes up only 6″ of roof corner, freeing up valuable space
- Rugged metal enclosure protects from debris
- Innovative by-pass design allows for continued power generation even when panels shaded
- Double-slided solar panels generate power even if top covered, eg by snow
- 360 degree airflow maximizes solar generation
- Doubles as an awning for light weather conditions
- Integrated mobile app enables you to remotely operate, and monitor and manage real-time energy generation and battery draw

## Media Coverage

“This Cool New Solar Awning Could Revolutionize Camping Trailers” –

“The Xpanse™ Solar Awning…can provide up to 1.2 kW of solar power to power up an RV full of appliances while it shades your comfy seats below.” –

“…innovative option certainly takes solar to new places.” –

“…revolutionary Xpanse™ solar awning may have what it takes to turn the market on it’s head.” –

”Ready for Limitless Off-Grid Adventures?” –

## Customer

“I wanted to add a lot of solar but couldn’t figure out how to do it. I’d have to buy a new RV because I’ve run out of rooftop space. Xpanse™ neatly fits on the small amount of space right on edge of the vehicle. I haven’t been able to find anything else that does what Xpanse™ does.“

“Very, very high quality. Wow. That’s a work of art.”

“I can see the complexity of the parts, pieces, and joints. Some really clever engineering went into this.”

“The by-pass design is ingenious. If a small shadow fell on one part of one of my panels, they all stop working, so that by-pass feature is fantastic.”

“A great way to have more real estate as far as solar goes… there’s only so much room on the roof of an RV”

URL: https://xponentpower.com/xpanse-solar-system/
Xpanse Solar System | Xponent Power

Xpanse™ by Xponent

## BIG POWER IN A SMALL PACKAGE

Retrofit solar power in an ultra-low footprint for commercial and service vehicles, RVs, motorhomes, trailers, mobile command centers, and food trucks.

https://xponentpower.com/xpanse-solar-system/

https://xponentpower.com/xpanse-solar-system/

https://xponentpower.com/xpanse-solar-system/

https://xponentpower.com/xpanse-solar-system/

← →

←

→

# Xpanse 1200 Watt Solar System

Imagine pulling up to a remote mountain vista, desert plateau, or forest clearing, and with the push of a button, your vehicle transforms into a self-sustaining powerhouse. That’s not science fiction. That’s Xpanse — the world’s first retractable solar system that’s redefining access to clean, reliable off-grid energy for RVs, trailers, and commercial and military vehicles.

## As Powerful As 1600 Watt Solar

Xpanse is the most powerful solar on the market today.

In comparison tests, Xpanse produces 30-50% more energy than the typical vehicle rooftop solar panel and generates as much power as a 1600 watt system. Learn why we outperform below.

List Price $9,900

Xceptional Summer Savings Price (Limited Time Only) $8,400

Additional Savings

Net Price After IRS Investment Tax Credit* USD $5,880

REFUNDABLE DEPOSIT DUE TODAY $100

Order now for $100

Request Info

Once we receive your order, a Customer Success Specialist will contact you to conduct a Vehicle Fit Check, confirm your requirements, and determine the best location for your installation.

* Customer responsible for verifying IRS eligibility

## What's Included

#### Includes:

- 50 bi-facial high-efficiency solar panels
- Industrial-strength motors
- High durability fiberglass wire-core belts
- Industrial-strength latches with electronic locks
- Rugged aluminum shell
- Heavy duty double-walled metal arms
- SmartSensors for wind and obstruction detection
- Multi-functional user control panel
- Roadside kit
- 3-year warranty

#### Not Included:

- Charge controller, inverter, battery monitor, and battery

## What's Included

#### Includes:

- 50 bi-facial high-efficiency solar panels
- Industrial-strength motors
- High durability fiberglass wire-core belts
- Industrial-strength latches with electronic locks
- Rugged aluminum shell
- Heavy duty double-walled metal arms
- SmartSensors for wind and obstruction detection
- Multi-functional user control panel
- Roadside kit
- 3-year warranty

#### Not Included:

- Charge controller, inv

URL: https://xponentpower.com/technology/
Technology | Xponent Power

# Our Technology

## Designed With Focus on Quality

Quality, reliability, performance and safety are the core principles that underpin the design methodologies and product development efforts at Xponent™ Power. We have not only designed Xpanse™ with these attributes in mind but also subjected the product to extensive testing to ensure its reliable performance in the field.

## RIGID AND DURABLE SOLAR PANEL

Xpanse™ combines high efficiency silicon solar cell technology with an innovative panel design to deliver an ultra-compact, high-performance, off-grid solar system kit. The robust glass-based solar panel uses traditional and proven materials used in the construction of standard residential and commercial solar modules to ensure high durability without sacrificing any performance.

The uniqueness of the Xpanse™ solar panel lies in the design of an ultra-compact form factor that enables its seamless integration with the mechanical components in the awning and a very small footprint when the awning is closed.

## Wind Tolerant Design

Traditional RV awnings consist of a very large single piece of fabric that experiences high wind uplift even at low wind speeds, very much like a sail on a boat. In contrast, the Xpanse™ solar awning has a patented design that creates small gaps between the solar panels when the awning is slightly retracted. These gaps allow wind to pass between the panels, thereby greatly reducing the wind uplift and making the awning very wind-tolerant.

The awning also has built-in intelligence that can monitor wind speed and automatically trigger a slight retraction when needed. The awning will also completely retract when wind speeds cross a preset threshold to ensure safe operation.

### Rigorously Tested

Designed with a strong focus on quality and performance, the whole awning as well the individual components in the awning have undergone a series of tests to ensure their durability.

### Tested to 45mph wind

The Xpanse™ awning has been subjected to rigorous wind tests both in real life and under simulated conditions. A side-by-side comparison of the Xpanse™ solar awning with a traditional fabric awning shown highlights the effectiveness of the air gaps in the Xpanse™ system. When both awnings were subjected to 25 mph wind speeds for several hours in a windy location, the Xpanse™ awning remained stable while the fabric awning severely rocked back and forth as shown in our video.

In another set of tests, 

URL: https://xponentpower.com/
High Power Retractable Solar for Vehicles, Trailers, and Homes

# Retrofit Retractable Solar

## LinkedIn about

Xponent Power offers Xpanse, a stylish, compact, and retractable solar awning with solar panels for RV motorhomes.

## News (Exa, top 3)
- xponent power (2026-06-29) https://xponentpower.com/
  # Xponent Power (Xponent Power, Inc.)

Xponent Power is a Renewable Energy Equipment Manufacturing company. XPanse is a company that specializes in retrofit retractable solar panels for go-anywhere power. They offer up to 56% more power than rooftop solar and are known for their innovative and affordable products. Xponent Power employs 5 people (-50.0% YoY). Headquartered in Fremont, California, United States, with presence in India. Has $13M in total funding.

## About

Xponent Power offers Xpa
- How Much Solar Power Does Your RV Need? (2025-04-29) https://xponentpower.com/xponent-power-begins-customer-deliveries-ramps-up-commercial-production-and-secures-multi-state-dealership-partnerships/
  How Much Solar Power Does Your RV Need? | Xponent Power Guide

# Xponent™ Power Begins Customer Deliveries, Ramps Up Commercial Production, and Secures Multi-State Dealership Partnerships

- More than $3 million in presales and pipeline for Xponent Power’s Xpanse™ retractable solar systems underscore market demand as production scales
- Dealer partnerships in Arizona, California, and Texas mark early distribution wins across both recreational and commercial vehicle customer segments.
- Rigorous 
- Xponent™ Power Begins Customer Deliveries, Ramps Up Commercial Production, and Secures Multi-State Dealership Partnerships (2025-04-30) https://www.businesswire.com/news/home/20250430016872/en/Xponent-Power-Begins-Customer-Deliveries-Ramps-Up-Commercial-Production-and-Secures-Multi-State-Dealership-Partnerships
  Xponent™ Power Begins Customer Deliveries, Ramps Up Commercial Production, and Secures Multi-State Dealership Partnerships

Apr 30, 2025 6:00 AM Eastern Daylight Time

# Xponent™ Power Begins Customer Deliveries, Ramps Up Commercial Production, and Secures Multi-State Dealership Partnerships

Share

---

- More than $3 million in presales and pipeline for Xponent Power’s Xpanse™ retractable solar systems underscore market demand as production scales
- Dealer partnerships in Arizona, California, 

## Open roles
(no jobs data found)

---
# Context for the scoring agent (do not re-search)

## Barry Hess niche
Defense and intelligence technology companies in the $5M-$10M range whose proven prototype is stuck between demo and fielded deployment, guided by a CTO who fielded a patented tactical Android app platform with operators in theater and now engineers deployment architecture and acquisition advocacy from day one.

**EDP:** When months-to-field exceeds the funded months remaining on the contract, the program dies in the DoD valley of death - and for a $5M-$10M defense tech company, one dead transition is often the company.

## Deployed-reality checklist (score absence against this)
- **DDIL**: denied/disconnected/intermittent/low-bandwidth comms. Canonical example: an AI
  crowd-detection startup streamed 30fps video; the contested environment had bandwidth for
  1 frame every 5 seconds. One architecture change (frame-rate decoupling) saved the project.
- **SWaP-C**: size, weight, power, cost on the platform actually carried/mounted.
- **ATO/RMF**: Authority to Operate on government networks; STIGs, POA&Ms, IL4/IL5.
- **CMMC / NIST 800-171**: company-level compliance before the contract dies.
- **ITAR/export**: controlled data handling.
- **Operator training & sustainment**: who trains the E-5, who fixes it in the field, spares,
  battery logistics, GFE integration.
- **Advocacy chain**: a champion above the operator level; operators loving it is not adoption.
- **Prime/flow-down mechanics**: subcontract structure, government back-end integration.
- **TAK ecosystem**: if it touches situational awareness and never mentions ATAK/TAK, ask why.


## Doctrine
# Targeting doctrine brief (inject into every dossier & company agent — do NOT re-read the corpus)

Distilled from Jordan Crawford's Cannonball/Blueprint corpus, 2026-07-23.

1. **Pain-Qualified Segment beats ICP.** Never target firmographics ("50-200 person SaaS").
   Target companies in an observable painful SITUATION right now. The situation must be
   provable from public data.
2. **Existential Data Point (EDP).** For each niche, name the single number that kills the
   company if the problem stays unfixed (e.g. equipment utilization 60% vs 80% margins;
   fleet utilization <70% = crisis). Every dossier must state the EDP of its CTO's target
   situation: "what does it cost a company, in what metric, to leave this unfixed?"
3. **Jobs are confessions.** A job posting is a company publicly describing its own pain.
   We match jobs whose EXISTENCE signals a problem the CTO has already fixed — not jobs the
   CTO would apply to. "First Head of Engineering" = no technical leadership. "Integration
   Engineer (Epic, HL7)" = drowning in healthcare plumbing.
4. **The message is a redescription of the targeting.** If targeting is precise, the message
   writes itself by describing what we found. The reader should think "how do they know that?"
5. **Message-as-gift (GTM Shift letter standard).** Conversational, unhurried, 4–6 short
   sentences. Opens with a verified surprising fact. Every claim traces to a named public
   source. Valuable even if never answered. One light question, under 15 words. Bans: "I
   noticed", flattery, "hope you're well", meeting asks, personal details about the recipient
   as the opener. Lead with the CTO's expertise matched to the company's evident problem.
6. **Niche = Industry × Situation × Unique Angle** — all three specific (Ben Horowitz
   principle: not best botanist; best Japanese botanist specializing in Zen gardens).
7. **Insight tests**: counterintuitive (would other CTOs argue?), specific (exact tools/
   patterns), quantifiable (before/after numbers), demonstrable (showable in 5 minutes).
8. **Evidence honesty.** Actions > words > database records. Aggregator/API data (Blitz,
   FullEnrich) never corroborates itself — load-bearing claims need the person's or
   company's OWN public artifacts. Date every claim; label estimates as estimates.
9. **Dropping a bad fit is success.** Never pad a list to hit a count.
