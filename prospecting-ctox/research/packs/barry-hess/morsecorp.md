# Evidence pack — morsecorp (for CTO barry-hess)
ATS: greenhouse | liveness: live | feed slug: morsecorp

## CTO niche
Fractional CTO for US Series A/B defense tech startups of roughly 8-30 people building AI/ML, geospatial or sensor products for DoW and Intelligence Community customers - the team built the technology but needs cleared senior leadership (active TS/SCI) to get it procurement-ready and into warfighters' hands.
EDP: Months-to-field versus funded months remaining: when time-to-field outruns the period of performance, the program dies in the valley of death - and at 8-30 people one dead transition is often the company.

## Matched jobs (from the 2026-06-25 snapshot; 'live' = re-verified today)
### Chief Engineer  [score 0.4698, live=True]
apply: https://boards.greenhouse.io/morsecorp/jobs/7589946003?gh_jid=7589946003
posted: 2026-05-05T23:07:49-04:00 | company_does: employee-owned software and algorithm development services for national security | industry: defense | stage: scaleup
signals: strong_title=False gap=True jd_lexicon=True
JD excerpt (snapshot):
```
MORSE Corp is an employee owned, small business based in Cambridge, MA, Arlington, VA, and Seattle, WA with a history of fielding cutting-edge technology. MORSE boasts a specially selected team of scientists, engineers, and software developers to deliver best-in-class technical solutions that solve difficult multidisciplinary problems faced by the US National Security Ecosystem.  

 

### Job Summary:

We are seeking an experienced and talented Chief Engineer to lead our efforts in developing and deploying capabilities for artificial intelligence, machine learning, decision systems, and mission planning applications. This is an exceptional opportunity for a seasoned engineer to drive innovation, collaboration, and excellence in a fast-paced and growing organization.

### Responsibilities:

-

Set the technical vision and direction for programs, collaborating with internal technical staff and external customers

-

Lead multi-disciplinary teams developing algorithmic, software, cloud, and data solutions

-

Manage program execution, ensuring schedule and budget adherence, and overseeing successful program delivery

-

Cultivate strong relationships with external partners and stakeho
```
### Chief Engineer  [score 0.4688, live=True]
apply: https://boards.greenhouse.io/morsecorp/jobs/5845023003?gh_jid=5845023003
posted: 2026-05-05T23:07:49-04:00 | company_does: employee-owned software and algorithm development services for national security | industry: defense | stage: scaleup
signals: strong_title=False gap=True jd_lexicon=True
JD excerpt (snapshot):
```
MORSE Corp is an employee owned, small business based in Cambridge, MA, Arlington, VA, and Seattle, WA with a history of fielding cutting-edge technology. MORSE boasts a specially selected team of scientists, engineers, and software developers to deliver best-in-class technical solutions that solve difficult multidisciplinary problems faced by the US National Security Ecosystem.  

###  

MORSE is seeking an experienced and talented Chief Engineer to lead our efforts in developing and deploying capabilities for artificial intelligence, machine learning, decision systems, and mission planning applications. This is an exceptional opportunity for a seasoned engineer to drive innovation, collaboration, and excellence in a fast-paced and growing organization.

### Responsibilities:

-

Set the technical vision and direction for programs, collaborating with internal technical staff and external customers

-

Lead multi-disciplinary teams developing algorithmic, software, cloud, and data solutions

-

Manage program execution, ensuring schedule and budget adherence, and overseeing successful program delivery

-

Cultivate strong relationships with external partners and stakeholders, advoc
```
### Chief Engineer - AI Test & Evaluation (T&E)  [score 0.4684, live=True]
apply: https://boards.greenhouse.io/morsecorp/jobs/7732702003?gh_jid=7732702003
posted: 2026-05-12T21:54:21-04:00 | company_does: employee-owned software and algorithm development services for national security | industry: defense | stage: scaleup
signals: strong_title=False gap=True jd_lexicon=True
JD excerpt (snapshot):
```
MORSE Corp is an employee owned, small business based in Cambridge, MA, Arlington, VA, and Seattle, WA with a history of fielding cutting-edge technology. MORSE boasts a specially selected team of scientists, engineers, and software developers to deliver best-in-class technical solutions that solve difficult multidisciplinary problems faced by the US National Security Ecosystem.  

At MORSE, our Chief Engineers are the "triple threat" leaders at the heart of our impactful work. For our AI Test & Evaluation (T&E) work, we are seeking a leader who tacitly understands that the measure of AI success is the tangible advantage it provides the warfighter and intelligence analyst. Standalone model performance and standalone system performance are critical foundations to those measures. This role is for the systems-thinker who lives at the intersection of AI effectiveness, human-computer interaction, and applied R&D. You are the expert who can translate a first-principles understanding of complex systems—from model behaviors to user workflows—into engineering-driven frameworks that prove and improve mission outcomes. By setting the "North Star" for your program, you will guide a world-class
```
### Deputy Chief Engineer  [score 0.467, live=True]
apply: https://boards.greenhouse.io/morsecorp/jobs/7511486003?gh_jid=7511486003
posted: 2026-05-05T23:08:29-04:00 | company_does: employee-owned software and algorithm development services for national security | industry: defense | stage: scaleup
signals: strong_title=False gap=True jd_lexicon=True
JD excerpt (snapshot):
```
MORSE Corp is an employee owned, small business based in Cambridge, MA, Arlington, VA, and Seattle, WA with a history of fielding cutting-edge technology. MORSE boasts a specially selected team of scientists, engineers, and software developers to deliver best-in-class technical solutions that solve difficult multidisciplinary problems faced by the US National Security Ecosystem.  

We are seeking a Deputy Chief Engineer to lead our efforts in developing and deploying capabilities for artificial intelligence, machine learning, data management, decision systems, and mission planning software applications for the US National Security Ecosystem. This is an exceptional opportunity for an engineer with a combination of technical expertise, program leadership experience, and the ability to interface with customers to drive innovation, collaboration, and excellence in a fast-paced, growing organization.

### Responsibilities:

-

Set the technical vision and direction for programs, collaborating with internal technical staff and external customers

-

Lead multidisciplinary teams developing algorithms, software, cloud, and data management solutions

-

Manage program execution, ensuring sc
```

## Company's FULL current job list (fresh, from today's ATS feed)
- Cambridge, MA
- Autonomy Software Engineer
- Cambridge, MA, Arlington, VA 
- Chief Engineer
- Chief Engineer - AI Test \u0026 Evaluation (T\u0026E)
- Chief Engineer - Battle Management 
- Cloud Engineer
- Cloud Software Engineer
- Data Scientist
- Arlington, VA
- Data Scientist- TS/SCI 
- Cambridge, MA, Arlington, VA, Seattle, WA 
- Deputy Chief Engineer
- Embedded Software Engineer 
- Cambridge, MA 
- Flight Test Engineer 
- Front End Software Engineer 
- Full Stack Software Engineer
- Human Computer Interaction Engineer 
- Cambridge, MA, Arlington, VA
- Human Factors Engineer
- IT Helpdesk Specialist 
- Lead Data Scientist (TS/SCI)
- Manufacturing Engineer 
- Python Software Engineer
- R\u0026D Technician
- Robotics Software Engineer 
- Systems Engineer 
- Technical Recruiter